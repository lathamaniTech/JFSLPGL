     ...
     BARCODE_EAN_13_STANDARD : 0x05,
	  ...
	  RDSERVICE_NOT_AVAILABLE:-85,
        ...
        vDeviceInitialization: function(fnSuccess, fnError,retainConn){
		  exec(fnSuccess, fnError, "RDService", "vDeviceInitialization", [retainConn]);
        },
   	
   
        // ListBTName
		vListDevBTName: function(fnSuccess, fnError){
		  exec(fnSuccess, fnError, "RDService", "vListDevBTName", []);
	    },
   
       // ListDMAC
        vListDevBTMAC: function(fnSuccess, fnError){
          exec(fnSuccess, fnError, "RDService", "vListDevBTMAC", []);
        },
		
		// GetMACFromDeviceName
        vGetBTName: function(fnSuccess, fnError,dmac){
          exec(fnSuccess, fnError, "RDService", "vGetBTName", [dmac]);
        },
		
		// GetBTNameFromDeviceMAC
        vGetBTMAC: function(fnSuccess, fnError,btname){
          exec(fnSuccess, fnError, "RDService", "vGetBTMAC", [btname]);
        },
   
        // FlushBuffer
        vFlushBuf: function(fnSuccess, fnError){
         exec(fnSuccess, fnError, "RDService", "vFlushBuf", []);
        },
		 
       // TestPrint
		vTestPrint: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vTestPrint", [txn,btname,dmac]);
        },
		
       // AddData
       vAddData: function(fnSuccess, fnError, font, sInData){
		  exec(fnSuccess, fnError, "RDService", "vAddData",[font, sInData]);
        },
   
      /*//AddLine
        vAddLine: function(fnSuccess, fnError, font, chSymbol){
		  exec(fnSuccess, fnError, "RDService", "vAddLine", [font, chSymbol]);
        },*/
   
		// vGetTextPrintXML 
	    vGetTextPrintXML : function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetTextPrintXML", [txn,btname,dmac]);
	   },
   
		// vGetBarcodePrintXML
		vGetBarcodePrintXML: function(fnSuccess, fnError,type,data,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetBarcodePrintXML", [type,data,txn,btname,dmac]);
	   },
   
	   // vGetBMP_FilePath_XML
	   vGetBMP_FilePath_XML: function(fnSuccess, fnError,filepath,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetBMP_FilePath_XML",[filepath,txn,btname,dmac]);
	   },
   
	   // vGetBMP_Base64_XML
	   vGetBMP_Base64_XML: function(fnSuccess, fnError,imageData,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetBMP_Base64_XML",[imageData,txn,btname,dmac]);
	   },
   
	   // vGetGrayScale_FilePath_XML
	   vGetGreyScale_FilePath_XML: function(fnSuccess, fnError,filepath,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetGrayScale_FilePath_XML",[filepath,txn,btname,dmac]);
	   },
   
	   // vGetGrayScale_Base64_XML
	   vGetGreyScale_Base64_XML: function(fnSuccess, fnError,imageData,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vGetGrayScale_Base64_XML",[imageData,txn,btname,dmac]);
	   },
   
	   // Read MAGCard
	   /*vMAGCard: function(fnSuccess, fnError,timeout,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vMAGCard",[timeout,txn,btname,dmac]);
	   },*/
   
		// CombinedXML
		vGetCombinedXML: function(fnSuccess, fnError,xml_one,xml_two){
		  exec(fnSuccess, fnError, "RDService", "vGetCombinedXML",[xml_one,xml_two]);
	   },
   
	   // PaperFeed
		vPaperFeed: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vPaperFeed",[txn,btname,dmac]);
	   },
   
	   // Diagnostics
		vDiagnostics: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vDiagnostics",[txn,btname,dmac]);
	   },
   
	   // IdentifyPeripherals
		vIdentifyPeripherals: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vIdentifyPeripherals",[txn,btname,dmac]);
	   },
   
       // Disconnect BT
	   vDisconnect: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vDisconnect",[txn,btname,dmac]);
	   },
	   
	   // BatteryStatus
		vBatteryStatus: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vBatteryStatus",[txn,btname,dmac]);
	   },
   
		// SerialNumber
		vSerialNumber: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vSerialNumber",[txn,btname,dmac]);
	   },
   
	   // FirmwareVersion
		vFirmwareVersion: function(fnSuccess, fnError,txn,btname,dmac){
		  exec(fnSuccess, fnError, "RDService", "vFirmwareVersion",[txn,btname,dmac]);
	   },
   
        // StartPrint
		vStartPrint: function(fnSuccess, fnError,inxml){
		  exec(fnSuccess, fnError, "RDService", "vStartPrint",[inxml]);
	   },
   
	   // Capture
		vCapture: function(fnSuccess, fnError,pid_option){
		  exec(fnSuccess, fnError, "RDService", "vCapture",[pid_option]);
	   },
	   
	    // GetCaptureXml
		vGetPidOptionXML: function(fnSuccess, fnError,timeout,pidver,format,ftype,fcount,env,demotag,btconval,btname,dmac,wadh,otp){
		  exec(fnSuccess, fnError, "RDService", "vGetPidOptionXML",[timeout,pidver,format,ftype,fcount,env,demotag,btconval,btname,dmac,wadh,otp]);
	    },
   
	   // Device_Info
		vDevice_Info: function(fnSuccess, fnError){
		  exec(fnSuccess, fnError, "RDService", "vDevice_Info", []);
	   },
   
	   // RDService_Info
		vRDService_Info: function(fnSuccess, fnError){
		  exec(fnSuccess, fnError, "RDService", "vRDService_Info", []);
	   }, 
};

module.exports = RDService;
