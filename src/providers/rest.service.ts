...
import { HttpClient, HttpRequest, HttpEvent, HttpEventType, HttpHeaders, HttpInterceptor } from '@angular/common/http';
  ...
  restApiCallAngular(method, data, isKarza?): any {
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      if (method == "finalSubmission") {
        ...
        this.sqliteService.addAuditTrail(curDateTime, link, method + " Request", JSON.stringify(exceptDocument));
      } else {
        if (method != "LoginDocument" && method != "UploadDocs") {
          this.sqliteService.addAuditTrail(curDateTime, link, method + " Request", JSON.stringify(data));
        }
      }
        ...
        this.global.getCertPinningStatus().then(data => {
          if (data == true) {
            ...
            this.http.post(link, encryptData, headers).then(response => {
              ...
              this.sqliteService.addAuditTrail(curDateTime, link, method + " Response", JSON.stringify(decryRes));
              resolve(JSON.parse(decryRes.data));
            }, error => {
              reject(error);
              console.log(error, "error");
              this.global.globalLodingDismiss();
              this.global.showAlert('Alert',error);
              this.sqliteService.addAuditTrail(curDateTime, link, method + " Error Response", JSON.stringify(error));
            })
          } else {
            this.global.globalLodingDismiss();
            this.global.showAlert("Alert", "Certificates pinning error!");
          }
        })
      })
    }
    ...
    return new Promise(async (resolve, reject) => {
      if (value) {
        for (var i = 0; i < value.length; i++) {
          await this.sqliteService.getProductValuesCount(value[i]).then((data: any) => {
            userPrdCode.push({ 'main': data[0].prdSchemeId, 'sub': data[0].prdCode });
          })
          if (i == value.length - 1) {
            resolve(userPrdCode);
          }
