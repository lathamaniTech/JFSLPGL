...
export class CasadetailsComponent {
  ...
  accDetailsSave(value) {
    this.sqlSupport.InsertCASADetails(this.refId, this.id, this.usertype, value, this.casaId, '1').then(data => {
      if (this.casaId == null || this.casaId == '' || this.casaId == undefined) {
        this.casaId = data.insertId;
        this.globalData.showAlert("Alert!", "Current/Savings Account details saved!").then(data => {
          if (this.accDetails.get('janaAcc').value == 'N') {
            this.proceedNextPage();
          }
        })
      } else {
        this.globalData.showAlert("Alert!", "Current/Savings Account details updated!").then(data => {
          if (this.accDetails.get('janaAcc').value == 'N') {
            this.proceedNextPage();
          }
        })
  ...
  getNomineeDetails(event) {
    ...
    if (value == 'Y') {
      ...
      this.sqlSupport.getNomineeDetails(this.refId, this.id).then(async nom => {
        if (nom.length > 0) {
