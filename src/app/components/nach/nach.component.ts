...
export class NachComponent {
  ...
  private childConstructor(value) {
    ...
    this.nachDetails = this.formBuilder.group({
      ...
      nachBranName: ["", Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      nachACNumber: ["", Validators.compose([Validators.minLength(9), Validators.maxLength(17), Validators.pattern('[0-9]*'), Validators.required])],
      ...
      nachACName: ["", Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z ]*'), Validators.required])],
    });
  ...
  proceedNextPage() {
    let leadStatus;
    this.username = this.globalFunction.basicDec(localStorage.getItem('username'));
    ...
    this.globalData.proccedOk('Alert', 'proceed to existing leads').then(data => {
      if (data == 'yes') {
        this.navCtrl.navigate(['/ExistingPage'], { queryParams: { username: this.username, _leadStatus: leadStatus }, skipLocationChange: true, replaceUrl: true });
      }
    })
  }
    ...
    if(this.sanctionModify){
      if (this.nachImgs.length > 0) {
        if (this.nachStateImgs.length > 0) {
          this.sqlsupport.InsertNachDetails(this.refId, this.id, this.userType, value, this.nachId).then(data => {
            if (this.nachId == "" || this.nachId == undefined || this.nachId == null) {
              this.nachId = data.insertId;
              this.uploadnachImg();
              this.uploadNachStateImg();
              this.globalData.showAlert("Alert!", "Nach Details Added Successfully").then(data => {
                // this.proceedNextPage();
              });
            } else {
              this.updatenachImg(this.nachId);
              this.updateNachStateImg(this.nachId);
              this.globalData.showAlert("Alert!", "Nach Details Updated Successfully").then(data => {
                // this.proceedNextPage();
              });;
            }
            this.getNACHDetailsFromDB();
          ...
          this.globalData.showAlert("Alert!", "Please add Statement Image.");
        }
      } else {
        this.globalData.showAlert("Alert!", "Please add Cheque Image.");
      }
    }else{
      this.globalData.showAlert("Alert!", "Please complete Post Sanction Modification Details.");
    }
  }
  ...
  async showpicmaodal(value) {
    if (value == "nachcheque") {
      let emodal = await this.modalCtrl.create({
        ...
        componentProps: { nachpics: this.nachImgs, submitstatus: this.submitDisable }
      })
      ...
      let emodal = await this.modalCtrl.create({
        component: SignAnnexImgsPage,
        componentProps: { nachStatePics: this.nachStateImgs, submitstatus: this.submitDisable }
      });
      emodal.onDidDismiss().then((data: any) => {
        this.nachStateImgs = [];
  ...
  uploadnachImg() {
    for (let i = 0; i < this.nachImgs.length; i++) {
      this.sqlsupport.addNachImages(this.refId, this.id, this.nachImgs[i].imgpath, this.nachId).then(data => {
  ...
  updatenachImg(nachId) {
    this.sqlsupport.removeNachImages(nachId).then(data => {
      for (let i = 0; i < this.nachImgs.length; i++) {
        this.sqlsupport.addNachImages(this.refId, this.id, this.nachImgs[i].imgpath, nachId).then(data => {
        }).catch(Error => {
          console.log("Failed!" + Error);
          this.globalData.showAlert("Alert!", "Failed!");
        })
  ...
  uploadNachStateImg() {
    for (let i = 0; i < this.nachStateImgs.length; i++) {
      this.sqlsupport.addNachStateImages(this.refId, this.id, this.nachStateImgs[i].imgpath, this.nachId).then(data => {
        // console.log("promoter image insert" + data);
      }).catch(Error => {
        console.log("Failed!" + Error);
        this.globalData.showAlert("Alert!", "Failed!");
  ...
  updateNachStateImg(nachId) {
    this.sqlsupport.removeNachStateImages(nachId).then(data => {
      for (let i = 0; i < this.nachStateImgs.length; i++) {
        this.sqlsupport.addNachStateImages(this.refId, this.id, this.nachStateImgs[i].imgpath, nachId).then(data => {
        }).catch(Error => {
          console.log("Failed!" + Error);
          this.globalData.showAlert("Alert!", "Failed!");
        })
  ...
  getNACHDetailsFromDB() {
    this.sqlSupport.getNachDetails(this.userInfo.refId, this.userInfo.id).then(data => {
      if (data.length > 0) {
        this.NACH_data = data;
  ...
  getNachcash(nachId) {
    this.sqlSupport.getNachImages(nachId).then(nach => {
      if (nach.length > 0) {
        this.nach_imgs = [];
        for (let i = 0; i < nach.length; i++) {
          // this.base64.encodeFile(nach[i].imgpath).then((base64File: string) => {
  ...
  getAppCibilCheckStatus() {
    this.sqliteProvider.getSubmitDetails(this.userInfo.refId, this.userInfo.id).then(data => {
      ...
      if (submitlead[0].NACH == "0") {
        ...
        this.master.restApiCallAngular('updateNachDetails', this.nachSubmitData).then(data => {
          if ((<any>data).ErrorCode == "000") {
            ...
            this.sqlSupport.updateNACHSubmitDetails('1', this.applicationNumber).then(data => {
              this.submitDisable = true;
  ...
  uploadDocs() {
    if (this.nachDoc == 'Y') {
      this.globalData.showAlert("Alert!", "Documents already Submitted!");
    } else {
      // this.globalFunction.globalLodingPresent('Please wait...');
      let docs_upload = {
        "OtherDoc": {
      ...
      this.master.restApiCallAngular('UploadDocs', docs_upload).then(data => {
        if ((<any>data).errorCode == "00") {
          this.sqlSupport.updateNACHDocSubmitDetails('Y', this.applicationNumber).then(data => {
            this.submitNach = true;
            // this.globalFunction.globalLodingDismiss();
            this.globalData.showAlert("Alert!", "Documents Submitted Successfully!");
          })

        } else {
          // this.globalFunction.globalLodingDismiss();
          this.globalData.showAlert("Alert!", "Documents Submitted Failed!");
        }
      }, err => {
        // this.globalFunction.globalLodingDismiss();
        if (err.name == "TimeoutError") {
  ...
  checkEnachRequest() {
    ...
    this.master.restApiCallAngular('EnachRequest', body).then(res => {
      ...
      if (response.success == true) {
        ...
        this.globalFunction.globalLodingDismiss();
        this.globalFunction.showAlert("Alert!", response.fetchAlert);
        this.eRequest = true;
      }
    })
  }
  ...
  checkEnachStatus() {
    ...
    this.master.restApiCallAngular('EnachStatus', body).then(res => {
      ...
      if (response.success == true) {
        // this.eRequest = false;
        this.eStatus = false;
        this.globalFunction.showAlert("Alert!", response.success);
        this.globalFunction.globalLodingDismiss();
      } else {
        this.globalFunction.globalLodingDismiss();
        this.globalFunction.showAlert("Alert!", response.fetchAlert);
