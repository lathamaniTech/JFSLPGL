...
export class NomineedetailsComponent {
  ...
  constructor(public navCtrl: NavController,
    ...
    public sqlSupport: SquliteSupportProviderService, public globalData: DataPassingProviderService,
    // public global: DataPassingProvider, 
    public sqliteProvider: SqliteService,
    // public events: Events, 
    public platform: Platform, public globFunc: GlobalService) {
    ...
    this.globalData.nomineeValue.subscribe(data => {
      debugger
      console.log(data);
      (data == 'Y' || data.length > 2) ? this.fetchGuaranDetails(data) : (
  ...
  nomiDetailsSave(value) {
    this.sqlSupport.InsertNomineeDetails(this.refId, this.id, this.usertype, this.leadId, value, this.nomId).then(data => {
      if (this.nomId == null || this.nomId == '' || this.nomId == undefined) {
        this.globalData.showAlert("Alert!", "Nominee details saved!");
      } else {
        this.globalData.showAlert("Alert!", "Nominee details updated!");
      }
    ...
    this.sqlSupport.getCASADetails(this.refId, this.id).then(gua => {
      ...
      if ((gua.length > 0 && gua[0].guaAvail == "Y") || subData) {
        ...
        this.sqliteProvider.getNomList(this.refId, nomId).then(data => {
          ...
          this.nomiDetails.get('nomi_address1').setValue(this.globFunc.basicDec(data[0].perm_plots));
          this.nomiDetails.get('nomi_address2').setValue(this.globFunc.basicDec(data[0].perm_locality));
          this.nomiDetails.get('nomi_address3').setValue(this.globFunc.basicDec(data[0].perm_line3));
          this.nomiDetails.get('nomicities').setValue(this.globFunc.basicDec(data[0].perm_cities));
          this.nomiDetails.get('nomistates').setValue(this.globFunc.basicDec(data[0].perm_states));
          this.nomiDetails.get('nomipincode').setValue(this.globFunc.basicDec(data[0].perm_pincode));
          this.nomiDetails.get('nomicountries').setValue(data[0].perm_countries);
          this.nomiDetails.get('nomiCNum').setValue(this.globFunc.basicDec(data[0].mobNum));
