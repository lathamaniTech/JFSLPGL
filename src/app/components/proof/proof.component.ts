...
export class ProofComponent implements OnInit {
  ...
  proofpromotersave(value) {
    try {
      if (this.aadhar && this.vaultStatus == 'N') {
        ...
        if ((this.showBusiness && localStorage.getItem('Business') == "businessAddSaved") || (!this.showBusiness && localStorage.getItem('Present') == "presentAddSaved")) {
          if (this.proofImgs.length > 0) {
            this.refId = this.globalData.getrefId();
            this.id = this.globalData.getId();
            this.sqliteProvider.getPromoterProofDetailsByProof(this.refId, value.promoIDRef).then(proofData => {
              if (proofData.length > 0) {
                this.sqliteProvider.getPromoterProofDetailsByProofID(this.refId, this.id, value.promoIDType).then(sameApp => {
                  if (sameApp.length > 0) {
                    ...
                    this.globalData.globalLodingDismiss();
                    this.globalData.showAlert("Alert", "Same ID proof is already captured in the same application!!!");
                  }

                }).catch(error => {
                  this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-proofpromotersave-getPromoterProofDetailsByProofID");
                });
                ...
                this.sqliteProvider.addPromotersProofDetails(this.refId, this.id, value, this.vaultStatus, this.proofName, this.pproofId).then(data => {
                  if (this.pproofId === "" || this.pproofId === null || this.pproofId === undefined) {
                    ...
                    this.globalData.globalLodingDismiss();
                    if (this.userType == "A") {
                      this.globalData.showAlert("Alert!", "Borrower Proof Details Added Successfully");
                    }
                  ...
                  this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-proofpromotersave-addPromotersProofDetails");
                })
              }
            }).catch(error => {
              this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-proofpromotersave");
            });
          }
          else {
            this.globalData.globalLodingDismiss();
          ...
          if (this.showBusiness) {
            this.globalData.showAlert("Alert!", "Must Save Business Address Details");
          } else {
            this.globalData.showAlert("Alert!", "Must Save Present Address Details");
          }
        }
      }
    } catch (error) {
      ...
      modal.onDidDismiss().then(async (proofvalue: any) => {
        ...
        if (value == "proofDelete") {
          ...
          switch (this.proofType) {
            case 'ProofPromoter':
              this.clearpromo();
              if (kycdata.promoexpiry == null || kycdata.promoexpiry == "null") {
                kycdata.promoexpiry = "";
              }
              ...
              if (kycdata.proofName == "AADHAR CARD" || kycdata.proofName == "AADHAR") {
    ...
    try {
      ...
      if (custType == '1') {
        ...
        this.sqliteProvider.getDocumentsByIndividualPrdCode(prdCode, entityStat).then(data => {
          ...
          this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocumentValue-getDocumentsByIndividualPrdCode");
        });
      } else {
        entityStat = 'Y';
        this.sqliteProvider.getDocumentsByPrdCode(prdCode).then(data => {
          ...
          this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocumentValue-getDocumentsByPrdCode");
        });
      }
    } catch (error) {
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-viewIdProof");
      ...
      if (this.custType == "E") {
        ...
        this.sqliteProvider.checkProofDetails(docCode, this.id).then(data => {
          if (data.length > 0) {
            ...
            this.sqliteProvider.selectExData(this.globalData.getURN()).then(data => {
              this.proofImgs = [];
              this.proofImglen = 0;
              this.getDocumentsValidator(docCode, data);
            }).catch(error => {
              this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocuments-selectExData");
            });
          }
        }).catch(error => {
          this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocuments-checkProofDetails");
        });
      } else {
        this.sqliteProvider.checkProofDetails(docCode, this.id).then(data => {
          if (data.length > 0) {
            ...
            this.proofImgs = [];
            this.proofImglen = 0;
            if (this.getDocumentName(docCode) == 'DRIVING LICENSE' || this.getDocumentName(docCode) == 'DRIVING LISENCE') { // DL
              this.getStoredDrivingLicense(docCode);
            } else if (this.getDocumentName(docCode) == 'PASSPORT') { // Pasport
              this.getStoredPassport(docCode);
            } else if (this.getDocumentName(docCode) == 'PAN CARD' || this.getDocumentName(docCode) == 'PAN') { // PAN
              ...
              if (this.userType == 'A') {
                ...
                this.sqlSupport.getEKYCDetails(localStorage.getItem('leadId')).then(data => {
                  if (data.length > 0) {
                    if (data[0].janaid != '' && data[0].janaid != undefined && data[0].janaid != null) {
                      this.idNumDisable = true;
                      this.aadhar = true;
                      this.expdate = false;
                      this.janaRefId = true;
                  ...
                  this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocuments-getEKYCDetails");
                });
              } else {

                this.sqlSupport.getEKYCDetails(localStorage.getItem('leadId')).then(ekyc => {
                  if (ekyc.length > 0) {
                    if (ekyc[0].janaid) {
                      this.idNumDisable = true;
                      this.aadhar = true;
                      ...
                      this.proofData.controls.aepsStatus.setValue(ekyc[0].aepsFlag);
                      this.vaultStatus = 'Y';
                      this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(12), Validators.minLength(12), Validators.pattern('[0-9]{12}'), Validators.required]));
                      this.proofData.controls.promoexpiry.clearValidators();
                      this.proofData.controls.promoIDRef.updateValueAndValidity();
              ...
              this.sqliteProvider.getEntityDetails(this.refId, this.id).then(data => {
                if (data.length > 0) {
                  if (data[0].gst != "" && data[0].gst != undefined && data[0].gst != null) {
                    this.proofName = this.getDocumentName(docCode);
                    this.aadhar = false; this.textType = 'text'; this.vaultDisable = false; this.janaRefId = false; this.vaultStatus = '';
                    this.proofData.controls.promoIDRef.setValue(data[0].gst);
              ...
              this.sqliteProvider.getEntityDetails(this.refId, this.id).then(data => {
                if (data.length > 0) {
                  if (data[0].cin != "" && data[0].cin != undefined && data[0].cin != null) {
                    this.proofName = this.getDocumentName(docCode);
                    this.aadhar = false; this.textType = 'text'; this.vaultDisable = false; this.janaRefId = false; this.vaultStatus = '';
                    this.proofData.controls.promoIDRef.setValue(data[0].cin);
              ...
              this.proofName = this.getDocumentName(docCode);
              this.proofData.controls.promoIDRef.setValue('');
              this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(20), Validators.minLength(10), Validators.pattern('[a-zA-Z0-9]*'), Validators.required]));
              this.proofData.controls.promoexpiry.clearValidators();
              this.proofData.controls.promoIDRef.updateValueAndValidity();
              this.proofData.controls.promoexpiry.updateValueAndValidity();
              this.proofData.controls.promoexpiry.clearValidators();
  ...
  async showpicmaodal(value) {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-showpicmaodal");
    }
  }
  ...
  uploadimg() {
    for (let i = 0; i < this.proofImgs.length; i++) {
      this.sqliteProvider.addpromoterproofImages(this.refId, this.id, this.proofImgs[i].imgpath, this.pproofId).then(data => {
        ...
        this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-uploadimg-addpromoterproofImages");
      });
      //alert(i);
    }
    this.proofImgs = [];
    ...
    this.sqliteProvider.removepromoterproofImages(upid).then(data => {
      ...
      for (let i = 0; i < this.proofImgs.length; i++) {
        this.sqliteProvider.addpromoterproofImages(this.refId, this.id, this.proofImgs[i].imgpath, upid).then(data => {
          ...
          console.log("Failed!" + error);
          this.globalData.showAlert("Alert!", "Failed!");
          this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-updateimg-addpromoterproofImages");
        })
        //alert(i);
      }
      this.proofImgs = [];
      ...
      console.log("Failed!" + Error);
      this.globalData.showAlert("Alert!", "Failed!");
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-updateimg-removepromoterproofImages");
    })
  }
  ...
  getIdProofLength() {
    this.sqliteProvider.getPromoterProofDetails(this.refId, this.id).then(data => {
      ...
      console.log(error);
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getIdProofLength-getPromoterProofDetails");
    });
  }
  ...
  aadharValidation() {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-aadharValidation");
    }
  }
  ...
  getCibilCheckedDetails() {
    ...
    this.sqliteProvider.getSubmitDetails(this.refId, this.id).then(data => {
      ...
      console.log(error);
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getCibilCheckedDetails-getSubmitDetails");
    });
  }
  ...
  callmaxdate(docCode?) {
    try {
      ...
      let yyyy;
      if (this.getDocumentName(docCode) == 'DRIVING LICENSE' || this.getDocumentName(docCode) == 'DRIVING LISENCE') {
  ...
  clearpromoall() {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-clearpromoall");
    }
  }
  ...
  getDocumentName(value: string) {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocumentName");
    }
  }
      ...
      this.sqliteProvider.getMandDocumentsByPrdCode(docs).then(data => {
        ...
        for (let i = 0; i < this.mandDocs.length; i++) {
          this.sqliteProvider.getPromoterProofDetailsByProofID(this.refId, this.id, this.mandDocs[i].DocID).then(data => {
            ...
            console.log(error);
            this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-mandDocsCheck-getMandDocumentsByPrdCode-getPromoterProofDetailsByProofID");
          });
        }
      }).catch(error => {
        console.log(error);
        this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-mandDocsCheck-getMandDocumentsByPrdCode");
      });
    } catch (error) {
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-clearpromoall");
    }
  ...
  mandDocsCheckcount(docLength) {
    try {
      ...
      this.sqliteProvider.getProductValuesCount(localStorage.getItem('product')).then(data => {
        ...
        console.log(error);
        this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-mandDocsCheckcount-getProductValuesCount");
      });
    } catch (error) {
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-mandDocsCheckcount");
    }
  }
    ...
    try {
      if (this.network.type === 'none' || this.network.type == 'unknown') {
        ...
        if (aepsValue === "" || aepsValue === undefined || aepsValue === null) {
          this.globalData.showAlert("Alert!", "Please Select AEPS Status");
        } else {
          ...
          this.master.restApiCallAngular('AadharInsertVoulting', body).then((data) => {
            if ((<any>data).status == '00') {
              this.sqliteProvider.getPromoterProofDetailsByProof(this.refId, (<any>data).janaId).then(pro => {
                ...
                console.log(error);
                this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-aadhaarVaultInsert-getPromoterProofDetailsByProof");
              });
            } else {
              this.vaultDisable = false;
              this.janaRefId = false;
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-aadhaarVaultInsert");
    }
  }
  ...
  aadhaarVaultRetrieve(value) {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-aadhaarVaultRetrieve");
    }
  }
  ...
  getstoredPAN(docCode) {
    this.sqliteProvider.getPersonalDetailsByLeadId(localStorage.getItem('leadId')).then(data => {
      if (data.length > 0) {
        if (data[0].panNum != "" && data[0].panNum != undefined && data[0].panNum != null) {
          this.proofName = this.getDocumentName(docCode);
          this.aadhar = false; this.textType = 'text'; this.vaultDisable = false; this.janaRefId = false; this.vaultStatus = '';
          ...
          setTimeout(() => {
            if (this.userType == "A") {
              this.globalData.showAlert("Alert!", "Kindly update the PAN availability in Borrower Details!");
            } else if (this.userType == "C") {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocuments-getDocNumberFromPersonalDetails");
    });
  }

  getStoredNerga(docCode) {
    this.sqliteProvider.getDocNumberFromPersonalDetails(localStorage.getItem('leadId'), 'nregea').then(nregeadata => {
      if (nregeadata.length > 0) {
        this.proofName = this.getDocumentName(docCode);
        this.proofData.controls.promoIDRef.setValue(nregeadata[0].docValue);
        this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(20), Validators.minLength(10), Validators.pattern('[a-zA-Z0-9]*')]));
        this.proofData.controls.promoIDRef.updateValueAndValidity();
        this.proofData.controls.aepsStatus.clearValidators();
        this.proofData.controls.aepsStatus.updateValueAndValidity();
        this.idNumDisable = true;
        ...
        this.proofName = this.getDocumentName(docCode);
        this.proofData.controls.promoIDRef.setValue('');
        this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(20), Validators.minLength(10), Validators.pattern('[a-zA-Z0-9]*')])); //Validators.pattern(/^[a-zA-Z0-9-\/]*$/),
        this.proofData.controls.promoIDRef.updateValueAndValidity();
      }
    })
  }
  ...
  getStoredDrivingLicense(docCode) {
    this.sqliteProvider.getKarzaData(localStorage.getItem('leadId'), 'licence').then(data => {
      if (data.length > 0) {
        this.proofName = this.getDocumentName(docCode);
        this.proofData.controls.promoIDRef.setValue(data[0].idNumber);
        this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(17), Validators.minLength(15), Validators.required, Validators.pattern(/^[a-zA-Z0-9-\/]*$/)]));
        ...
        this.sqliteProvider.getDocNumberFromPersonalDetails(localStorage.getItem('leadId'), 'licence').then(dl => {
          if (dl.length > 0) {
            this.proofName = this.getDocumentName(docCode);
            ...
            this.proofName = this.getDocumentName(docCode);
            this.proofData.controls.promoIDRef.setValue('');
            this.proofData.controls.promoIDRef.setValidators(Validators.compose([Validators.maxLength(17), Validators.minLength(15), Validators.required, Validators.pattern(/^[a-zA-Z0-9-\/]*$/)])); //Validators.pattern(/^[a-zA-Z0-9-\/]*$/),
            this.proofData.controls.promoexpiry.setValidators(Validators.compose([Validators.required]));
            this.proofData.controls.promoexpiry.updateValueAndValidity();
            this.proofData.controls.promoIDRef.updateValueAndValidity();
            this.proofData.controls.aepsStatus.clearValidators();
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocuments-getKarzaData");
    });
  }

  getStoredPassport(docCode) {
    this.sqliteProvider.getKarzaData(localStorage.getItem('leadId'), 'passport').then(data => {
      if (data.length > 0) {
        this.proofName = this.getDocumentName(docCode);
        this.aadhar = false; this.textType = 'text'; this.vaultDisable = false; this.janaRefId = false; this.vaultStatus = '';
        this.proofData.controls.promoIDRef.setValue(data[0].idNumber);
  ...
  getStoredVoterId(docCode) {
    this.sqliteProvider.getKarzaData(localStorage.getItem('leadId'), 'voterid').then(data => {
      if (data.length > 0) {
        this.proofName = this.getDocumentName(docCode);
        this.proofData.controls.promoIDRef.setValue(data[0].idNumber);
        ...
        this.sqliteProvider.getDocNumberFromPersonalDetails(localStorage.getItem('leadId'), 'voterid').then(voter => {
          if (voter.length > 0) {
            this.proofName = this.getDocumentName(docCode);
  ...
  getDocumentsValidator(docCode, value) {
    try {
      if (this.getDocumentName(docCode) == 'DRIVING LICENSE' || this.getDocumentName(docCode) == 'DRIVING LISENCE') { // DL
        if (value[0].DrivingLic && /^[a-zA-Z0-9-\/]*$/.test(value[0].DrivingLic) && value[0].DrivingLic.length >= 15 && value[0].DrivingLic.length <= 17) {
          this.proofData.controls.promoIDRef.clearValidators();
          this.proofData.controls.promoexpiry.clearValidators();
        ...
        if (value[0].EKycStatus == 'Y' || value[0].EKycStatus == 'Positive') {
          if (value[0].JanaReferenceID != "") {
            ...
            this.vaultDisable = true;
            this.janaRefId = true;
            this.proofData.controls.promoIDRef.setValue(value[0].JanaReferenceID);
          ...
          this.sqlSupport.getEKYCDetails(localStorage.getItem('leadId')).then(data => {
            ...
            this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getEKYCDetails");
          });
        }
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-getDocumentsValidator");
    }
  }
  ...
  updateProofImg() {
    try {
      ...
      this.globalData.globalLodingDismiss();
      if (this.userType == "A") {
        this.globalData.showAlert("Alert!", "Borrower Proof Details Updated Successfully");
      }
      else if (this.userType == "G") {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "ProofComponent-updateProofImg");
    }
  }
}
