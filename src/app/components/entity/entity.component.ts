...
export class EntityComponent {
  ...
  months: any = [{ month: 0, value: 0 }, { month: 1, value: 1 }, { month: 2, value: 2 }, { month: 3, value: 3 },
  { month: 4, value: 4 }, { month: 5, value: 5 }, { month: 6, value: 6 }, { month: 7, value: 7 },
  { month: 8, value: 8 }, { month: 9, value: 9 }, { month: 10, value: 10 }, { month: 11, value: 11 }
  ]

  curAccounts = [{ code: "Y", name: 'YES' }, { code: "N", name: 'NO' }]
  ...
  constructor(public navCtrl: Router, public navParams: NavParams,
    ...
    public network: Network, public sqlSupport: SquliteSupportProviderService) {
    ...
    this.entityData = this.formBuilder.group({
      enterName: ["", Validators.compose([Validators.maxLength(40), Validators.pattern('^[a-zA-Z0-9 ]*'), Validators.required])],
      constitution: ["", Validators.required],
      cin: ["", Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
      regNo: ["", Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
      gst: ["", Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
  ...
  entitysave(value) {
    ...
    if (saveStatus == "sourcingSaved") {
      if (this.entiProfPic) {
        this.applicantUniqueId = Math.floor(Math.random() * 900000000000) + 100000000000;
        this.applType = 'a';
        this.custType = this.globalData.getCustType();
        this.sqliteProvider.addEntityDetails(this.refId, this.id, value, this.profPic, this.entiProfPic, this.applicantUniqueId, this.applType, this.custType, this.leadStatus, this.entId).then(data => {
          if (this.entId === "" || this.entId === null || this.entId === undefined) {
        ...
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert("Alert!", "Must Capture the Entity Profile Image!");
      }
    } else {
      this.globalData.globalLodingDismiss();
  ...
  getEntityDetails() {
    this.sqliteProvider.getEntityDetails(this.refId, this.id).then(data => {
      ...
      this.entityData = this.formBuilder.group({
        enterName: [this.getEntityData[0].enterName, Validators.compose([Validators.maxLength(40), Validators.pattern('^[a-zA-Z0-9 ]*'), Validators.required])],
        constitution: [this.getEntityData[0].constitution, Validators.required],
        cin: [this.getEntityData[0].cin, Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
        regNo: [this.getEntityData[0].regNo, Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
        gst: [this.getEntityData[0].gst, Validators.compose([Validators.maxLength(20), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required])],
      ...
      this.profPic = this.globalData.setProfileImage(this.getEntityData[0].profPic);
      this.entiProfPic = this.globalData.setEntiProfileImage(this.getEntityData[0].entiProfPic);
  ...
  corpExperianceMonth() {
    ...
    if ((getExpYear == this._corpExp) && (getExpMonth > this._corpExpMonths)) {
      this.entityData.controls.monCurBusiAddr.setErrors({ 'inCorrect': true, 'errMsg': `Enter Month Value between 0 - ${this._corpExpMonths}` })
    } else if ((getExpYear == 0) && (getExpMonth == 0)) {
      this.entityData.controls.monCurBusiAddr.setErrors({ 'inCorrect': true, 'errMsg': `Enter Month Value between 1 - ${this._corpExpMonths}` })
    } else {
  ...
  getEnterValue(event?) {
    this.sqliteProvider.getMasterDataUsingType('NatureofBussiness').then(data => {
  ...
  constitutionChng(value) {
    if (value == '4' || value == '5' || value == '8') {
      ...
      this.entityData.controls.cin.setValidators(Validators.compose([Validators.maxLength(30), Validators.minLength(10), Validators.pattern('^[a-zA-Z0-9]*'), Validators.required]));
      this.entityData.controls.cin.updateValueAndValidity();
    } else {
      this.cinMand = false;
      this.entityData.controls.cin.clearValidators();
      this.entityData.controls.cin.setValidators(Validators.compose([Validators.maxLength(30), Validators.minLength(10), Validators.pattern('^[a-zA-Z0-9]*')]));
      this.entityData.controls.cin.updateValueAndValidity();
    }
  }
}
