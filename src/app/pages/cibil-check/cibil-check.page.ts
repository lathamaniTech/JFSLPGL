...
export class CibilCheckPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    public loadCtrl: LoadingController, public sqliteProvider: SqliteService,
    public globFunc: GlobalService,
    public network: Network, public master: RestService,
    public sqlSupport: SquliteSupportProviderService, public modalCtrl: ModalController,
  ...
  loadAllApplicantDetails() {
    this.sqliteProvider.getSubmitDataDetailsCibil(this.userInfo.refId).then(data => {
      this.items = [];
      this.items = data;
        ...
        if (coapp.length > 0) {
          if (this.coAppFlag == 'Y' && this.proceedCoAppStatus) {
            this.globalData.proccedOk("Alert", "Proceed to Document details").then(data => {
              if (data == "yes") {
                this.router.navigate(['/DocumentUploadPage'], { queryParams: { viewData: JSON.stringify(this.userInfo) },skipLocationChange: true, replaceUrl: true });
              }
            });
          }
        } else {
  ...
  getGuaCibilCheckStatus() {
    for (let i = 0; i < this.guarantors.length; i++) {
      this.sqliteProvider.getSubmitDetails(this.guarantors[i].refId, this.guarantors[i].id).then(res => {
  ...
  getAppCibilCheckStatus() {
    for (let i = 0; i < this.items.length; i++) {
      this.sqliteProvider.getSubmitDetails(this.items[i].refId, this.items[i].id).then(res => {
  ...
  getCoappCibilCheckStatus(value = true) {
    ...
    for (let i = 0; i < this.coapplicants.length; i++) {
      this.sqliteProvider.getSubmitDetails(this.coapplicants[i].refId, this.coapplicants[i].id).then(res => {
        if (res.length > 0) {
          ...
          if (i == this.coapplicants.length - 1) {
            ...
            if (coAppcibilStatus.includes('0') || coAppHimarkStatus.includes('0')) {
  ...
  async checkHimark(value) {
    if (this.himarkCheckStat == '0') {
      if (this.network.type == 'none' || this.network.type == "unknown") {
        this.globalData.showAlert('Alert!', 'Check your network data Connection');
      } else {
          ...
          if (data.data) {
            ...
            let body = {
              "CBHimarkRequest": {
                ...
                "clientName": this.items[0].firstname.toUpperCase() + " " + this.items[0].lastname.toUpperCase(),
                "memberID": this.items[0].coAppGuaId ? this.items[0].coAppGuaId : this.items[0].leadId,
                ...
                "city": this.getCityName(this.globFunc.basicDec(this.items[0].perm_cities)),
                "branchCode": this.items[0].loginBranch,
                "regionCode": this.getStateName(this.globFunc.basicDec(this.items[0].perm_states)),
            ...
            this.master.restApiCallAngular('CBhimark', body).then((res) => {
              loading.dismiss();
              // (<any>res).CBHimarkResponse.statusCode = "SUCCESS";
              if ((<any>res).CBHimarkResponse.statusCode.toUpperCase() === 'SUCCESS') {
                // this.himarkResult = {
                ...
                if(this.scheme == '1052' || this.scheme == '1060'){
                  if (+this.himarkResult.Score >= 21 && +this.himarkResult.Score < 650) {
                    this.rejectApplication()
                  } else {
                    let himarkstatus = "1";
                    this.sqliteProvider.updateHimark(this.himarkResult, himarkstatus, this.statId).then(data => {
                      this.sqliteProvider.updateCibilSubmitDetails('1', this.statId).then(data => {
                        this.globalData.showAlert("Alert!", "Hi-Mark Success!").then(data => {
                          this.getCibilCheckStatus();
                        });
                      })
                    }).catch(Error => {
                ...
                if (this.uatLocalStat) {
                  ...
                  this.globalData.showAlert("Alert!", (<any>res).CBHimarkResponse.ErrorDetails[0] ? (<any>res).CBHimarkResponse.ErrorDetails[0].errorDesc : (<any>res).CBHimarkResponse.statusCode);
                }
              }
            }, (err) => {
              loading.dismiss();
            ...
            // }
            // else {
            //   this.globalData.showAlert("Alert!", "Must Complete the CIBIL Process!");
            // }
          }
        })
      }
    } else {
      this.globalData.showAlert("Alert!", "Already Applicant Hi-Mark Checked!");
    }
  }
  ...
  async checkHimarkCoapplicant(coapp) {
    if (this.himarkGuaCheckStat == '0') {
          ...
          if (data) {
            ...
            let body = {
              "CBHimarkRequest": {
                ...
                "city": this.getCityName(this.globFunc.basicDec(coapp.perm_cities)),
                "branchCode": this.items[0].loginBranch,
                "regionCode": this.getStateName(this.globFunc.basicDec(coapp.perm_states)),
                "productCode": this.items[0].janaLoan,
            ...
            this.master.restApiCallAngular('CBhimark', body).then((res) => {
              ...
              if ((<any>res).CBHimarkResponse.statusCode.toUpperCase() == 'SUCCESS') {
                this.himarkResult = (<any>res).CBHimarkResponse;
                let himarkstatus = "1";
                this.sqliteProvider.updateHimark(this.himarkResult, himarkstatus, coapp.statId).then(data => {
                  this.sqliteProvider.updateCoCibilSubmitDetails('1', coapp.statId).then(data => {
                    this.globalData.showAlert("Alert!", "Hi-Mark Success!").then(data => {
                      this.getCoappCibilCheckStatus(true);
                    })
                  });
      ...
      this.globalData.showAlert("Alert!", "Already Gurantor Hi-Mark Checked!");
    }
  }
          ...
          if (data) {
            ...
            let body = {
              "CBHimarkRequest": {
                ...
                "city": this.getCityName(this.globFunc.basicDec(guarantor.perm_cities)),
                "branchCode": this.items[0].loginBranch,
                "regionCode": this.getStateName(this.globFunc.basicDec(guarantor.perm_states)),
                "productCode": this.items[0].janaLoan,
            ...
            this.master.restApiCallAngular('CBhimark', body).then((res) => {
              ...
              if ((<any>res).CBHimarkResponse.statusCode.toUpperCase() == 'SUCCESS') {
                this.himarkResult = (<any>res).CBHimarkResponse;
                let himarkstatus = "1";
                this.sqliteProvider.updateHimark(this.himarkResult, himarkstatus, guarantor.statId).then(data => {
                  this.sqliteProvider.updateCoCibilSubmitDetails('1', guarantor.statId).then(data => {
                    this.getGuaCibilCheckStatus();
                    this.globalData.showAlert("Alert!", "Hi-Mark Success!");
                  })
                }).catch(Error => {
                  console.log("Failed!");
                });
                ...
                if (this.uatLocalStat) {
                  ...
                  this.himarkResult = undefined;
                  this.globalData.showAlert("Alert!", (<any>res).CBHimarkResponse.ErrorDetails[0].errorDesc);
                }
              }
            }, (err) => {
              loading.dismiss();
  ...
  cibilConcern(value) {
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      if (this.red) {
        ...
        this.globalData.showAlert("Alert!", "Already Applicant Cibil Checked!");
      } else {
  ...
  promoterDocumentchecking(i) {
    ...
    //     }
    //     else {
    //       this.globalData.showAlert("Alert!", "Minimum " + parseInt(this.coapplicantMandatory[i].ProofMandatory) + " " + this.coapplicantMandatory[i].ProofName + " needed for Cibil Check in Applicant Kyc Documents..");
    //     }
    //   }).catch(Error => {
    //     console.log("Failed!");
    //   });
    ...
    this.router.navigate(['/OtpConcernPage'], { queryParams: { applicant: this.passvalue, cbusertype: "A" },skipLocationChange: true, replaceUrl: true })
    // }
  }

  // cibilConcernGuarantor(value) {
  //   if (this.cibilGuaCheckStat === '1') {
  //     this.globalData.showAlert("Alert!", "Already Guarantor Cibil Checked!");
  //   }
  //   else {
  ...
  //       }
  //       else {
  //         this.globalData.showAlert("Alert!", "Minimum Two KYC Document with Cibil needed for Cibil Check.")
  //       }
  //     }).catch(Error => {
  //       console.log("Failed!");
  ...
  cibilConcernCoapp(value) {
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      if (this.red) {
        ...
        this.globalData.showAlert("Alert!", "Already Co-Applicant Cibil Checked!");
      }
      else {
        this.sqliteProvider.getPromoterProofDetails(value.refId, value.id).then(prof => {
          if (prof.length > 0) {
            this.sqliteProvider.getPermanentAddress(value.refId, value.id).then(perm => {
              if (perm.length > 0) {
                this.sqliteProvider.getPresentAddress(value.refId, value.id).then(pers => {
                  if (pers.length > 0) {
                    this.sqliteProvider.getBusinessAddress(value.refId, value.id).then(busi => {
                      if (busi.length > 0) {
                        let manDocumentCount = 0;
                        let custType = this.globalData.getCustomerType();
                        this.sqlSupport.getBasicDetailsUserType(value.refId, "A").then(basic => {
                          if (basic) {
                            // this.sqliteProvider.getProductValuesCount(localStorage.getItem('product')).then(data => {
                            this.sqliteProvider.getProductValuesCount(basic[0].janaLoan).then(data => {
                              if (data) {
                                if (custType == "1") {
                                  if (value.userType == "A") {
                                ...
                                this.sqliteProvider.getPromoterProofDetails(value.refId, value.id).then(data => {
                                  if (data.length >= manDocumentCount) {
                                    ...
                                    this.globalData.showAlert("Alert!", "Please complete Co-Applicant Mandatory KYC Docs!");
                                  }
                                })
                              }
                            });
                        ...
                        this.globalData.showAlert("Alert!", "Please complete Co-applicant Business Address");
                      }
                    }).catch(err => {
                      console.log(err)
                    })
                  } else {
                    this.globalData.showAlert("Alert!", "Please complete Co-applicant Present Address");
                  }
                }).catch(err => {
                  console.log(err)
                })
                ...
                this.globalData.showAlert("Alert!", "Please complete Co-applicant Permanent Address");
              }
            }).catch(err => {
              console.log(err)
            })
          } else {
            this.globalData.showAlert("Alert!", "Please complete Co-applicant Proof Details");
          }
        }).catch(err => {
          console.log(err)
        })
  ...
  coappDocumentchecking(i) {
    this.docLength = 0;
    this.router.navigate(['/OtpConcernPage'], { queryParams: { coapp: this.coapppassvalue, cbusertype: "C" },skipLocationChange: true, replaceUrl: true })
  }
              ...
              if (data) {
                ...
                this.sqliteProvider.getPromoterProofDetails(value.refId, value.id).then(data => {
                  if (data.length >= manDocumentCount) {
                    ...
                    this.globalData.showAlert("Alert!", "Please complete Guarantor Mandatory KYC Docs!");
                  }
                });
              }
            });
  ...
  gurantorDocumentchecking(i) {
    ...
    //       }
    //       else {
    //         this.globalData.showAlert("Alert!", "Minimum " + parseInt(this.guMandatory[i].ProofMandatory) + " " + this.guMandatory[i].ProofName + " needed for Cibil Check in Gurantor Kyc Documents.");
    //       }
    //     }).catch(Error => {
    //       console.log("Failed!");
    //     });
    //   } else {
    this.docLength = 0;
    this.router.navigate(['/OtpConcernPage'], { queryParams: { gurantor: this.gupassvalue, cbusertype: "G" },skipLocationChange: true, replaceUrl: true })
    //   }
  ...
  GetHimarkDocuments() {
    ...
    this.sqliteProvider.GetCbildocs(this.id).then(data => {
      let details = [];
      let IDType = "";
      if (data.length > 0) {
        for (var i = 0; i < data.length; i++) {
          if (this.getDocumentName(data[i].promoIDType) == "PAN CARD") { // PAN CARD	
            IDType = "PAN"
          }
          else if (this.getDocumentName(data[i].promoIDType) == "AADHAR" || this.getDocumentName(data[i].promoIDType) == "AADHAR CARD") { // aadhar
            //IDType = "ad"
            this.appJanaRefId = this.globFunc.basicDec(data[i].promoIDRef);
          }
          else if (this.getDocumentName(data[i].promoIDType) == "PASSPORT") { // Passport
            IDType = "PASS"
          }
          else if (this.getDocumentName(data[i].promoIDType) == "VOTER ID") { //VOTER
            IDType = "VOTER"
          }
          else if (this.getDocumentName(data[i].promoIDType) == "DRIVING LICENSE") { // DL
            IDType = "DRIV"
          }
          else {
            IDType = "07"
          }
          if (this.getDocumentName(data[i].promoIDType) != "AADHAR" && this.getDocumentName(data[i].promoIDType) != "AADHAR CARD") { // aadhar
  ...
  GetCoappHimarkDocs() {
    ...
    // }
    if (this.coapplicants.length > 0) {
      this.sqliteProvider.GetCbildocs(this.coapplicants[0].id).then(data => {
        let details = [];
        let IDType = "";
        if (data.length > 0) {
          for (var i = 0; i < data.length; i++) {
            if (this.getDocumentName(data[i].promoIDType) == "PAN CARD") { // PAN CARD	
              ...
              //IDType = "ad"
              this.coappJanaRefId = this.globFunc.basicDec(data[i].promoIDRef);
            }
            else if (this.getDocumentName(data[i].promoIDType) == "PASSPORT") { // Passport
              IDType = "PASS"
            }
  ...
  rejectApplication() {
    if (this.network.type === 'none') {
      ...
      this.submitData = {
        "LeadMain": {
          "Lead": {
            ...
            "AppCreationdt": this.items[0].applDate.substring(0, 2) + "/" + this.items[0].applDate.substring(3, 5) + "/" + this.items[0].applDate.substring(6, 10),
            ...
            "ApplicantDetails": [{
              ...
              "JanaRefID": this.globFunc.basicDec(this.getKarzaData(this.items[0].coAppGuaId, 'aadhaar')) || "",
              "leadId": this.items[0].coAppGuaId,
              "VoterId": this.globFunc.basicDec(this.getKarzaData(this.items[0].coAppGuaId, 'voterid')) || "",
              "PassportNo": this.globFunc.basicDec(this.getKarzaData(this.items[0].coAppGuaId, 'passport')) || "",
              "OtherIdType": "1",
              "OtherIdvalue": this.globFunc.basicDec(this.getKarzaData(this.items[0].coAppGuaId, 'licence')) || "",
              "OtherIdExpireDate": "20/09/2035",
              "CustomerAddressDetails": [
                {
                  "AddressType": "1",
                  "Address1": this.globFunc.basicDec(this.items[0].perm_plots),
                  "Address2": this.globFunc.basicDec(this.items[0].perm_locality),
                  "Address3": "",
                ...
                },
                {
                  ...
                  "Address2": this.globFunc.basicDec(this.items[0].pres_locality),
                  "Address3": "",
      ...
      this.master.restApiCallAngular('rejectleads', this.submitData).then((res) => {
        if ((<any>res).ErrorCode == "000") {
          this.submitStat = 2;
          this.globalData.showAlert("Alert!", "CIBIL Score is not valid! Application submitted successfully! Lead will be removed from the device");

          this.router.navigate(['/JsfhomePage'],{skipLocationChange: true, replaceUrl: true});
        } else {
          this.globalData.showAlert("Alert!", "Something went wrong! Try after sometime!");
        }
      }).catch(cat => {
        console.log(cat);
      })
  ...
  getApplicantDocs() {
    this.sqliteProvider.getPersonalDetails(this.items[0].refId, this.items[0].id).then(data => {
      if (data.length > 0) {
        this.appLeadId = data[0].coAppGuaId;
        this.sqliteProvider.getPromoterProofDetails(this.items[0].refId, this.items[0].id).then(proof => {
          if (proof.length > 0) {
            for (let i = 0; i < proof.length; i++) {
              let docReq = {
                ...
                "DocName": "Img_" + this.appLeadId + "_" + proof[i].promoIDType + "_" + i + 1 + ".jpg"
              }
      ...
      return new Promise(async (resolve, reject) => {
        if (applicantType == 'A') {
          let model = await this.modalCtrl.create({
            ...
            componentProps: { applicant: value, cbusertype: applicantType, showBackdrop: true, enableBackdropDismiss: true, cssClass: 'modalCss' }
          })
          ...
          let model = await this.modalCtrl.create({
            component: VerifyOtpComponent,
            componentProps: { gurantor: value, cbusertype: applicantType, showBackdrop: true, enableBackdropDismiss: true, cssClass: 'modalCss' }
          })
          model.onDidDismiss().then((data) => {
            if (data) {
              resolve(data)
          ...
          let model = await this.modalCtrl.create({
            component: VerifyOtpComponent,
            componentProps: { coapp: value, cbusertype: applicantType, showBackdrop: true, enableBackdropDismiss: true, cssClass: 'modalCss' }
          })
          model.onDidDismiss().then((data) => {
            if (data) {
              resolve(data)
