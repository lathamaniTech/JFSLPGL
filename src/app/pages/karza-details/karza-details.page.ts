...
export class KarzaDetailsPage implements OnInit {
  ...
  constructor(public navCtrl: Router, public navParams: NavParams, public globFunc: GlobalService, 
    public formBuilder: FormBuilder, public master: RestService, public modalCntrl : ModalController,
    public globalData: DataPassingProviderService, public network: Network, public sqlite: SqliteService, 
    ...
    if (this.idType == "pan" || this.idType == "voterid" || this.idType == "aadhar") {
      this.panData = JSON.parse(this.navParams.get('data'));
      console.log("JSON.parse(this.navParams.get('pan'))", JSON.parse(this.navParams.get('data')));
    ...
    this.passportForm = this.formBuilder.group({
      ...
      countryCode: ['', Validators.compose([Validators.maxLength(3), Validators.pattern(/^[a-zA-Z]{3}$/), Validators.required])]
    });
  ...
  passportKarza(value) {
    ...
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      let dob = value.dob.substring(8, 10) + "/" + value.dob.substring(5, 7) + "/" + value.dob.substring(0, 4);
      let doe = value.doe.substring(8, 10) + "/" + value.doe.substring(5, 7) + "/" + value.doe.substring(0, 4);
      this.idExpiry = doe;
      let body = {
        "name": value.name,
      ...
      this.master.restApiCallAngular('PassportMRZ', body,'Y').then((result) => {
        if ((<any>result).success) {
          ...
          if (resData.status_code == 101) {
            ...
            let father = str1.substring(str1.indexOf('IND') + 3, str1.indexOf('<<'));
            let name = str1.substring(str1.indexOf('<<') + 2, str1.indexOf('<<<<<<'));
            name = name.replace('<', ' ');
            ...
            this.sqlite.InsertKarzaData(this.leadId, name, father, value.dob, idNumber, "", "", "", "", "", value.gender[0], 'passport', this.idExpiry, this.secondKyc);
          ...
          buttons: [
            ...
            },
            {
              ...
              handler: () => {
                this.globalData.globalLodingDismiss();
                this.globalData.setCustType('N');
                this.navCtrl.navigate(['/NewapplicationPage'], {queryParams :{ leadStatus: this.leadStatus, leadId: this.leadId, userType: this.globalData.getborrowerType() },skipLocationChange: true, replaceUrl: true});
              }
            }
          ]
        });
  ...
  dlKarza(value) {
    ...
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      let dob = value.dob.substring(8, 10) + "-" + value.dob.substring(5, 7) + "-" + value.dob.substring(0, 4);
      let dlExp = value.doeLic.substring(8, 10) + "/" + value.doeLic.substring(5, 7) + "/" + value.doeLic.substring(0, 4);
      this.idExpiry = dlExp;
      let body = {
        "dlNo": this.idNumber,
        "dob": dob,
      ...
      this.master.restApiCallAngular('DriverLicenseAuthentication', body, 'Y').then((result) => {
        if ((<any>result).success) {
          let resData = JSON.parse((<any>result).responseData.driverLicense);
          if (resData.statusCode == 101) {
            ...
            if (res.address.length > 0) {
              if (res.address[0].pin != "" && res.address[0].pin != undefined && res.address[0].pin != null) {
            ...
            if (res.address.length > 0) {
              ...
              address1 = permAdrs.substring(0, permAdrs.indexOf(',')).trim();
              address2 = permAdrs.substring(permAdrs.indexOf(',') + 1).trim();
              pin = pin;
  ...
  selectFirstName() {
    if ((this.dlDataForm.controls.firstname.value != "" && this.dlDataForm.controls.firstname.value != undefined && this.dlDataForm.controls.firstname.value != null) && (this.dlDataForm.controls.lastname.value != "" && this.dlDataForm.controls.lastname.value != undefined && this.dlDataForm.controls.lastname.value != null)) {
      if (this.dlDataForm.controls.firstname.value == this.dlDataForm.controls.lastname.value) {
        setTimeout(() => {
          this.globalData.showAlert("Alert!", "First and Last name should not be same!!!");
          this.dlDataForm.controls.lastname.setValue('');
        }, 400);
      } else if (this.dlDataForm.controls.firstname.value == this.dlDataForm.controls.middlename.value) {
        setTimeout(() => {
          this.globalData.showAlert("Alert!", "First and Middle name should not be same!!!");
          this.dlDataForm.controls.middlename.setValue('');
        }, 400);
      } else if (this.dlDataForm.controls.lastname.value == this.dlDataForm.controls.middlename.value) {
        setTimeout(() => {
          this.globalData.showAlert("Alert!", "Middle and Last name should not be same!!!");
          this.dlDataForm.controls.middlename.setValue('');
        }, 400);
      }
    }
  ...
  dlDataFormPopulate(value) {
    ...
    if (this.idType == 'pan') {
      ...
      this.globalData.setCustType('N')
      this.sqlite.InsertKarzaData(this.leadId, this.dlDataFetch.name, this.dlDataFetch.fathername, this.dlDataFetch.dob, this.dlDataFetch.idNumber,
        this.dlDataFetch.permAddressLine1, this.dlDataFetch.permAddressLine2, this.dlDataFetch.permState,
        this.dlDataFetch.permPincode, this.dlDataFetch.type, "", 'licence', this.idExpiry, this.secondKyc);
