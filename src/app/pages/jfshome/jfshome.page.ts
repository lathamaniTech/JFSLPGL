...
export class JfshomePage implements OnInit {
  ...
  constructor(public navCtrl: NavController, public modalCtrl: ModalController, public loadCtrl: LoadingController,
    public network: Network, public globalData: DataPassingProviderService, private globFunc: GlobalService,
    public platform: Platform, public toastCtrl: ToastController, public sqliteProvider: SqliteService, public master: RestService, public router: Router) {
  ...
  newapplicantclick() {
    if ((this.userGroupsName.includes('CRES') && this.userGroupsName.includes('CPCOPS')) || this.userGroupsName.includes('CRES')) {
      if (this.network.type == 'none' || this.network.type == "unknown") {
        this.leadStatus = "offline";
        this.userType = "A";
        this.globalData.setborrowerType(this.userType);
        this.globalData.setCustType('N');
        this.router.navigate(['/home'], { queryParams: { userType: this.userType, leadStatus: this.leadStatus },skipLocationChange: true, replaceUrl: true });
      } else {
      ...
      this.globFunc.showAlert('Alert', 'This user is not allowed to create lead!');
    }
  }
  ...
  existapplicantclick(leadStatus) {
    ...
    if ((this.userGroupsName.includes('CRES') && this.userGroupsName.includes('CPCOPS')) || this.userGroupsName.includes('CRES')) {
      ...
      this.globFunc.showAlert('Alert', 'This user is allowed to view lead!');
    }
  }
  ...
  checkLMS() {
    this.router.navigate(['/LmsPage'], { queryParams: { user: this.username, branch: this.branchCode },skipLocationChange: true, replaceUrl: true });
  }

  groupInbox() {
    this.router.navigate(['/GroupInboxPage'], { queryParams: { user: this.username, branch: this.branchCode },skipLocationChange: true, replaceUrl: true });
  }
