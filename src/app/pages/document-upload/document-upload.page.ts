...
export class DocumentUploadPage {
  ...
  constructor(public navCtrl: NavController,
    public globalData: DataPassingProviderService,public navParams: NavParams, 
    public sqliteProvider: SqliteService, public modalCtrl: ModalController,
  ...
  viewIdProof(data) {
    ...
    this.sqliteProvider.getPromoterProofDetails(this.refId, data.id).then(async documents => {
      ...
      let modal = await this.modalCtrl.create({
        ...
        componentProps: { document: this.custType == 'Promoter' ? this.appkycDocument : this.coappkycDocument, userInfo: data, custType: this.custType, submitStatus: this.submitStatus }
      });
      modal.present();
  ...
  proceedNextPage() {
    let coAppStatus = '';
    this.sqliteProvider.getPromoterProofDetails(this.refId, this.id).then(proofData => {
      if (proofData) {
        this.sqliteProvider.getPromoterProofImageDetails(this.refId, this.id).then(appProof => {
          if (proofData.length == appProof.length) {
            console.log("UserInfo", this.userInfo)
            if (this.userInfo.coAppFlag == 'Y') {
              this.sqliteProvider.getCoappDetails(this.userInfo.refId).then(coAppData => {
                if (coAppData.length > 0) {
                  for (let i = 0; i < coAppData.length; i++) {
                    this.sqliteProvider.getPromoterProofDetails(this.userInfo.refId, coAppData[i].id).then(coAppProof => {
                      this.sqliteProvider.getPromoterProofImageDetails(this.userInfo.refId, coAppData[i].id).then(coProofData => {
                  ...
                  this.globalData.proccedOk("Alert", "Proceed to Co-Applicant details").then(data => {
                    if (data == "yes") {
                      ...
                      this.globalData.setrefId(this.userInfo.refId);
                      this.globalData.setId(this.userInfo.id);
                      this.router.navigate(['/ViewDetailsPage'], {queryParams: {refvalue: JSON.stringify(this.userInfo), userVal: JSON.stringify(this.userInfo), user_Type: 'G'},skipLocationChange : true,replaceUrl:true})
                    }
                  });
                }
              })
