...
import { ActionSheetController, AlertController, LoadingController, ModalController, NavParams, Platform } from '@ionic/angular';
...
export class FingerprintPage implements OnInit {
  ...
  constructor(
    ...
    public router: Router, public network: Network,
    public route: ActivatedRoute, public platform: Platform, public modalCtrl: ModalController,
    ...
    if (this.navParams.get('second') == true) {
      ...
      this.existAather = JSON.parse(this.route.snapshot.queryParamMap.get('existAather'));
  ...
  ionViewWillLeave() {
    this.platform.backButton.subscribe(() => {
      this.router.navigate(['/ExistingPage', { _leadStatus: status, replaceUrl: true, skipLocationChange: true }]);
    })
  }
  ...
  fingerLL() {
    this.fingerbutton1 = true; if (this.fingerbutton1 === true) {
  ...
  fingerLR() {
    this.fingerbutton2 = true; if (this.fingerbutton2 === true) {
  ...
  fingerLM() {
    this.fingerbutton3 = true; if (this.fingerbutton3 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
  ...
  fingerLI() {
    this.fingerbutton4 = true; if (this.fingerbutton4 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
  ...
  fingerLT() {
    this.fingerbutton5 = true; if (this.fingerbutton5 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  fingerRT() {
    this.fingerbutton6 = true; if (this.fingerbutton6 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  fingerRI() {
    this.fingerbutton7 = true; if (this.fingerbutton7 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  fingerRM() {
    this.fingerbutton8 = true; if (this.fingerbutton8 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  fingerRR() {
    this.fingerbutton9 = true; if (this.fingerbutton9 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  fingerRL() {
    this.fingerbutton10 = true; if (this.fingerbutton10 === true) {
      this.divColor1 = "#da107e";
      this.divColor2 = "#da107e";
      this.divColor3 = "#da107e";
      this.divColor4 = "#da107e";
  ...
  async LoadingCustom() {
    ...
    await loading.onDidDismiss().then(async () => {
      ...
      await alert.onDidDismiss().then(() => {
        this.router.navigate(['/aadhaarpreview'], { replaceUrl: true, skipLocationChange: true });
      });
  ...
  fingerPrintScan() {
    ...
    this.platform.ready().then(() => {
      ...
      if (this.selectedDeviceName !== '') {
        ...
        this.bioMetricService.selectedDevice(this.selectedDeviceName).then((data: any) => {
          self.globalData.globalLodingPresent("Getting data!!!");
          ...
          if ((<any>jsonData).PidData.Resp._errCode == "0") {
            ...
            let fpDeviceCode = (<any>jsonData).PidData.DeviceInfo.additional_info.Param[0]._value;
            ...
            self.masterProvider.restApiCallAngular('EkyBiometric', reqData).then(async data => {
              console.log((<any>data));
              if ((<any>data).errorCode == "000") {
                let res = JSON.parse((<any>data).EkycResponse);
                if (res.NpciResponse.statusInfo.respCode == "00") {
                  var decodedKyc = window.atob(res.NpciResponse.eKycResp.kycRes);
                  // console.log("decodedKyc", decodedKyc);
                  var x2js = new X2JS();
                  var ekycData = x2js.xml2js(decodedKyc);
                  console.log("jsonData", ekycData);
                  ekycData['KycRes'].UidData.Poi._name = ekycData['KycRes'].UidData.Poi._name.replace(/[~!#$%^&*=|?+@.]/g, " ");
                  ...
                  if (self.existAather) {
                    self.sqlSupport.insertEKYCDetails(self.leadId, self.janaid, "", "", this.aepsStatus);
                    self.globalData.setCustType('E');
                    self.globalData.globalLodingDismiss();
                    self.router.navigate(['/NewapplicationPage'], { queryParams: { leadStatus: self.leadStatus, leadId: self.leadId, userType: self.globalData.getborrowerType(), janaid: self.janaid, existAather: self.existAather, getExistAather: ekycData, ekyc: 'bio' }, skipLocationChange: true, replaceUrl: true });
                  } else {
                    self.sqlSupport.insertEKYCDetails(self.leadId, janaid, EkycPersonal, Ekycaddress, this.aepsStatus);
                    self.globalData.showAlert("Alert!!!", res.NpciResponse.statusInfo.respMsg);
                    self.globalData.globalLodingDismiss();
                ...
                self.globalData.globalLodingDismiss();
                self.globalData.showAlert("Alert!!!", (<any>data).errorDesc);
              }
            }, err => {
              if (err.name == "TimeoutError") {
                self.globalData.globalLodingDismiss();
                self.globalData.showAlert("Alert!", err.message);
              } else {
                self.globalData.globalLodingDismiss();
                self.globalData.showAlert("Alert!", "No Response from Server!");
              }
            }).catch(err => {
              self.globalData.globalLodingDismiss();
              self.globalData.showAlert("Alert!!!", JSON.stringify(err));
            });
          } else {
            self.globalData.globalLodingDismiss();
            self.globalData.showAlert("Alert!!!", (<any>jsonData).PidData.Resp._errInfo);
          }
        ...
        this.globFunc.showAlert('Alert', 'Please Select Device Type!')
      }
    });
  }
  ...
  private _getGetPidOptionsXML() {
    ...
    return new Promise((resolve, reject) => {
      ...
      EsysRDService.vGetPidOptionXML(function (pid) {
        ...
        console.log("Error");
        // console.log(JSON.stringify(err));
        _self.globalData.showAlert("Alert!!!", JSON.stringify(err));
        reject(null);
  ...
  sendOtp() {
    if (this.network.type == 'none') {
      ...
      //16digit
      let sixteenDigit = Math.floor(Math.random() * 9000000000000000) + 1000000000000000;
      // console.log(sixteenDigit);

      //datetime
      let newDate = new Date();
      let timeformat = newDate.getFullYear() + "-" + newDate.getMonth() + "-" + newDate.getDate() + "T" + newDate.toTimeString().split(" ")[0];
      ...
      this.masterProvider.restApiCallAngular('otpRequestApi', otpRequestEKYC).then(data => {
        console.log(data, "otp 1st service response");
        if ((<any>data).errorCode === '000') {
          this.globalData.globalLodingDismiss();
          this.randomDataRefNumber = otpRequestEKYC.NpciRequest['headerInfo']["dataRef"];
          this.otpCheckEnable = true;
          ...
          //   this.globalData.showAlert("Alert", "OTP sent Successfully!.");
          //   this.otpCheckEnable = true;OTP
          // } else {
          //   this.globalData.showAlert("Alert", JSON.parse((<any>data).EkycResponse).NpciResponse.statusInfo.respMsg);
        ...
        // } else {
        //   this.globalData.showAlert("Alert", "Request Error. Try Again");
        // }
  ...
  checkOTP(val) {
    ...
    let random16digit = Math.floor(Math.random() * 9000000000000000) + 1000000000000000;
    ...
    this.masterProvider.restApiCallAngular('aadharOtpVerification', otpCheckEKYC).then(async data => {
      console.log(data, 'otp 2nd service response');
      let aadharRes = (<any>data);
      if (aadharRes.ErrorCode === '000') {
        // await this.functionalityRest();
        if (this.existAather) {
          this.sqlSupport.insertEKYCDetails(this.leadId, this.janaid, "", "", this.aepsStatus);
          this.globalData.setCustType('E');
          this.globalData.globalLodingDismiss();
          aadharRes.name = aadharRes.name.replace(/[~!#$%^&*=|?+@.]/g, " ")
          this.router.navigate(['/NewapplicationPage'], { queryParams: { leadStatus: this.leadStatus, leadId: this.leadId, userType: this.globalData.getborrowerType(), janaid: this.janaid, aadharName: aadharRes, existAather: this.existAather, ekyc: 'OTP' }, replaceUrl: true, skipLocationChange: true });
        } else {
        ...
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert("Alert", aadharRes.ErrorDesc);
      }
      // if ((<any>data).EkycResponse.NpciResponse.statusInfo.respCode == "00") {
      //   this.globalData.showAlert("Alert", "OTP Verified Successfully!.");
      //   this.viewCtrl.dismiss('success');
      // } else if (this.OTPNUM != receivedOTP) {
      //   this.OTP = { first: '', second: '', third: '', forth: '', fifth: '', sixth: '' };
      //   this.globalData.showAlert("Alert", "Please enter a valid OTP");
  ...
  async initKarzaAPi(idType, idNumber, leadId, body?, ekycData?) {
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: {
        ...
        idType: idType, idNumber: idNumber, leadId: leadId,
        leadStatus: this.leadStatus, userType: this.globalData.getborrowerType()
      }
    });
        ...
        if (idType == 'aadhar') {
          ...
          this.router.navigate(['/NewapplicationPage'], {
            queryParams: {
              ...
              leadId: this.leadId, userType: this.globalData.getborrowerType(), ekycData: ekycData,
              janaid: idNumber, aadharName: data, ekyc: 'OTP'
  ...
  async normalWorkflow(aadharRes) {
    ...
    // let janaid = "";
    let ekycData = { ...EkycPersonal, ...Ekycaddress };
    this.sqlSupport.insertEKYCDetails(this.leadId, this.janaid, EkycPersonal, Ekycaddress, this.aepsStatus);
    // if (JSON.parse((<any>data).EkycResponse).NpciResponse.statusInfo.respCode == "00") {
    this.globalData.globalLodingDismiss();
    ...
    if (this.secKyc) {
      ...
      this.router.navigate(['/secondKycPage'], { queryParams: { data: JSON.stringify(body), idType: "aadhar", idNumber: this.janaid, leadId: this.leadId, ekycData: JSON.stringify(ekycData), leadStatus: this.leadStatus, userType: this.globalData.getborrowerType() }, replaceUrl: true, skipLocationChange: true })
  ...
  fingerPrintScanMantra() {
    return new Promise(async (resolve, reject) => {
      MantraRDService.finger_deviceInfo("finger_deviceInfo", (async (success) => {
        var x2js = new X2JS();
        ...
        if ((<any>jsonData).RDService._status == "READY") {
          ...
          this.globFunc.showAlert('Alert', `Please Check the Device is ${(<any>jsonData).RDService._status}`);
          await this.captureFingurePrint();
        }
      }), ((failed) => {
        this.globFunc.showAlert('Failed', JSON.stringify(failed));
        reject('')
      }));
    })
  }
      ...
      MantraRDService.finger_capture("finger_capture", ((sucess) => {
        if ((<any>sucess).pid_data) {
          ...
          if ((<any>jsonData).PidData.Resp._errCode) {
            this.globalData.confirmationAlert('Alert', JSON.stringify((<any>jsonData).PidData.Resp)).then(data => {
              let dpId = (<any>jsonData).PidData.DeviceInfo._dpId;
              let rdsId = (<any>jsonData).PidData.DeviceInfo._rdsId;
              let rdsVer = (<any>jsonData).PidData.DeviceInfo._rdsVer;
              let dc = (<any>jsonData).PidData.DeviceInfo._dc;
            ...
            this.globalData.showAlert('Alert', JSON.stringify((<any>jsonData).PidData.Resp));
          }
        } else {
          this.globFunc.showAlert('Failed', JSON.stringify(sucess))
        }
      }), ((failed) => {
        this.globFunc.showAlert('Failed', JSON.stringify(failed));
      }));
