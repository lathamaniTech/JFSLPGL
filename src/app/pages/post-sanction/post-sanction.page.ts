...
export class PostSanctionPage implements OnInit {
  ...
  private childConstructor(value) {
    ...
    this.accountCreation = this.formBuilder.group({
      customerId: [""],
      instaKitNumber: ["", Validators.compose([Validators.maxLength(12), Validators.minLength(12)])],
      accountNo: [""]
    });
    this.getVlTenure();
    this.getAgriProof();
  ...
  onClick() {
    ...
    if (this.purpose == '3' || this.purpose == '4') {
      this.sqliteProvider.getPSLDetails(this.refId, this.id).then(val => {
        console.log(val);
        if (val.length < 1) {
          this.custType = 'psl';
          this.globFunc.showAlert("Alert!", `Please complete PSL Details`)
        }
      }).catch(err => err)
    }
  }
  ...
  calcendDate() {
    this.maxdate = this.maxdate = moment(this.mindate).add(30, 'days').format("YYYY-MM-DD");
  }
  ...
  async getVariantbasedOnModel(value?) {
    ...
    if (model) {
      if (value) {
        ...
        this.variantMaster = await this.sqliteProvider.getVariantBasedonModel(model);
      } else {
        this.variantMaster = await this.sqliteProvider.getVariantBasedonModel(model);
      }
    }
  ...
  downPaymentCalc() {
    ...
    this.sqliteProvider.getApplicantDataAfterSubmit(this.refId).then(data => {
      if (data.length > 0) {
        ...
        this.dbAmt = dbAmt
        if (marginCost) {
          let downPayment = marginCost + processingFee + gstonPf + charges + advanceEmi;
    ...
    if (flage == 'Y' && (onRoadprice || assetAmt)) {
      if (this.vehicleType == "N") {
        if (+onRoadprice <= loanAmt) {
          this.globalData.showAlert("Alert!", `Loan Amount should be lesser than On Road Price Amount(Rs. ${onRoadprice})`);
          ...
          this.sqlSupport.updateORPinPostSanction(onRoadprice, this.refId, this.id);
          this.downPaymentCalc();
        }
      } else {
        if (+assetAmt <= loanAmt) {
          this.globalData.showAlert("Alert!", `Loan Amount should be lesser than Final Asset Price(Rs. ${assetAmt})`);
          this.postsanction.controls.loanAmount.setValue('');
          this.postsanction.controls.loanAmount.updateValueAndValidity();
        } else {
          ...
          this.postsanction.controls.marginCost.setValue(marginCost);
          this.postsanction.controls.marginCost.updateValueAndValidity();
          this.sqlSupport.updateAssetPricePostSanction(assetAmt, this.refId, this.id);
          this.downPaymentCalc();
        }
      }
    } else {
      if (this.vehicleType == "N") {
        ...
        if (assetAmt) {
          if (assetAmt <= loanAmt) {
            this.globalData.showAlert("Alert!", `Final Asset Price should be lesser than On Road Price Amount(Rs. ${onRoadprice})`);
            this.postsanction.controls.loanAmount.setValue('');
            this.postsanction.controls.loanAmount.updateValueAndValidity();
          } else {
            let marginCost = +assetAmt - loanAmt;
  ...
  getAgriformertype() {
    this.sqliteProvider.getMasterDataUsingType('Agriformertype').then(data => {
      this.masterAgriformertype = data;
    })
  }

  getAgriactivitytype() {
    this.sqliteProvider.getMasterDataUsingType('Agriactivitytype').then(data => {
  ...
  getPostSanctionModifications(isFromScorecardRerun?: boolean) {
    this.sqlSupport.getPostSanctionDetails(this.refId, this.id).then(data => {
      if (data.length > 0) {
        ...
        if (!(this.sanctionModify && this.postSancModified != 'Y' && !this.psmSubmitted)) {
          if (+this.applicantDetails[0].eligibleAmt >= +data[0].loanAmount) {
            this.lessThaneligible = true;
          }
        }
        this.autoApprovalFlag = (this.lessThaneligible == true) ? 'Y' : data[0].autoApprovalFlag;
        ...
        this.disableAutoApprovalBtn = (data[0].autoApprovalStatus == 'Y') ? true : false;
        this.disableFISubmitBtn = (data[0].FieldInvStatus == 'Y') ? true : false;
        this.disableManualApprvalBtn = (data[0].ManualApprovalStatus == 'Y') ? true : false;
  ...
  editPostSanction() {
    if (this.autoApprovalFlag == 'Y' && this.disableAutoApprovalBtn) {
      this.globalData.confirmationAlert('Alert', `On editing,should re run the scorecard!`).then(data => {
        if (data) {
          this.submitDisable = false;
          this.disableEditBtn = true;
          this.showAutoApproval = false;
          this.sqlSupport.updataStatus('scoreCardRerun', this.refId, this.id).then(data => {
            this.sqlSupport.updataStatus('autoApprovalFlag', this.refId, this.id).then(data => {
              this.sqlSupport.updataStatus('autoApprovalStatus', this.refId, this.id).then(data => {
                this.sqlSupport.updateSanctionModifiedState(this.applicantDetails[0].applicationNumber).then(data => {
                  this.getPostSanctionModifications();
                  this.loadAllApplicantDetails();
                  this.getApplicantDataforPostSanction();
                });
  ...
  getEditedCasaInPS() {
    this.sqlSupport.getCASADetails(this.refId, this.id).then(data => {
      ...
      if (this.casaValue != '') {
        if ((this.casaValue.editedInPS == 'Y' && this.casaValue.janaAcc == 'Y') || (this.casaValue.janaAcc == 'Y' && this.casaValue.editedInPS == 'N')) {
  ...
  getVehicleWorkflow() {
    this.sqliteProvider.getVehicleApproval('Recommendation & Approval').then(manualUsers => {
      let newManualUserList = manualUsers;
      this.manualUserList = newManualUserList.filter(val => val.UserGroupName !== 'BH');
    })
  }

  getfieldInspecFlowGroup() {
    this.sqliteProvider.getVehicleWorkflowList('Field Investigation').then(flowDesc => {
      this.fieldInspecFlowGroup = flowDesc[0];
    }).then(data => {
      this.sqliteProvider.getVehicleWorkflowList('Post sanction Activities').then(postDesc => {
  ...
  loadAllApplicantDetails() {
    this.sqliteProvider.getApplicantDataAfterSubmit(this.refId).then(data => {
      if (data.length > 0) {
        ...
        //getPSL
        this.sqliteProvider.getPSLDetails(this.refId, this.id).then(pslData => {
          console.log(pslData);
          ...
          if (pslData.length > 0) {
            ...
            if (data[0].purpose == '3') {
              ...
              this.pslForm.controls.agriProofType.setValue(pslData[0].agriProofType);
              this.pslForm.controls.landHolding.setValue(pslData[0].landHolding);
              this.pslForm.controls.farmerType.setValue(pslData[0].farmerType);
              this.pslForm.controls.actType.setValue(pslData[0].actType);
              this.pslForm.controls.agriPurpose.setValue(pslData[0].agriPurpose);
            } else if (data[0].purpose == '4') {
              this.pslBusForm.controls.agriProofType.setValue(pslData[0].agriProofType);
              this.pslBusForm.controls.landHolding.setValue(pslData[0].landHolding);
              this.pslBusForm.controls.farmerType.setValue(pslData[0].farmerType);
              this.pslBusForm.controls.actType.setValue(pslData[0].actType);
              this.pslBusForm.controls.servUnit.setValue(pslData[0].agriPurpose);
              this.pslBusForm.controls.udyamCNo.setValue(pslData[0].udyamCNo);
              this.pslBusForm.controls.udyamRegNo.setValue(pslData[0].udyamRegNo);
              this.pslBusForm.controls.udyamClass.setValue(pslData[0].udyamClass);
              this.pslBusForm.controls.majorAct.setValue(pslData[0].majorAct);
              this.pslBusForm.controls.udyamInvest.setValue(pslData[0].udyamInvest);
              this.pslBusForm.controls.udyamTurnOver.setValue(pslData[0].udyamTurnOver);
            }
          }
        }).catch(err => err);
    ...
    this.sqlSupport.getCBSDetails(this.refId, this.id).then(data => {
      if (data.length > 0) {
        ...
        this.accountCreation.controls.instaKitNumber.setValue(data[0].instaKitNumber);
  ...
  usedFieldValidation(vehicleType) {
    if (vehicleType == "N") {
      this.postsanction.get("onroadPrice").setValidators(Validators.compose([Validators.pattern('[0-9]*'), Validators.required]));
      ...
      this.postsanction.get("cc").setValidators([Validators.compose([Validators.pattern('[0-9]*'), Validators.required])]);
      this.postsanction.get("rcNo").setValidators(Validators.compose([Validators.minLength(7), Validators.maxLength(11), Validators.pattern('[a-zA-z0-9]*'), Validators.required]));
      this.postsanction.get("engineNo").setValidators(Validators.compose([Validators.minLength(10), Validators.maxLength(14), Validators.pattern('[a-zA-z0-9]*'), Validators.required]));
      this.postsanction.get("chassisNo").setValidators(Validators.compose([Validators.minLength(10), Validators.maxLength(25), Validators.pattern('[a-zA-z0-9]*'), Validators.required]));
      this.postsanction.get("yearOfMan").setValidators(Validators.compose([Validators.minLength(4), Validators.maxLength(4), Validators.pattern('[0-9]*'), Validators.required]));
      this.postsanction.get("registrationDate").setValidators(Validators.required);
      ...
      this.postsanction.get("kmDriven").setValidators(Validators.compose([Validators.pattern('[0-9]*'), Validators.required]));
      this.postsanction.get("dealerQuotation").setValidators(Validators.compose([Validators.pattern('[0-9]*'), Validators.required]));
      this.postsanction.get("obv").setValidators(Validators.compose([Validators.pattern('[0-9]*'), Validators.required]));
      this.postsanction.get("assetPrice").setValidators(Validators.compose([Validators.pattern('[0-9]*'), Validators.required]));
  ...
  getCreditFlowGroup() {
    this.sqliteProvider.getVehicleWorkflowList('Disbursement Activities').then(creditDesc => {
      this.creditFlowGroup = creditDesc[0];
    })
  }

  getApplicantDataforPostSanction() {
    this.sqlSupport.getPostSanctionDetails(this.refId, this.id).then(postSanction => {
      if (postSanction.length > 0) {
        this.globalData.globalLodingDismiss();
        this.sqliteProvider.getApplicantDataforPostSanction(this.userInfo.refId).then(data => {
          if (data.length > 0) {
            this.globalData.globalLodingDismiss();
            ...
            if (postSanction[0].groupInboxFlag == 'Y' || postSanction[0].modified == '1') {
              ...
              this.savedAdvanceEmi = postSanction[0].advanceEmi ? postSanction[0].advanceEmi : data[0].advanceEmi;
              //used
              ...
              if (postSanction[0].groupInboxFlag == 'Y') {
                if (this.globalData.checkIsJson(postSanction[0].dealerName)) {
                  if (typeof JSON.parse(postSanction[0].dealerName) === 'object') {
                    this.dealerNameTemp = this.dummy_masterDealer.find(val => val.dealerCode == postSanction[0].dealerName);
                  } else {
                    this.dealerNameTemp = this.dummy_masterDealer.find(val => val.dealerCode == postSanction[0].dealerName);
                  }
                } else {
              ...
              if (!this.dealerNameTemp.dealerName.toLowerCase().includes('beepKart pvt ltd'.toLowerCase()) ||
                !this.dealerNameTemp.dealerName.toLowerCase().includes('drivex mobility private limited'.toLowerCase())) {
                this.obvEditable = true;
              }
              this.savedBrandName = postSanction[0].brandName;
              this.postsanction.get('dealerName').setValue(this.dealerNameTemp.dealerCode);
              this.postsanction.get('vehicleCatogery').setValue(postSanction[0].vehicleCatogery);
              this.postsanction.get('brandName').setValue(postSanction[0].brandName);
              // this.getModelbasedOnBrand();
              this.postsanction.get('loanAmount').setValue(postSanction[0].loanAmount);
              this.postsanction.get('tenure').setValue(postSanction[0].tenure);
              this.postsanction.get('model').setValue(postSanction[0].model);
              // this.getVariantbasedOnModel();
              this.postsanction.get('variant').setValue(postSanction[0].variant);
              this.getPriceBasedOnVariant('Y', this.onroadPrices);
              this.postsanction.get('segment').setValue(postSanction[0].segment);
              this.postsanction.get('totalloanAmount').setValue(postSanction[0].totalloanAmount);
              this.postsanction.get('advanceEmi').setValue(this.savedAdvanceEmi);
              this.postsanction.get('dbDate').setValue(postSanction[0].dbDate);
              // this.postsanction.get('onroadPrice').setValue(postSanction[0].onroadPrice);
              this.postsanction.get('onroadPrice').setValue(this.onroadPrices);
              ...
              this.postsanction.get('engineNo').setValue(postSanction[0].engineNo);
              this.postsanction.get('chassisNo').setValue(postSanction[0].chassisNo);
              this.postsanction.get("nameAsPerRC").setValue(postSanction[0].nameAsPerRC);
              this.postsanction.get('yearOfMan').setValue(postSanction[0].yearOfMan);
              this.orpAuthFetch('variant', postSanction[0].brandName)
              this.postsanction.get("variant").setValue(postSanction[0].variant);
              this.postsanction.get('registrationDate').setValue(postSanction[0].registrationDate);
              this.postsanction.get('vehicleAge').setValue(postSanction[0].vehicleAge);
              this.postsanction.get('hypothecation').setValue(postSanction[0].hypothecation);
              this.postsanction.get('noofOwner').setValue(postSanction[0].noofOwner);
              this.postsanction.get('kmDriven').setValue(postSanction[0].kmDriven);
              this.postsanction.get('dealerQuotation').setValue(postSanction[0].dealerQuotation);
              this.postsanction.get('obv').setValue(postSanction[0].obv);
              this.postsanction.get('assetPrice').setValue(postSanction[0].assetPrice);
              this.postsanction.get('assetAge').setValue(postSanction[0].assetAge);
  ...
  getSanctionandEligibleAmount() {
    this.sqliteProvider.getApplicantDataforPostSanction(this.userInfo.refId).then(submitData => {
  ...
  getApplicantDataforPost() {
    this.sqliteProvider.getApplicantDataforPostSanction(this.userInfo.refId).then(data => {
      if (data.length > 0) {
        ...
        this.postsanction.get('dealerName').setValue(this.dealerNameTemp.dealerCode);
        this.postsanction.get('vehicleCatogery').setValue(data[0].vehicleCatogery);
        ...
        this.postsanction.get("variant").setValue(data[0].variant);
        this.postsanction.get('registrationDate').setValue(data[0].registrationDate);
        this.postsanction.get('vehicleAge').setValue(data[0].vehicleAge);
        this.postsanction.get('hypothecation').setValue(data[0].hypothecation);
        this.postsanction.get('noofOwner').setValue(data[0].noofOwner);
        this.postsanction.get('kmDriven').setValue(data[0].kmDriven);
        this.postsanction.get('dealerQuotation').setValue(data[0].dealerQuotation);
  ...
  postsanctionSave(value) {
    ...
    if (this.sanctionModify == 2 || this.sanctionModify == 1 && +this.postsanction.get('loanAmount').value <= this.savedSanctionLoanAmount || this.postsanction.controls.dbDate.value >= this.mindate) {
      if (!this.ModelModified && !this.DealerNameModified && !this.BrandNameModified && !this.VariantModified && !this.segmentModified &&
        !this.ccModified && !this.rcNoModified && !this.engineNoModified && !this.chassisNoModified && !this.yearOfManModified && !this.registrationDateModified &&
        !this.vehicleAgeModified && !this.hypothecationModified && !this.noofOwnerModified && !this.kmDrivenModified && !this.dealerQuotationModified &&
        !this.obvModified && !this.assetPriceModified && !this.assetAgeModified || this.postsanction.controls.dbDate.value >= this.mindate) {
        if (+this.postsanction.controls.loanAmount.value < this.loanAmountFrom) {
          this.globalData.showAlert("Alert!", "You must enter the minimum loan amount of " + this.loanAmountFrom);
        }
        else {
          let segmentFlag = "N";
          if (this.sanctionModify == 1) {
            this.postsanction.controls.onroadPrice.setValue(value.onroadPrice ? value.onroadPrice : this.newOnRoadPrice);
            this.postsanction.controls.onroadPrice.updateValueAndValidity();
            if (this.postsanction.valid) {
              if (+this.postsanction.get('loanAmount').value > this.savedEligibleLoanAmount || +this.postsanction.get('loanAmount').value != this.savedSanctionLoanAmount && +this.postsanction.get('loanAmount').value <= this.savedEligibleLoanAmount) {
                if (+this.postsanction.get('loanAmount').value <= this.savedEligibleLoanAmount) {
                  let loanAmount = +this.postsanction.get('loanAmount').value;
                  this.sqliteProvider.updateBasicDetailsinPostSanction(loanAmount, this.refId, this.id);
                  this.lessThaneligible = true;
                } else {
                  let loanAmount = +this.postsanction.get('loanAmount').value;
                  this.sqliteProvider.updateBasicDetailsinPostSanction(loanAmount, this.refId, this.id);
              ...
              if (this.globalData.convertToString(this.postsanction.get('brandName').value) != this.savedBrandName) {
              ...
              if (this.globalData.convertToString(this.postsanction.get('model').value) != this.savedModel) {
              ...
              if (this.globalData.convertToString(this.postsanction.get('variant').value) != this.savedVariant) {
              ...
              if (+this.postsanction.get('downpayment').value != this.savedDownPayment) {
              ...
              if (this.postsanction.get('registrationDate').value != this.savedRegistrationDate) {
              ...
              if (this.postsanction.get('hypothecation').value != this.savedHypothecation) {
              ...
              if (this.postsanction.get('dealerQuotation').value != this.savedDealerQuotation) {
              ...
              if (this.LoanAmountModified || this.ModelModified || this.BrandNameModified || this.VariantModified || this.ccModified || this.rcNoModified
                || this.engineNoModified || this.chassisNoModified || this.yearOfManModified || this.registrationDateModified || this.vehicleAgeModified
                || this.hypothecationModified || this.noofOwnerModified || this.kmDrivenModified || this.dealerQuotationModified || this.obvModified ||
                this.assetPriceModified || this.assetAgeModified) {
                let messages = this.savedLoanAmountDesc + '' + this.savedBrandNameDesc + '' + this.savedModelDesc + '' + this.savedVariantDesc
                  + '' + this.savedCCDesc + '' + this.savedRcNoDesc + '' + this.savedEngineNoDesc + '' + this.savedChasisNoDesc + '' + this.savedYearOfManDesc
                  + '' + this.savedRegistrationDesc + '' + this.savedVehicleAgeDesc + '' + this.savedHypothecationDesc + '' + this.savedNoOfOwnerDesc + '' + this.savedkmDrivenDesc
                  + '' + this.savedDealerQuotationDesc + '' + this.savedObvDesc + '' + this.savedAssetPriceDesc + '' + this.savedAssetAgeDesc;
                if (this.lessThaneligible == true) {
                  this.valuesModified = 'Y';
                  this.loanFacilitiesServiceCall(value);
                } else {
                  this.globalData.confirmationAlert('Alert', `${messages} value(s) modified,Would you like to proceed?. If Yes, should re run the score card?`).then(data => {
                    if (data) {
                      this.valuesModified = 'Y';
                      this.loanFacilitiesServiceCall(value);
                    } else {
              ...
              this.globalData.showAlert("Alert!", "Please fill all mandatory fields!");
            }
          } else {
            this.postSanctionCheck = true;
            this.globalData.showAlert("Alert!", "Post sanction modification successful!");
            this.sqlSupport.insertPostSanctionDetails(this.refId, this.id, value, this.PropNo, this.sanctionModify, segmentFlag, this.valuesModified, (this.valuesModified == 'Y') ? 'Y' : 'N', this.psmSubmitted ? 'Y' : 'N', this.postSanId).then(data => {
              this.getApplicantDataforPostSanction();
            });
          }
        }
      } else {
        this.globalData.showAlert("Alert!", "The DB Date should be greater than the system date so kindly change the DB Date")
      }
    } else {
      this.globalData.showAlert("Alert!", "The DB Date should be greater than the system date so kindly change the DB Date")
    }
  ...
  postSanctionServiceCall(value) {
    let segmentFlag = "N";
    if (this.valuesModified == 'Y' || this.segmentModified || this.TenureModified || this.dbDateModified || this.DealerNameModified) {
      this.globalData.globalLodingPresent('Please wait...');
      ...
      let body = {
        "PropNo": this.PropNo,
        ...(this.LoanAmountModified || this.LoanAmtHigherThanSancAmt) && { "lpsdSaleRecAmtNewVal": value.loanAmount },
        ...(this.TenureModified) && { "lpsdTenorNewVal": value.tenure },
        ...(this.BrandNameModified) && { "lpsdCategory": value.vehicleCatogery.toString() },
        ...(this.BrandNameModified) && { "lpsdVehicleModelNewVal": value.brandName.toString() },
        ...(this.ModelModified) && { "lpsdModelDetail": value.model.toString() },
        ...(this.VariantModified) && { "lpsdModelVariant": value.variant.toString() },
        ...(this.DownPaymentModified) && { "lpsdDownPmtNewVal": value.downpayment },
        ...(this.segmentModified) && { "lpsdSegmentTypeNewVal": value.segment },
        ...(this.DealerNameModified) && { "lpsdDealerNewVal": value.dealerName },
        "lpsdUpmodifyCheck": segmentFlag, "lpsdApiFlag": value.apiFlag
      }
      console.log(body, "post sanction");
      this.master.restApiCallAngular('postsancmodification', body).then(res => {
        ...
        if (sanctionRes.ErrorCode == '000') {
          ...
          value.totalloanAmount = this.postsanction.get('totalloanAmount').value;

          this.sqlSupport.insertPostSanctionDetails(this.refId, this.id, value, this.PropNo, this.sanctionModify, segmentFlag, this.valuesModified, (this.valuesModified == 'Y') ? 'Y' : 'N', this.psmSubmitted ? 'Y' : 'N', this.postSanId);
          this.sqlSupport.updatePostSanctionStatus(this.PropNo);
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert!", "Post sanction modification successful!.Proceed further.");
          // this.submitDisable = true;
          this.getPostSanctionModifications();
          this.loadAllApplicantDetails();
          this.getApplicantDataforPostSanction();
          ...
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert('Alert', sanctionRes.ErrorDesc);
        }
      }, err => {
        this.globalData.globalLodingDismiss();
      ...
      this.postSanctionCheck = true;
      this.globalData.showAlert("Alert!", "No values modified.");
    }
  }

  async undoProposalServiceCall(flag) { // manual
    if (this.undoPropsal == 'Y') {
      ...
      let popover = await this.modalCtrl.create({
        ...
        componentProps: { data: this.PropNo, showBackdrop: true, enableBackdropDismiss: true, cssClass: 'modalCss' }
      });
      popover.onDidDismiss().then((remarks) => {
        if (remarks) {
          ...
          this.master.restApiCallAngular('UndoProposal', body).then(res => {
            ...
            if (undoPropRes.ErrorCode == '000') {
              ...
              // this.globalData.showAlert("Alert!", undoPropRes.ErrorDesc);
            } else {
              this.globalData.globalLodingDismiss();
              this.globalData.showAlert('Alert', undoPropRes.ErrorDesc);
            }
  ...
  scoreCardReRun() {
    ...
    this.master.restApiCallAngular('scorecard', body).then(res => {
      ...
      if (scoreResponse.errorCode == '000') {
        if (this.vehicleType !== 'N') {
          if (scoreResponse.SRFLAG == 'N') {
            ...
            this.sqlSupport.updateEligibleAmout('', this.applicantDetails[0].applicationNumber);
            this.sqlSupport.updatePostSanctionRerunFlags(scoreResponse.STPFLAG, scoreResponse.FIFLAG, 'scoreCard', this.refId, this.id, scoreResponse.STPFLAG, scoreResponse.SRFLAG, scoreResponse.NTCFLAG, scoreResponse.FIENTRY);
            this.globalData.globalLodingDismiss();
            this.getPostSanctionModifications(true);
            this.getSanctionandEligibleAmount();
            scoreResponse.NTCFLAG == 'N' ? this.postSanctionWFBLForETC(scoreResponse, 3)
            ...
            this.globalData.globalLodingDismiss();
            this.globFunc.showAlert('Alert', scoreResponse.errorDesc);
          }
        } else {
          if (scoreResponse.SRFLAG == 'N') {
            PostSanctionPage.prototype.scoreCardReRun
            ...
            if (+this.postsanction.controls.loanAmount.value < 100000) {
              scoreResponse.NTCFLAG == 'N' ? this.postSanctionWFBLForETC(scoreResponse, 1)
                : this.postSanctionWFBLForNTC(scoreResponse, 1)
            } else {
              if (+this.postsanction.controls.loanAmount.value >= 150000) {
                scoreResponse.NTCFLAG == 'N' ? this.postSanctionWFBLForETC(scoreResponse, 3)
                  : this.postSanctionWFBLForNTC(scoreResponse, 3)
              } else {//100000 to 149000
                scoreResponse.NTCFLAG == 'N' ? this.postSanctionWFBLForETC(scoreResponse, 2)
        ...
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert('Alert', scoreResponse.errorDesc);
      }
    }, err => {
      this.globalData.globalLodingDismiss();
      if (err.name == "TimeoutError") {
  ...
  srFlageEnable() {
    ...
    this.master.restApiCallAngular('proposalRejection', data).then(res => {
      let rejectRes = <any>res;
      if (rejectRes.ErrorCode === '000') {
        this.sqlSupport.updateApplicationStatus('SR', this.refId).then(data => {
          this.globFunc.globalLodingDismiss();
          this.globFunc.showAlert('Alert', 'This Proposal has been Rejected');
      ...
      this.globFunc.globalLodingDismiss();
      if (err.name == "TimeoutError") {
        this.globFunc.showAlert("Alert!", err.message);
      } else {
  ...
  onUserChange(event) {
    ...
    this.selectedUserForManual = this.manualUserList.find(data => data.UserGroup == value);
  }

  ManualApprovalCall() {
    this.globFunc.globalLodingPresent('Submitting for manual approval Please wait...');
    let body = {
      "userId": this.applicantDetails[0].createdUser,
      "PropNo": this.applicantDetails[0].applicationNumber,
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (fieldInvesRes.ErrorCode === '000') {
        ...
        this.sqlSupport.updateScorecardRerunStatus('ManualApprovalStatus', this.refId, this.id);
        this.sqlSupport.updateManualApproval(this.applicantDetails[0].applicationNumber, 'Y').then(data => {
          this.router.navigate(['/ExistApplicationsPage'], { skipLocationChange: true, replaceUrl: true })
        })

      } else {
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert('Alert', fieldInvesRes.ErrorDesc);
      }
  ...
  fieldInvestigationCall() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (fieldInsRes.ErrorCode === '000') {
        ...
        this.sqlSupport.updateScorecardRerunStatus('FieldInvStatus', this.refId, this.id);
        this.sqliteProvider.updateFIstatus(this.applicantDetails[0].applicationNumber, 'Y').then(data => {
          this.globFunc.globalLodingDismiss();
          this.router.navigate(['/ExistApplicationsPage'], { skipLocationChange: true, replaceUrl: true })
        })
      } else {
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert('Alert', fieldInsRes.ErrorDesc);
      }
    }, err => {
      this.globalData.globalLodingDismiss();
      if (err.name == "TimeoutError") {
    ...
    this.master.restApiCallAngular('autoapproval', data).then(res => {
      ...
      if (appData.errorCode === '000') {
        ...
        if (+this.postsanction.get('loanAmount').value > sanctionAmt) {
          this.globalData.showAlert('Alert', 'Your Requested Loan Amount is Greater then Sanction Amount, So The LoanAmount calculation was based on the Sanctioned LoanAmount.')
        }
        this.sqliteProvider.getApplicantDataforPostSanction(this.userInfo.refId).then(data => {
          ...
          setTimeout(() => {
            let body = {
              ...
              "LoanDetails": {
                ...
                //used
                "CC": this.postsanction.get('cc').value ? this.postsanction.get('cc').value : '',
                "UsedRcno": this.postsanction.get('rcNo').value ? this.postsanction.get('rcNo').value : '',
                "UsedEngineno": this.postsanction.get('engineNo').value ? this.postsanction.get('engineNo').value : '',
                "UsedChassisno": this.postsanction.get('chassisNo').value ? this.postsanction.get('chassisNo').value : '',
                "UsedYrsofmanufacture": this.postsanction.get('yearOfMan').value ? this.postsanction.get('yearOfMan').value : '',
                "UsedRegistrationdate": this.postsanction.get('registrationDate').value ? this.convertdate(this.postsanction.get('registrationDate').value) : '',
                "UsedVehicleAge": this.postsanction.get('vehicleAge').value ? this.postsanction.get('vehicleAge').value : '',
                "UsedHypothecationstatus": this.postsanction.get('hypothecation').value ? this.postsanction.get('hypothecation').value : '',
                "UsedNumberofowner": this.postsanction.get('noofOwner').value ? this.postsanction.get('noofOwner').value : '',
                "UsedKmdriven": this.postsanction.get('kmDriven').value ? this.postsanction.get('kmDriven').value : '',
                "UsedDealerquotation": this.postsanction.get('dealerQuotation').value ? this.postsanction.get('dealerQuotation').value : '',
                "UsedOBV": this.postsanction.get('obv').value ? this.postsanction.get('obv').value : '',
                "UsedFinalassetprice": this.postsanction.get('assetPrice').value ? this.postsanction.get('assetPrice').value : '',
                "UsedAssetageatmaturity": this.postsanction.get('assetAge').value ? this.postsanction.get('assetAge').value : ''
              }
            }
            this.master.restApiCallAngular('LoanDetails', body).then(res => {
              ...
              if (loanFacilityRes.ErrorCode === '000') {
                this.globalData.globalLodingDismiss();
                // this.globalData.showAlert('Alert',body.LoanDetails.AdvanceEmiAmt)
                this.sqlSupport.upDateLoanFacilites(body.LoanDetails, this.refId, this.id);
                this.sqlSupport.updateLoanAmountInPostSanction(sanctionAmt, this.refId, this.id);
                this.sqliteProvider.updateBasicDetailsinPostSanction(sanctionAmt, this.refId, this.id);
                this.sqlSupport.updateSanctionedAmount(sanctionAmt, this.applicantDetails[0].emi, this.applicantDetails[0].applicationNumber);
                this.getApplicantDataforPostSanction();
                this.sqlSupport.updateScorecardRerunStatus('autoApprovalStatus', this.refId, this.id);
                this.disableAutoApprovalBtn = true;
                // if (this.missingAmount !== undefined) {
                //   this.sqlSupport.updateMissingValuesInPS(this.missingAmount, this.refId, this.id)
                // }
                this.globalData.showAlert("Alert!", (appData.errorDesc) ? appData.errorDesc : appData.status);
              } else {
                this.globalData.showAlert('Alert', loanFacilityRes.ErrorDesc);
              }
            }, (err) => {
              if (err) {
                this.globFunc.showAlert('Alert', err.message);
        ...
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert("Alert!", appData.errorDesc);
      }
    }, (err) => {
      if (err) {
        this.globFunc.showAlert('Alert', err.message);
  ...
  async viewIdProof(doc, imgs) {
    let modal = await this.modalCtrl.create({
      ...
      componentProps: { proofpics: imgs, disableDoc: this.disableDoc, postSancDoc: true }
    });
  ...
  updateimg(upid, imgData) {
    for (let i = 0; i < imgData.length; i++) {
      this.sqliteProvider.addPostSanctionImages(this.refId, this.id, imgData[i].imgpath, upid, imgData[i].postSanImgId).then(data => {
        if (i == imgData.length - 1) {
          this.getProofImg();
        }
      }).catch(Error => {
        console.log("Failed!" + Error);
        this.globalData.showAlert("Alert!", "Document Insert Failed!");
      })
    }
  }
  ...
  getProofImg() {
    for (let i = 0; i < this.documents.length; i++) {
      this.sqliteProvider.getPostSanctionImages(this.documents[i].docId, this.refId, this.id).then(imgData => {
        if (0 < imgData.length) {
  ...
  loanCheckCBS() {
    if (!this.cbsButtonEnable) {
      this.globFunc.showAlert('Alert', "Please complete CBS Customer Creation");
    } else if (!this.cbsCustomerEnable) {
      this.globFunc.showAlert('Alert', "Please complete CBS Customer Creation");
    } else if ((this.casaData == 'Y' && !this.cbsAccountEnable && this.casaValue.editedInPS != 'Y')) {
      this.globFunc.showAlert('Alert', "Please complete Account Creation");
    } else if ((this.casaData == 'Y' && !this.instakitStatus && this.casaValue.editedInPS != 'Y')) {
      this.globFunc.showAlert('Alert', "Please complete Instakit Creation");
    } else {
      ...
      this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
        ...
        if (creditCheck.ErrorCode === '000') {
          this.fowardCheck = false;
          this.sqlSupport.updateDisbursement(this.applicantDetails[0].applicationNumber);
          this.globFunc.globalLodingDismiss();
          this.enableEditAccStage = 0;
          this.globFunc.showAlert('Alert', "Successfully Forwarded");
          this.router.navigate(['/ExistApplicationsPage'], { skipLocationChange: true, replaceUrl: true })
        } else {
          this.globFunc.globalLodingDismiss();
          this.globFunc.showAlert('Alert', creditCheck.ErrorDesc);
        }
    ...
    if (loanAmt) {
      ...
      if (processFeesAmt.length > 0) {
        ...
        if (+proceesPercentage <= +processFeesAmt[0].minProcessingFee) {
          ...
          this.applicantDetails[0].processingFee = (+processFeesAmt[0].minProcessingFee);
          ...
          this.applicantDetails[0].processingFee = (+processFeesAmt[0].maxProcessingFee);
        } else {
          let charges;
  ...
  calculateMarginCost() {
    let maronRoadprice = +this.postsanction.get('onroadPrice').value ? +this.postsanction.get('onroadPrice').value : +this.newOnRoadPrice;
    let finalRoadprice = +this.postsanction.get('assetPrice').value ? +this.postsanction.get('assetPrice').value : +this.newFinalRoadPrice;
    let marLoanAmt = +this.postsanction.controls.loanAmount.value;
    if (this.vehicleType == "N") {
      if (+maronRoadprice <= marLoanAmt) {
        this.globalData.showAlert("Alert!", `Loan Amount should be lesser than On Road Price Amount(Rs. ${maronRoadprice})`);
        this.postsanction.controls.loanAmount.setValue('');
        this.postsanction.controls.loanAmount.updateValueAndValidity();
      } else {
      ...
      if (+finalRoadprice <= marLoanAmt) {
        this.globalData.showAlert("Alert!", `Loan Amount should be lesser than Final Asset Price(Rs. ${finalRoadprice})`);
        this.postsanction.controls.loanAmount.setValue('');
        this.postsanction.controls.loanAmount.updateValueAndValidity();
      } else {
  ...
  EMICalculation() {
    if (this.postsanction.get('tenure').value !== '' && this.applicantDetails[0].intRate !== '' && this.postsanction.get('totalloanAmount').value !== '' && this.postsanction.get('totalloanAmount').value != null) {
      var Rate = (+this.applicantDetails[0].intRate) / (12 * 100);
      var denominator = Math.pow(Rate + 1, +this.postsanction.get('tenure').value) - 1;
      var Numerator = Rate * Math.pow(Rate + 1, +this.postsanction.get('tenure').value);
      var EMI = (Numerator / denominator) * (+this.postsanction.get('totalloanAmount').value);
      let emiAmt;
  ...
  uploadPslImg() {
    for (let i = 0; i < this.pslImg.length; i++) {
      this.sqlSupport.addPslImages(this.refId, this.id, this.pslImg[i].imgpath, this.pslId).then(data => {
      }).catch(Error => {
        console.log("Failed!" + Error);
  ...
  updatePslImg(pslId) {
    this.sqlSupport.removePslImages(pslId).then(data => {
      for (let i = 0; i < this.pslImg.length; i++) {
        this.sqlSupport.addPslImages(this.refId, this.id, this.pslImg[i].imgpath, pslId).then(data => {
        }).catch(Error => {
          console.log("Failed!" + Error);
          this.globalData.showAlert("Alert!", "Failed!");
        })
  ...
  pddChargesCalc() {
    ...
    if (pddcharges.length > 0) {
      ...
      //this.globalData.showAlert("Alert!", `Loan Amount value is ${loanAmt}`);
  ...
  postSanctionWFBLForETC(scoreResponse: any, loanAmountOption: number) {
    ...
    if (scrRsp == 'Y') { //STP
      this.globalData.showAlert("Alert!", "Please submit for auto approval!");
      this.showFieldInvestigation = this.showManualUser = false;
      this.showAutoApproval = true;
    } else { //FI
      if (scoreResponse.FIENTRY == 'Y') { //FIEntry
        this.sqlSupport.updatePostSanctionManualApprovalFlag('Y', this.refId, this.id);
        this.showAutoApproval = this.showFieldInvestigation = false;
        this.showManualUser = true;
      } else {
        this.globalData.showAlert("Alert!", "Please submit for FI!");
        this.showAutoApproval = this.showManualUser = false;
        // if (this.missingAmount !== undefined) {
        //   this.sqlSupport.updateMissingValuesInPS(this.missingAmount, this.refId, this.id)
        // }
  ...
  loanFacilitiesServiceCall(value) {
    if (this.valuesModified == 'Y' || this.TenureModified || this.segmentModified || this.dbDateModified || this.ccModified || this.rcNoModified ||
      this.engineNoModified || this.chassisNoModified || this.yearOfManModified || this.registrationDateModified || this.vehicleAgeModified ||
      this.hypothecationModified || this.noofOwnerModified || this.kmDrivenModified || this.dealerQuotationModified || this.obvModified ||
      this.assetPriceModified || this.assetAgeModified) {
      this.globalData.globalLodingPresent('Please wait...');
      ...
      this.master.restApiCallAngular('LoanDetails', body).then(res => {
        ...
        if (loanFacilityRes.ErrorCode === '000') {
          this.globalData.globalLodingDismiss();
          this.sqlSupport.upDateLoanFacilites(body.LoanDetails, this.refId, this.id);
          this.sqlSupport.updateORPinPostSanction(this.postsanction.get('onroadPrice').value, this.refId, this.id);
          this.sqlSupport.updateAssetPricePostSanction(this.postsanction.get('assetPrice').value, this.refId, this.id);
          // this.globalData.showAlert('Alert',body.LoanDetails.AdvanceEmiAmt)
  ...
  */
  ...
  casaDetailsCheck() {
    this.sqliteProvider.getSubmitDetails(this.userInfo.refId, this.userInfo.id).then(submitlead => {
      if (submitlead[0].NACH != "1" && submitlead[0].nachDoc != "Y") {
        this.casaType = 'nachDetails';
        this.globalData.showAlert("Alert!", `Please complete NACH Details`);
      }
    });
  ...
  cbsCustomerCreation() {
    if (this.editCasaSaved == 2 || this.casaData == 'N' || (this.casaData == 'Y' && this.accCreationEdit == true && this.editAccCreation == true)) {
      this.accCreationEdit = true;
      this.editAccCreation = true;
      this.globalData.globalLodingPresent("please wait...");
      let body = {
      ...
      this.master.restApiCallAngular('CasaCBSService', body).then(res => {
        ...
        if (cbsCustomerRes.ErrorCode == "000") {
          ...
          this.globalData.globalLodingDismiss();
          this.sqlSupport.updateCBSstatus(this.refId, this.id, cbsCustomerRes.cbsCustId, this.PropNo, "customerId", "cbsCustomerEnable");
          this.sqlSupport.updateCBSAfterCBS(this.refId, this.id, this.PropNo);


          this.globalData.showAlert("Alert!", cbsCustomerRes.ErrorDesc);
          this.accountCreation.get('customerId').setValue(cbsCustomerRes.cbsCustId);
      ...
      if (this.editCasaSaved == 1) {
        this.globFunc.showAlert('Alert', 'Please Complete the Service Request Details!')
      } else {
        this.globFunc.showAlert('Alert', 'Please Complete the Nominee and Service Request Details!')
      }
    }
  }
  ...
  casaAccountCreation() {
    ...
    this.master.restApiCallAngular('CasaCBSService', body).then(res => {
      ...
      if (cbsCustomerRes.ErrorCode == "000") {
        this.cbsAccountEnable = true;
        this.globalData.globalLodingDismiss();
        this.sqlSupport.updateCBSstatus(this.refId, this.id, cbsCustomerRes.cbsAccountNumber, this.PropNo, "accountNo", "cbsAccountEnable");

        this.globalData.showAlert('Alert', cbsCustomerRes.ErrorDesc);
        this.accountCreation.get('accountNo').setValue(cbsCustomerRes.cbsAccountNumber);
  ...
  instakitCreation() {
    ...
    if (instaKitValue == this.savedInstaKitNumber) {
      this.globalData.showAlert("Alert!", "This number already used for instakit creation!");
    } else {
      if (instaKitValue != null && instaKitValue != "" && instaKitValue != undefined && instaKitValue.length == 12) {
        this.globalData.globalLodingPresent("please wait...");
        let body = {
          "propNo": this.PropNo,
          "instakitNumber": instaKitValue
        }
        this.master.restApiCallAngular('instakitUpdate', body).then(res => {
          ...
          if (inskitRes.ErrorCode == "000") {
            ...
            this.globalData.globalLodingDismiss();
            this.globalData.showAlert('Alert', inskitRes.ErrorDesc);
            this.sqlSupport.updateCBSstatus(this.refId, this.id, instaKitValue, this.PropNo, "instaKitNumber", "instakitStatus");
        ...
        this.globalData.showAlert("Alert!", "Please enter valid Instakit Number");
      }
    }
  }
  ...
  getCBSButtonEnable() {
    console.log(this.psmSubmitted);
    console.log(this.sanctionModify == '1' && this.postSancModified == 'Y' && this.showAutoApproval && this.disableAutoApprovalBtn);
    if (!this.psmSubmitted && !(this.sanctionModify == '1' && this.postSancModified == 'Y' && this.showAutoApproval && this.disableAutoApprovalBtn)) {
      if (this.sanctionModify != '2') {
        this.globFunc.showAlert("Alert!", `Please complete Post Sanction Modification Details`);
        this.custType = '';
    ...
    if (!this.cbsButtonEnable) {
      ...
      this.master.restApiCallAngular('getCasaApiDetails', body).then(res => {
        let cbsButtonData = <any>res;
        if (cbsButtonData.ErrorCode == "000") {
          this.cbsCustomerEnable = (cbsButtonData.cbsCustomerFlag == 'enable') ? false : true;
          this.cbsAccountEnable = (cbsButtonData.cbsAccountFlag == 'enable') ? false : true;
          this.instaKitEnable = !this.cbsCustomerEnable && !this.cbsAccountEnable && (0 < +cbsButtonData.cbsAccountNumber);

          this.cbsButtonEnable = true;
          this.cbsButtonServiceCheck = false;
          this.accountCreation.controls.customerId.setValue(cbsButtonData.cbsCustId);
          this.accountCreation.controls.customerId.updateValueAndValidity();
          this.accountCreation.controls.accountNo.setValue(cbsButtonData.cbsAccountNumber);
          this.accountCreation.controls.accountNo.updateValueAndValidity();
          this.sqlSupport.insertCBSstatus(this.refId, this.id, cbsButtonData.cbsCustId, cbsButtonData.cbsAccountNumber, this.PropNo, this.cbsCustomerEnable, this.cbsAccountEnable, this.instaKitEnable, this.cbsButtonEnable);
          ...
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert('Alert', cbsButtonData.ErrorDesc);
        }
  ...
  */
  ...
  pslDocUpload() {
    if (this.pslImgArr.length > 0) {
      ...
      this.master.restApiCallAngular('UploadDocs', docs_upload).then(res => {
        this.globFunc.globalLodingDismiss();
        let pslRes = <any>res;
        if (pslRes.errorCode == '00') {
          this.globFunc.showAlert('Alert', 'PSL Submitted Successfully')
        }
      }).catch(err => err)
    }
  }
  ...
  pslAgriSave(value) {
    ...
    if (this.pslImg.length > 0) {
      ...
      this.master.restApiCallAngular('pslDetailsVL', body).then(res => {
        console.log(res)
        let response = <any>res;
        if (response.ErrorCode === '000') {
          this.sqliteProvider.addPSLDetails(this.refId, this.id, value, this.pslId).then(data => {
            if (this.pslId) {
          ...
          this.globFunc.globalLodingDismiss();
          this.globFunc.showAlert("Alert", response.ErrorDesc);
        }
      }).catch(err => err)
    } else {
      this.globFunc.showAlert('Alert', 'Please Add Certificate image')
    }
  }
  ...
  */
  ...
  submitPostSanction() {
    ...
    this.sqlSupport.getCASADetails(this.refId, this.id).then(async data => {
      ...
      if (casaValue.editedInPS == 'Y') {
        ...
        if (customerIdValue && customerIdValue != '0') {          // new condition
          this.getNomineeDetails().then(data => {
            this.getServiceDetails(casaValue).then(async data => {
              if (this.documents.every(data => data.imgs.length > 0)) {
                for (let i = 0; i < this.documents.length; i++) {
                  for (let j = 0; j < this.documents[i].imgs.length; j++) {
                    if (this.documents[i].imgs[j].uploaded == 'N') {
                      ...
                      let postSancImgs: string = this.documents[i].imgs[j]['imgpath'];
                      if (postSancImgs.includes('jpeg') || postSancImgs.includes('data:image/*')
                      ) filetype = 'jpg'
                      else {
                      ...
                      if (j == this.documents[i].imgs.length - 1 && i == this.documents.length - 1) {
                ...
                this.globalData.showAlert("Alert!", "Please add documents!");
          ...
          this.globalData.showAlert("Alert!", "Please Complete CBS Customer Creation!");
        }
      } else {
        if (this.documents.every(data => data.imgs.length > 0)) {
          for (let i = 0; i < this.documents.length; i++) {
            for (let j = 0; j < this.documents[i].imgs.length; j++) {
              if (this.documents[i].imgs[j].uploaded == 'N') {
                ...
                if (postSancImgs.includes('jpeg') || postSancImgs.includes('data:image/*')) filetype = 'jpg'
                else {
                  postSancImgs = `data:image/*;charset=utf-8;base64,${postSancImgs}`
                  filetype = 'pdf'
                }
    ...
    if (this.post_other_docs.length > 0) {
      ...
      this.master.restApiCallAngular('UploadDocs', docs_upload).then(res => {
        ...
        if (sanctionRes.errorCode == "00") {
          ...
          for (let i = 0; i < this.documents.length; i++) {
            for (let j = 0; j < this.documents[i].imgs.length; j++) {
              this.sqlSupport.updatePostSancDocUploadStatus(this.documents[i].imgs[j]).then(data => {
                if (j == this.documents[i].imgs.length - 1 && i == this.documents.length - 1) {
                  this.getProofImg();
                }
              });
          ...
          this.post_other_docs = [];
          this.globFunc.globalLodingDismiss();
          this.globalData.showAlert("Alert!", "Documents uploaded successfully!");

        } else {
          this.globFunc.globalLodingDismiss();
          this.globalData.showAlert('Alert', sanctionRes.errorDesc || sanctionRes.errorMsg);
        }
      }, err => {
        this.globalData.globalLodingDismiss();
        if (err.name == "TimeoutError") {
      ...
      this.globFunc.globalLodingDismiss();
      this.globalData.showAlert("Alert!", "All documents already uploaded!");
    }
    // this.globFunc.globalLodingDismiss();
  }
    ...
    if (this.postSanctionCheck) {
      if (this.sanctionModify == '1') {
        if (this.psmSubmitted) {
          ...
          if (this.scoreCardRerun == 'Y' && this.psmFlowState != 'scoreCard' && this.FientryFlag == 'N') {
            this.globFunc.showAlert('Alert', `Please complete score card rerun in ScoreCard Page!`);
            ...
            //   }
            // });
          } else if (this.autoApprovalFlag == 'Y' && !this.disableAutoApprovalBtn) {
            this.globFunc.showAlert('Alert', `Please complete Auto Approval in post sanction modification!`);
          } else if ((this.autoApprovalFlag == 'N' && this.FieldInvFlag == 'N') && this.showFieldInvestigation && !this.disableFISubmitBtn) {
            this.globFunc.showAlert('Alert', `Please complete FI in post sanction modification!`);
          } else if ((this.autoApprovalFlag == 'N' && this.FieldInvFlag == 'Y') && !this.disableManualApprvalBtn) {
            this.globFunc.showAlert('Alert', `Please complete Manual Approval in post sanction modification!`);
          } else {
            this.loanCheckCBS();
          }
        }
      ...
      this.globFunc.showAlert('Alert', "Please complete Post Sanction Modification");
    }
  }
  ...
  getJanaAccDetails(event) {
    ...
    if (value == 'N' && this.enableEditAccStage == 1) {
      this.globalData.confirmationAlert('Alert', 'Your CASA Account will be delete. Do you want to Proceed with No?').then(data => {
        if (data) {
          this.sqlSupport.updateCasaNoinPS(this.refId, this.id).then(data => {
            this.editAccCreationTab = false;
            this.editAccCreation = true;
            this.enableEditAccStage = 0;
  ...
  async accCreationTab(value, option) {
    ...
    if (value == 'Nominee') {
      if (this.casaValue.editedInPS == 'Y') {
        ...
        this.sqlSupport.getNomineeDetails(this.refId, this.id).then(async val => {
          let nomineeValues = [];
          nomineeValues = val;
          let nominee = await this.modalCtrl.create({
            component: NomineeAfterPsPage,
          ...
          nominee.onDidDismiss().then((data) => {
            ...
            if (data != undefined) {
              ...
              this.nomineeTick = true;
              this.accCreationEdit = true;
              this.globalData.showAlert('Alert', 'Nominee saved Successfully. Proceed Services Requested details')
            } else if (nomineeValues.length > 0 || this.casaDetails.nomAvail == 'N') {
              this.nomineeTick = true;
              this.accCreationEdit = true;
            } else {
              this.globalData.showAlert('Alert', 'Please Fill nominee details, and then proceed further.');
              this.casaType = 'casaDetails';
              this.nomineeTick = false;
            }
          })
      ...
      if (this.casaValue.editedInPS == 'Y') {
        ...
        if (this.nomineeTick == true) {
          this.sqlSupport.getServDetails(this.refId, this.id).then(async val => {
            let servDetails = []
            servDetails = val;
            let nominee = await this.modalCtrl.create({
              component: ServiceAfterPsPage,
            ...
            nominee.onDidDismiss().then((data) => {
              if (data != undefined) {
                ...
                this.saveCasaAccDetails().then(data => {
                  ...
                  this.casaType = 'casaDetails';
                  this.servicesTick = true;
                  this.globalData.showAlert('Alert', 'Service request saved Successfully.')
                })
              } else if (servDetails.length > 0) {
                this.servicesTick = true;
              } else {
                this.globalData.showAlert('Alert', 'Please Fill Service request, and then proceed further.');
                this.casaType = 'casaDetails';
                this.servicesTick = false;
              }
            })
          ...
          this.globFunc.showAlert('Alert', 'Please Complete and Save the Nominee details!')
        }
      }
    }
  }
  ...
  saveCasaAccDetails() {
    return new Promise((resolve, reject) => {
      this.sqlSupport.updateCasaOnPS(this.refId, this.id).then(data => {
        if (this.casaId == null || this.casaId == '' || this.casaId == undefined) {
          this.casaId = data.insertId;
          resolve(this.casaId);
          this.globFunc.showAlert("Alert!", "Current/Savings Account details saved!")
        } else {
          this.globFunc.showAlert("Alert!", "Current/Savings Account details updated!")
        }
    ...
    return new Promise((resolve, reject) => {
      this.sqlSupport.getNomineeDetails(this.refId, this.id).then(data => {
        if (data.length > 0) {
          ...
          this.globalData.showAlert("Alert!", "Nominee details not found");
        }
      }).catch(e => {
        console.log("er" + e);
      })
  ...
  getServiceDetails(value) {
    return new Promise((resolve, reject) => {
      if (value.janaAcc == 'Y') {
        ...
        this.globalData.showAlert('Alert', 'ServiceDetails Not saved Properly!')
      }
    })
  }
  ...
  editedCasaSave(noInPs) {
    return new Promise((resolve, reject) => {
      try {
          ...
          if (casaValues) {
            ...
            if (casaValues.nomAvail == "Y" && !noInPs) {
              this.casaSubmitData = {
                ...
                "nomineeDetails": {
                  "nomieeTitle": this.nominee_Data ? this.nominee_Data[0].nomTitle.toUpperCase() : "",
                  "nomieeName": this.nominee_Data ? this.nominee_Data[0].nominame : "",
                  "nomieedob": this.nominee_Data ? this.nominee_Data[0].nomdob.substring(8, 10) + "/" + this.nominee_Data[0].nomdob.substring(5, 7) + "/" + this.nominee_Data[0].nomdob.substring(0, 4) : "",
                  "nomineeguardianname": this.nominee_Data ? this.nominee_Data[0].guaname : "",
                  "nomieeage": this.nominee_Data ? this.nominee_Data[0].nomiage : "",
                  "nomieerel": this.nominee_Data ? this.nominee_Data[0].nomrelation : "",
                  "nomieeadd1": this.nominee_Data ? this.nominee_Data[0].nomi_address1 : "",
                  "nomieeadd2": this.nominee_Data ? this.nominee_Data[0].nomi_address2 : "",
                  "nomieeadd3": this.nominee_Data ? this.nominee_Data[0].nomi_address3 : "",
                  "nomieecity": this.nominee_Data ? this.nominee_Data[0].nomicities : "",
                  "nomieestate": this.nominee_Data ? this.nominee_Data[0].nomistates : "",
                  "nomieepin": this.nominee_Data ? this.nominee_Data[0].nomipincode : "",
                  "nomieecontry": "INDIA",
                  "nomieecontno": this.nominee_Data ? this.nominee_Data[0].nomiCNum : "",
                  "guardianDetails":
                  {
                    "guardianTitle": this.nominee_Data ? this.nominee_Data[0].guaTitle : "",
                    "guardianName": this.nominee_Data ? this.nominee_Data[0].guaname : "",
                    "reltoNominee": this.nominee_Data ? this.nominee_Data[0].guarelation : "",
                    "guardianContNo": this.nominee_Data ? this.nominee_Data[0].guaCNum : "",
                    "guardianAddressLine1": this.nominee_Data ? this.nominee_Data[0].gua_address1 : "",
                    "guardianAddressLine2": this.nominee_Data ? this.nominee_Data[0].gua_address2 : "",
                    "guardianCity": this.nominee_Data ? this.nominee_Data[0].guacities : "",
                    "guardianState": this.nominee_Data ? this.nominee_Data[0].guastates : "",
                    "guardianPincode": this.nominee_Data ? this.nominee_Data[0].guapincode : "",
                    "guardianCountry": "INDIA"
                  }
                },
                "serviceRequested": {
                  "modeofop": this.service_data ? this.service_data[0].modeofoper : "",
                  "operinstruction": this.service_data ? this.service_data[0].operaInst : "",
                  "custid": "",
                  "instakitno": this.accountCreation.get('instaKitNumber').value ? this.accountCreation.get('instaKitNumber').value : "",
            ...
            this.master.restApiCallAngular('CurrentAccDet', this.casaSubmitData).then(data => {
              if ((<any>data).ErrorCode == "000") {
                if (!noInPs) {
                  this.getAnnexImgs(this.service_data[0].serId).then(data => {
                    this.getServiceImgs(this.service_data[0].serId)
                    resolve(data);
                  })
                } else {
                  this.globalData.showAlert('Alert', 'CASA Submitted Successfully')
                }
              } else {
                if ((<any>data).ErrorDesc) {
                  ...
                  this.globalData.globalLodingDismissAll();
                  this.globalData.showAlert("Alert!", "Application CA/SA Details submission Failed!");
                }
              }
            }, err => {
              this.globalData.globalLodingDismissAll();
        ...
        this.globalData.showAlert("Alert!", error.message);
        this.sqliteProvider.addAuditTrail(moment(new Date()).format("YYYY-MM-DD HH:mm:ss"), "casaInit", "casaInit", JSON.stringify(error));
      }
    })
  }
  ...
  getAnnexImgs(serId) {
    return new Promise((resolve, reject) => {
      this.sqlSupport.getAnnexure(serId).then(annex => {
        if (annex.length > 0) {
          this.annex_imgs = [];
          for (let i = 0; i < annex.length; i++) {
            ...
            let temp_annex = {
              ...
              "annexureImgName": this.applicantDetails[0].applicationNumber + "_ANN_" + i + ".jpg"
            };
            this.annex_imgs.push(temp_annex);

            let annexData = {
              "DocName": "ANNEX",
              "DocDesc": this.applicantDetails[0].applicationNumber + "_ANN_" + i + ".jpg",
  ...
  getServiceImgs(serId) {
    return new Promise((resolve, reject) => {
      this.sqlSupport.getSignImages(serId).then(sign => {
        if (sign.length > 0) {
  ...
  advanceEMIAdded(event) {
    ...
    if (advanceEMI.length > 0 && advanceEMI !== this.applicantDetails[0].advanceEmi) {
      this.applicantDetails[0].advanceEmi = event.detail.value
      this.downPaymentCalc();
      this.preEmiNoCalculation();
    } else if (advanceEMI == "") {
      this.globalData.showAlert('Alert', 'Advance EMI should not be Empty!')
    }
    // }
  }
  ...
  tenureValidation() {
    ...
    if (((+tenureval[0].NAME) < this.tenureFrom) || ((+tenureval[0].NAME) > this.tenureTo)) {
  ...
  getPermAddressInfo() {
    ...
    this.sqliteProvider.getPresentAddress(this.refId, this.id).then(async data => {
      ...
      await this.sqliteProvider.getSelectedCity(pres).then(data => {
        let presCity = data;
        this.presentCity = presCity.filter(data => data.cityCode == this.globFunc.basicDec(presAddressData[0].pres_cities));
      });
    })
  }
...
*/
  async orpAuthFetch(type, brandName?: string) {
    ...
    let value: string;
    let year: string = this.vehicleType == 'N' ? this.today.getFullYear() : this.postsanction.get("yearOfMan").value;
    switch (type) {
      case vehicleType.category:
        value = this.globalData.convertToString(this.postsanction.get("vehicleCatogery").value);
        await this.orpApi.frameRequestORPNew('category', value).then(response => {
          this.makeApiList = response;
        })
        break;
      case vehicleType.model:
        value = this.globalData.convertToString(this.postsanction.get("brandName").value);
        await this.orpApi.frameRequestORPNew('model', value).then(response => {
          this.modelApiList = response;
        })
        break;
      case vehicleType.variant:
        value = this.globalData.convertToString(this.postsanction.get("model").value);
        brandName = brandName ? brandName : ''
        await this.orpApi.frameRequestORPNew('variant', value, year, brandName).then(response => {
          this.varientApiList = response;
        })
        break;
      case vehicleType.ORP:
        if (this.vehicleType == 'N') {
          value = this.globalData.convertToString(this.postsanction.get("variant").value);
          await this.orpApi.frameRequestORPNew('ORP', value, year, '', this.presentCity[0].cityName).then((response: any) => {
          ...
          let body = {
            make: this.globalData.convertToString(this.postsanction.get("brandName").value),
            model: this.globalData.convertToString(this.postsanction.get("model").value),
            year: this.globalData.convertToString(this.postsanction.get("yearOfMan").value),
            trim: this.globalData.convertToString(this.postsanction.get("variant").value),
            kms_driven: this.globalData.convertToString(this.postsanction.get("kmDriven").value),
            city: this.presentCity[0].cityName,
            noOfOwners: this.globalData.convertToString(this.postsanction.get("noofOwner").value),
          }
          await this.orpApi.frameRequestORPUsed(body).then(async response => {
            if (response) {
              ...
              this.postsanction.get("apiFlag").setValue('Y');
              this.obvEditable = await this.setDealerValue(this.postsanction.value.dealerName, 'Y');
  ...
  async setDealerValue(dealerCode: any, findName?: string) {
    const dealer = await this.dummy_masterDealer.find(val => val.dealerCode == dealerCode);
    if (dealer) {
      if (findName == 'Y') {
...
*/
  obvChange(value: number) {
    try {
      ...
      this.obvEditable ? this.postsanction.controls.dealerQuotation.setValue('') : '';
...
*/
  async fetchDetailsFromRC() {
    try {
      ...
      await this.orpApi.getRCDetails(this.postsanction.get('rcNo').value).then((data: any) => {
        this.globFunc.globalLodingDismiss();
        console.log('getRCDetails', data);
        if (data) {
          if (!data.blacklisted) {
            this.postsanction.get('engineNo').setValue(data.engine_number ? data.engine_number : 'NA');
            this.postsanction.get('chassisNo').setValue(data.chassis_number ? data.chassis_number : 'NA');
            this.postsanction.get('registrationDate').setValue(data.registration_date ? this.globalData.formatDateString(data.registration_date) : 'NA');
            this.postsanction.get('hypothecation').setValue(data.financing_authority ? data.financing_authority : 'No');
            this.postsanction.get('noofOwner').setValue(data.owner_serial_number);
            this.postsanction.get('nameAsPerRC').setValue(data.vehicle_model ? data.vehicle_model : 'NA');
            ...
            this.globFunc.showAlert('Alert', 'Given RC Number is Black Listed!');
            this.reSetRCDetails();
            this.RCFetchDetailsDisable = false;
          }
        } else {
          this.globFunc.showAlert('Alert', 'Details not found!');
          this.reSetRCDetails();
          this.RCFetchDetailsDisable = false;
        }
      })
...
*/
  orpApiPriceCheck() {
    if (this.orpEmpty) {
      if (+this.postsanction.get("onroadPrice").value > this.apiORPPrice && this.apiORPPrice !== 0) {
    ...
    } else if (+this.postsanction.get("onroadPrice").value > this.manualORPLimit) {
      this.postsanction.get("onroadPrice").setValue('');
      this.postsanction.updateValueAndValidity();
  ...
  orpApiValueChangeFieldReset(type: string) {
    switch (type) {
      ...
      case 'model':
        ...
        break;
      case 'variant':
        this.vehicleType == 'N' ? this.postsanction.get("onroadPrice").reset() : this.postsanction.get("kmDriven").reset()
        break;
    }
  }
}
