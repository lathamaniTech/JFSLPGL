...
export class PosidexCheckPage {
  ...
  ionViewDidLoad() {
    ...
    this.globFunc.publishPageNavigation({ title: 'Posidex Details', component: PosidexCheckPage });
  }
  ...
  loadAllPosidexDetails(id?) {
    this.sqliteProvider.getPosidexDetails(this.refId, this.userType, id).then(data => {
      if (data.length > 0) {
        if (data[0].verified == 'Y') {
          this.posidexInfo = data;
          if (data[0].existCust == 'Y' && JSON.parse(data[0].matchedCustomerDetails).MatchedCRN !== 'new') {
            this.showCbs = true;
            this.showEixtCust = true;
            this.existingCustomer = [];
            this.existCustCheck = true;
            this.existingCustomer.push(JSON.parse(data[0].matchedCustomerDetails));
          ...
          } else if (data[0].matchedCustomerDetails && JSON.parse(data[0].matchedCustomerDetails).MatchedCRN == 'new') {
            this.existingUrn = data[0].urnNo;
            this.showAml = true;
            this.storedRemarks = data[0].newCustomerRemarks;
            this.existingCustomer = [];
  ...
  loadCoappDetails() {
    this.sqliteProvider.getCoApplicantDataAfterSubmit(this.refId).then(res => {
      if (res.length > 0) {
  ...
  fetchPosidex(ev?) {
    ...
    if (this.appCustId || this.coAppCustId) {
      ...
      this.globFunc.showAlert('Alert', 'The Customer Id is Empty in the Application');
    }
    this.master.restApiCallAngular('fetchcustposidex', body).then(res => {
      let posidexData = <any>res;
      if (posidexData.errorCode === '000') {
        this.globFunc.globalLodingDismiss();
        if (posidexData.hasOwnProperty('MatchedCustomer') && posidexData.MatchedCustomer.length > 0) {
          this.existingUrn = posidexData.lpUrnNo
          this.existingCustomer = posidexData.MatchedCustomer;
          if (this.leadUserType != 'E') {
            this.existingCustomer.unshift({ newCust: 'Proceed as new customer', MatchedCRN: 'new', urn: this.existingUrn })
          }
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', posidexData.errorStatus);
      }
  ...
  async verifyPosidex(ev?) {
    let body, remarks;
    if ((this.showNewCust && this.urnSelected) || (this.selectedItem && this.showEixtCust)) {
      let popover = await this.modalCtrl.create({
        component: RemarksComponent,
        componentProps: { data: (this.showEixtCust) ? this.selectedItem : this.newCustomerUrn, showBackdrop: true, enableBackdropDismiss: true, cssClass: 'modalCss' }
      });
      popover.onDidDismiss().then((data) => {
        ...
        if (data) {
          ...
          if (this.showEixtCust && this.selectedItem.MatchedCRN != 'new') {
            ...
            // new 
            let urn;
            if (this.selectedItem && this.selectedItem.hasOwnProperty('MatchedCRN')) {
              urn = this.selectedItem.MatchedCRN == 'new' ? this.selectedItem.urn : (this.newCustomerUrn) ? this.newCustomerUrn : this.existingUrn;
            } else {
              urn = (this.newCustomerUrn) ? this.newCustomerUrn : this.existingUrn;
            }
            body = {
              "propNo": this.applicantDetails[0].applicationNumber,
              "CustId": (this.custType === 'Promoter') ? this.appCustId : this.coAppCustId,
          ...
          this.master.restApiCallAngular('verifycustposidex', body).then(res => {
            let verifyPosidexData = <any>res;
            if (verifyPosidexData.errorCode === '000') {
              if (this.showEixtCust) {
                this.sqliteProvider.insertVerifiedPosidex(this.refId, this.id, this.userType, 'Y', JSON.stringify(this.selectedItem), remarks, 'Y', this.existingUrn, this.applicantDetails[0].applicationNumber, (this.custType === 'Promoter') ? this.appCustId : this.coAppCustId);
              } else {
                this.sqliteProvider.insertVerifiedPosidex(this.refId, this.id, this.userType, 'Y', JSON.stringify(this.selectedItem), remarks, 'N', this.newCustomerUrn, this.applicantDetails[0].applicationNumber, (this.custType === 'Promoter') ? this.appCustId : this.coAppCustId);
              }
              this.loadAppPosidexDetails();
              this.loadAllPosidexDetails(this.id);
              // this.globFunc.globalLodingDismiss();
              if (this.showNewCust || this.selectedItem.MatchedCRN == 'new') {
                this.showAml = true;
                this.globFunc.showAlert('Alert', 'Posidex Verified Successfully. Please check AML');
              } else {
                this.showCbs = true;
                this.globFunc.showAlert('Alert', 'Posidex Verified Successfully. Please check CBS');
              }

            } else {
              // this.globFunc.globalLodingDismiss();
              this.globFunc.showAlert('Alert', verifyPosidexData.errorDesc ? verifyPosidexData.errorDesc : 'Posidex Failed to Verify');
            }

          }, (err) => {
            // this.globFunc.globalLodingDismiss();
            if (err) {
              this.globFunc.showAlert('Alert', err.message ? err.message : 'Error');
            } else {
              this.globFunc.showAlert("Alert", "No Response from Server!");
            }
          })
      ...
      // this.globFunc.globalLodingDismiss();
      this.globFunc.showAlert('Alert', 'Please Select a URN');
  ...
  fetchAml() {
    ...
    if (this.posidexInfo[0].amlStatus == 'AML Initiated' || this.posidexInfo[0].amlStatus == 'Match') {
    ...
    this.master.restApiCallAngular(methodName, body).then(res => {
      let amlData = <any>res;
      if (amlData.errorCode === '000') {
        ...
        this.sqliteProvider.updatePosidexAMLDetails(this.refId, this.id, (amlData.errorDesc == 'Success') ? 'AML Initiated' : amlData.errorDesc, this.userType, 'Y');
        this.loadAllPosidexDetails(this.id);
        this.globFunc.globalLodingDismiss();
        if (amlData.errorDesc == 'Success') {
          this.globalData.showAlert('Alert', 'AML request initiated. Please try again!');
        } else if (amlData.errorDesc == 'Match') {
          this.globalData.showAlert("Alert", "The AML Status is Match, so please wait untill Processing!");
        } else {
          this.globalData.showAlert('Alert', amlData.errorDesc).then(data => {
            this.proceedNextPage();
          });
        }
        // this.globalData.showAlert('Alert', amlData.errorDesc ? amlData.errorDesc : 'Aml Initiated').then(data => {
        // this.proceedNextPage();
        // })
    ...
    this.master.restApiCallAngular(methodName, body).then(res => {
      ...
      if (!this.isEmpty(cbsData) && cbsData.errorCode === '000') {
        if (cbsData.ExistingVehicle == 'Y') {
          this.sqliteProvider.updatePosidexCBSDetails(this.refId, this.id, "rejected", this.userType, 'Y');
          this.loadAllPosidexDetails(this.id);
          this.globalData.showAlert('Alert', 'You are not eligible application rejected');
        } else {
          this.sqliteProvider.updatePosidexCBSDetails(this.refId, this.id, (cbsData.errorDesc) ? "Inititated" : "Approved", this.userType, 'Y');
          this.loadAllPosidexDetails(this.id);
          if (cbsData.errorDesc == "Success") {
            this.globFunc.showAlert('Alert', 'CBS request initiated. Please try again!');
          } else {
            this.globalData.showAlert('Alert', 'Proceed further').then(data => {
              this.proceedNextPage();
            });
          }
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', (cbsData.errorDesc) ? cbsData.errorDesc : "No response");
      }
      ...
      this.sqliteProvider.getPosidexDetails(this.refId, 'C').then(coApp => {
        if (coApp.length == this.coapplicantsLength) {
          this.sqliteSupport.getPosidexData(this.refId).then(data => {
            appStatus = data.every(data => (data.cbsCheck == 'Y' && data.cbsStatus == 'Approved') || (data.amlCheck == 'Y' && data.amlStatus == 'No Match' || data.amlStatus == 'AML Approved'))
            if (appStatus) {
              this.globalData.proccedOk("Alert", "Proceed to Credit check!").then(data => {
                if (data == "yes") {
                  this.router.navigate(['/ExistApplicationsPage'], { queryParams: { submitData: JSON.stringify(this.userInfo) }, skipLocationChange: true, replaceUrl: true })
                }
              });
      ...
      this.sqliteProvider.getPosidexDetails(this.refId, 'A').then(data => {
        if (data.length > 0) {
          ...
          if (appStatus) {
            this.globalData.proccedOk("Alert", "Proceed to Credit check!").then(data => {
              if (data == "yes") {
                this.router.navigate(['/ExistApplicationsPage'], { queryParams: { submitData: this.userInfo }, skipLocationChange: true, replaceUrl: true })
              }
            });
          }
        }
  ...
  //       this.globFunc.showAlert("Alert", "The AML Status is Match, so please wait untill Processing!");
  //     }else if(this.amlResp == 'Success'){
  //       this.globFunc.showAlert("Alert", "AML request initiated. Please try again!");
  //     } else if (this.amlResp == 'AML Rejected') {
  //       this.globFunc.showAlert("Alert", "The AML Status is AML Rejected, so the Application has been Rejected!");
  //     }
  //   } else {
  //     this.globFunc.showAlert("Alert", "The AML Status is Aml Initated Can't Proceed Further!");
  //   }
  // }
  ...
  view360(item) {
    ...
    this.master.restApiCallAngular('sfdcCust360', body).then(async res => {
      let res1 = {
        "sfdc360Res": {
      ...
      if (cfdsData.errorCode === '000') {
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert("Alert", cfdsData.errorDesc);
      }
    }, (err) => {
      if (err) {
        this.globFunc.showAlert('Alert', err.message);
  ...
  existChange(event) {
    ...
    if (index && index !== 'Y') {
      ...
      this.globFunc.showAlert('Alert', 'event value are empty..!')
    }
  }
