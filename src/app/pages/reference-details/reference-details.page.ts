...
export class ReferenceDetailsPage implements OnInit {
  ...
  constructor(public navCtrl: NavController, 
    ...
    public globalData: DataPassingProviderService, public modalCtrl: ModalController,
    ...
    this.referenceDetails = this.formBuilder.group({
      refName: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      mobileNo: ['', Validators.compose([Validators.maxLength(10), Validators.pattern('[0-9]{10}$'), Validators.required])],
  ...
  async getRelationShip() {
    let data = await this.sqliteProvider.getMasterDataUsingType('VLRelationShip')
  ...
  referenceDetailsSave(value) {
    this.sqlsupport.insertReferenceDetails(this.refId,this.id,this.userType,value,this.detailId).then(data=>{
      if(this.detailId){
        this.globalData.showAlert('Alert','Reference details updated successfully');
      }else{
        this.globalData.showAlert('Alert','Reference details saved successfully');
      }
  ...
  async viewReferenceDetails(){
    if(this.referenceDetailsData.length>0){
      let modal = await this.modalCtrl.create({
        ...
        componentProps: { viewRefDetails: this.referenceDetailsData, submitDisable: this.submitDisable }
      });
      modal.onDidDismiss().then(async(refValue: any) => {
        ...
        if(refData){
          ...
          this.referenceDetails.get('relationship').setValue(refData.relationship);
        }else{
          this.referenceDetails.reset();
        }
        this.getReferenceDetailsFromDB();
