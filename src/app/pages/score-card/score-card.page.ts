...
export class ScoreCardPage implements OnInit {
  ...
  async getDetails() {
    await this.sqliteProvider.getScoreCard(this.refId).then(data => {
      if (data.length > 0) {
    ...
    this.sqliteProvider.getVehicleScoreMaster().then(data => {
  ...
  ManualApprovalCall() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (fieldInvesRes.ErrorCode === '000') {
        ...
        this.sqlSupport.updateManualApprovalForSubmit(this.applicantDetails[0].applicationNumber);
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', fieldInvesRes.ErrorDesc);
      }
      ...
      if (scoreResponse.errorCode === '000') {
        if (this.existVehicleType == '2') {
          if (scoreResponse.SRFLAG == 'N' && scoreResponse.STPFLAG == 'Y') {
            if ((+scoreResponse.HimarkScore >= this.commonHimarkvalue) && (+this.reqLoanAmount < this.commonLoanAmount) && (this.houseOwned == '1')) {
          ...
          if (scoreResponse.SRFLAG == 'N') {
            let chargesFormatValue = scoreResponse.eligbleAmout.toString().split(".");
            ...
            this.sqlSupport.updateEligibleAmout(this.eligibleAmt, this.applicantDetails[0].applicationNumber);

            if (scoreResponse.NTCFLAG == 'Y') { // NTC
              if (this.reqLoanAmount < 100000) {
                ...
                if (scoreResponse.STPFLAG == 'Y' || scoreResponse.FIFLAG == 'Y') {
                  // go to FI
                  if (scoreResponse.FIENTRY == 'N') {
                    this.fiFlageEnble(scoreResponse)
                  } else {
              ...
              } else if (this.reqLoanAmount > 150000 && this.scheme !== '1052') {
                if (scoreResponse.STPFLAG == 'Y' || scoreResponse.FIFLAG == 'Y') {
                  // go to FI
                  if (scoreResponse.FIENTRY == 'N') {
                    this.fiFlageEnble(scoreResponse)
  ...
  reSubmitForautoApproval() {
    if (this.reqLoanAmount <= 150000) {
      this.autoApproval();
      this.reSubmitDone = 1;
      this.router.navigate(['/PostSanctionPage'], { queryParams: { viewData: this.userInfo }, skipLocationChange: true, replaceUrl: true })
    } else {
  ...
  autoApprovalFlageEnable(scoreResponse: any) {
    ...
    if (this.scoreId == undefined || null) {
      this.sqliteProvider.addScoreCard(this.refId, 'Y', scoreResponse.lookalikeScore || "", scoreResponse.ltvScore || "0", JSON.stringify(this.questionRes), scoreResponse.AutoApprovalFlag, scoreResponse.FIFLAG, scoreResponse.SRFLAG, scoreResponse.STPFLAG, scoreResponse.NTCFLAG).then(data => {
        this.scoreId = data["insertId"];
        this.getDetails();
      })
    } else {
      this.sqliteProvider.updateScoreCardinPostsanction('Y', this.refId, this.scoreId).then(data => {
    ...
    this.isAutoApprovalFlag = true;
    this.globFunc.globalLodingDismiss();
    this.globFunc.showAlert('Your Score', "is Eligible");
  }
  ...
  fiFlageEnble(scoreResponse: any) {
    ...
    this.isAutoApprovalFlag = false;
    this.globFunc.globalLodingDismiss();
    this.globFunc.showAlert('Your Request', "Move to Field Investigation");
  }
  ...
  fieldInvestigation() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (fieldInsRes.ErrorCode === '000') {
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', fieldInsRes.ErrorDesc);
      }
    }, err => {
      this.globFunc.globalLodingDismiss();
      if (err.name == "TimeoutError") {
  ...
  autoApproval() {
    ...
    //   this.globFunc.showAlert('Alert', 'Network Error')
    // }, 2000);
    let data = {
    ...
    this.master.restApiCallAngular('autoapproval', data).then(res => {
      ...
      if (appData.errorCode === '000') {
        ...
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', (appData.errorDesc) ? appData.errorDesc : appData.status);
      }
    })
  ...
  loadAllApplicantDetails() {
    this.sqliteProvider.getApplicantDataAfterSubmit(this.refId).then(async data => {
      if (data.length > 0) {
        ...
        // }
        this.sqliteProvider.getApplicantDataforPostSanction(this.userInfo.refId).then(appdata => {
          if (appdata.length > 0) {
  ...
  postSubmission() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (postSanctionRes.ErrorCode === '000') {
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', postSanctionRes.ErrorDesc)
        this.sqlsupport.updatePostSanction(this.appStatId).then(data => {
          this.getDetails();
          this.router.navigate(['/ExistApplicationsPage'], { skipLocationChange: true, replaceUrl: true });
        }).catch(err => err);
  ...
  meterLoader() {
    ...
    var gauge = function (container, configuration) {
      ...
      function render(newValue) {
        ...
        var colors = ['rgb(224, 11, 11)', 'rgb(224, 90, 11)', 'rgb(255,165,0)', 'rgb(91,141,0)', 'rgb(0,100,0)'];
    ...
    return new Promise((resolve, reject) => {
      this.sqliteProvider.getBasicDetails(this.refId, this.id).then(data => {
        ...
        if (data.length > 0) {
          ...
          if (data[0].tenure !== '' && data[0].intRate !== '' && sanctionedAmt) {
  ...
  async getPddCharges(prdCode) {
    try {
      ...
      this.globFunc.globalLodingDismiss();
      this.globFunc.showAlert('Alert', JSON.stringify(error));
    }
  }
      ...
      if (loanAmt) {
        ...
        if (processFeesAmt.length > 0) {
          ...
          if (+proceesPercentage <= +processFeesAmt[0].minProcessingFee) {
            ...
            this.applicantDetails[0].processingFee = +proceesPercentage.toFixed(2);
          }
          this.gstCallForPF();
          // } else {
    ...
    try {
      this.sqliteProvider.getBasicDetails(this.refId, this.id).then(data => {
        ...
        if (data.length > 0) {
          let sanctionedAmt;
          if (data[0].preEmiDB == "1") {
            sanctionedAmt = +this.applicantDetails[0].loanAmount + +this.applicantDetails[0].insPremium;
          } else {
            sanctionedAmt = +this.applicantDetails[0].loanAmount + +this.applicantDetails[0].insPremium + preEmi;
  ...
  downPaymentCalc() {
    try {
      ...
      if (this.applicantDetails[0].preEmiDB == "1") {
        ...
        charges = stampDuty + gstonSdc + nachCharges + gstonNach + pddCharges + gstonPddCharges + otherCharges + gstonOtherCharges + borHealthIns + coBorHealthIns + preEmi;
  ...
  loanFacilitiesServiceCall() {
    ...
    this.master.restApiCallAngular('LoanDetails', body).then(res => {
      ...
      if (loanFacilityRes.ErrorCode === '000') {
        this.globFunc.globalLodingDismiss();
        this.globFunc.showAlert('Alert', (this.autoApprovalMsg.errorDesc) ? this.autoApprovalMsg.errorDesc : this.autoApprovalMsg.status);
        ...
        this.sqlSupport.updateSanctionedAmount(this.sanctionedAmt, this.emi, this.applicantDetails[0].applicationNumber);
        this.sqlsupport.updateAutoApproval(this.appStatId).then(data => {
          this.getDetails();
        }).catch(err => err);
