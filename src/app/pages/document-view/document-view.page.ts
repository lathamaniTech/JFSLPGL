...
export class DocumentViewPage  {
  ...
  getProofImg() {
    for (let i = 0; i < this.documents.length; i++) {
      this.sqliteProvider.getpromoterproofImages(this.documents[i].pproofId).then(imgData => {
        if (0 < imgData.length) {
          this.documents.filter(imgdoc => {
  ...
  updateimg(upid, imgData,proofName) {
    this.sqliteProvider.removepromoterproofImages(upid).then(data => {
      for (let i = 0; i < imgData.length; i++) {
        this.sqliteProvider.addpromoterproofImages(this.refId, this.id, imgData[i].imgpath, upid).then(data => {
        }).catch(Error => {
