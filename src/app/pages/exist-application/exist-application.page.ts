...
import { AlertController, IonItemSliding, LoadingController, ModalController, NavParams, Platform } from '@ionic/angular';
...
export class ExistApplicationPage {
    ...
    constructor(public navCtrl: Router, public navParams: NavParams, 
      public modalCtrl: ModalController, public sqliteProvider: SqliteService, 
      public globalData: DataPassingProviderService, public alertCtrl: AlertController, 
      public loadCtrl: LoadingController, public globFunc: GlobalService, 
      public http: HTTP, public network: Network, 
      public platform: Platform, public master: RestService,
    ...
    docsUpload(item) {
      ...
      this.navCtrl.navigate(['/OtherDocsPage'], { queryParams :{application: item.applicationNumber },skipLocationChange: true, replaceUrl: true});
    }
    ...
    viewDetails(item) {
      ...
      this.navCtrl.navigate(['/ViewDetailsPage'], { queryParams : { refvalue: JSON.stringify(item), userVal: JSON.stringify(item), user_Type: 'G' },skipLocationChange: true, replaceUrl: true});
          ...
          this.sqliteProvider.getPersonalEntityDetails(item.refId, item.id).then(data => {
            if (data.length > 0) {
              if (data[0].URNnumber != "" && data[0].URNnumber != null && data[0].URNnumber != undefined) {
                ...
                this.navCtrl.navigate(['/NewapplicationPage'], {queryParams : { appRefValue: JSON.stringify(item), usertype: "A", fieldDisable: true },skipLocationChange: true, replaceUrl: true});
              } else {
                this.globalData.setborrowerType(item.userType);
                this.globalData.setrefId(item.refId);
                this.globalData.setId(item.id);
    ...
    passGuaDetails(item) {
      this.sqliteProvider.getguappDetails(item.refId, 'G').then(data => {
        ...
        this.globalData.setProfileImage(guaran[0].profPic);
          this.navCtrl.navigate(['/NewapplicationPage'], {queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true },skipLocationChange: true, replaceUrl: true});
      });
    }
    ...
    checkPosidex(item) {
      this.navCtrl.navigate(['/PosidexCheckPage'], { queryParams : {userType: JSON.stringify(this.userType), GrefId: JSON.stringify(this.refId), fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
    }
  
    creditCheck(item) {
      let coAppStatus = '';
      console.log(item, "creditcheck")
      this.sqliteProvider.getPosidexDetails(item.refId, item.userType).then(data => {
        if (data.length > 0) {
          if ((data[0].cbsCheck == 'Y' && data[0].cbsStatus == 'Approved') || (data[0].amlCheck == 'Y' && data[0].amlStatus == 'No Match' || data[0].amlStatus == 'AML Approved')) {
            if (item.coAppFlag == 'Y') {
              this.sqliteProvider.getPosidexDetails(item.refId, 'C').then(coApp => {
                if (coApp.length > 0) {
                  if (coApp.every(data => (data.cbsCheck == 'Y'&& data.cbsStatus == 'Approved') || (data.amlCheck == 'Y' && data.amlStatus == 'No Match' || data.amlStatus == 'AML Approved'))) {
                    this.navCtrl.navigate(['/CreditCheckPage'], { queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
                  } else {
                    let coAppAlert = coApp.filter(data => (data.cbsCheck == 'Y'&& data.cbsStatus != 'Approved') || (data.amlCheck == 'Y' && data.amlStatus != 'No Match' || data.amlStatus != 'AML Approved'));
                    this.globFunc.showAlert("Alert",(coAppAlert[0].amlCheck == 'Y' || coAppAlert[0].cbsCheck == 'Y') ? `${coAppAlert[0].amlCheck == 'Y' ? 'AML' : 'CBS'} status -Co-Applicant: ${coAppAlert[0].amlCheck == 'Y' ? coAppAlert[0].amlStatus : (coAppAlert[0].cbsStatus) ? coAppAlert[0].cbsStatus:'Initiated' }. Cannot proceed further`:'Please complete Co-Applicant AML/CBS check');
                  }
                  ...
                  this.globFunc.showAlert("Alert", "Please complete Co-Applicant Posidex check");
                }
              })
            } else {
              this.navCtrl.navigate(['/CreditCheckPage'], { queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
            ...
            this.globFunc.showAlert("Alert", (data[0].amlCheck == 'Y' || data[0].cbsCheck == 'Y') ? `${data[0].amlCheck == 'Y' ? 'AML' : 'CBS'} status - Applicant: ${data[0].amlCheck == 'Y' ? data[0].amlStatus : (data[0].cbsStatus)?data[0].cbsStatus:"Initiated" }. Cannot proceed further`: 'Please complete Applicant AML/CBS check');
          }
        } else {
          this.globFunc.showAlert("Alert", "Please complete Applicant Posidex check");
        }
      });
    }
    ...
    scoreCard(item) {
      let coAppStatus = '';
      this.sqliteProvider.getSubmitDetails(item.refId, item.id).then(creditCheckData => {
        if (creditCheckData.length && creditCheckData[0].creditEligibility == "eligible") {
          this.globFunc.setScoreCardChecked(item.refId);
          this.navCtrl.navigate(['/ScoreCardPage'], { queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
          // if (item.coAppFlag == 'Y') {
          ...
          this.globFunc.showAlert("Alert", "Please Complete Applicant Credit Check");
        }
      })
    }
    ...
    additional(item) {
      ...
      this.globalData.setrefId(item.refId);
      this.globalData.setId(item.id);
      this.navCtrl.navigate(['/AdditionalDetailsPage'], { queryParams : { fieldDisable: true },skipLocationChange: true, replaceUrl: true});
    }
    ...
    loadAllApplicantDetails() {
      this.sqliteProvider.getSubmittedApplications(this.username).then(data => {
        ...
        for (let i = 0; i < this.items.length; i++) {
          ...
          this.items[i].inputDate = newDate == "Invalid Date" ? this.items[i].createdDate : this.convertDate(this.items[i].createdDate);//this.convertDate(this.items[i].createdDate)
        }
    ...
    appstatus(item) {
      ...
      this.master.restApiCallAngular('statusCheck', body).then(res => {
        // console.log((<any>res));
        if ((<any>res).errorCode == '000') {
          this.status = (<any>res).status + ": " + (<any>res).flowPoint;
          this.sqliteProvider.updateSubmitDetails(item.cibilCheckStat, item.submitStat, item.applicationNumber, (<any>res).status + ": " + (<any>res).flowPoint, item.cibilColor, item.cibilScore, item.statId);
          // this.navCtrl.setRoot(this.navCtrl.getActive().component);
          this.globalData.globalLodingDismiss();
          this.showStatusAlert();
        } else {
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert!", (<any>res).errorDesc);
        }
      }, err => {
        this.globalData.globalLodingDismiss();
    ...
    filterItems(event: any) {
        ...
        if (searchTerm && searchTerm !== '') {
          this.items = this.items.filter((item) => {
            return (
              (item.applicationNumber && item.applicationNumber.toString().toLowerCase().indexOf(searchTerm.toLowerCase()) > -1) ||
    ...
    IMD(item) {
      ...
      this.globalData.setrefId(item.refId);
      this.globalData.setId(item.id);
      this.navCtrl.navigate(['/ImddetailsPage'], { queryParams : { fieldDisable: true },skipLocationChange: true, replaceUrl: true});
    }
    ...
    CASA(item) {
      ...
      this.navCtrl.navigate(['/TabsPage'], { queryParams : { userVal: item, fieldDisable: true },skipLocationChange: true, replaceUrl: true});
    }
    ...
    referenceDetails(item){
      this.globalData.setborrowerType(item.userType);
          this.globalData.setrefId(item.refId);
          this.globalData.setId(item.id);
          this.navCtrl.navigate(['/ReferenceDetailsPage'], { queryParams : { fieldDisable: true },skipLocationChange: true, replaceUrl: true});
    }
    ...
    nach(item) {
      ...
      //   if (data.length > 0) {
          ...
          this.globalData.setrefId(item.refId);
          this.globalData.setId(item.id);
          this.navCtrl.navigate(['/NachPage'], { queryParams : { fieldDisable: true },skipLocationChange: true, replaceUrl: true});
      //   } else {
    ...
    vehicleDetails(item) {
      ...
      this.globalData.setCoAppFlag(item.coAppFlag);
      localStorage.setItem("submit", "true");
      this.navCtrl.navigate(['/AssetTabsPage'], { queryParams : { userVal: item, fieldDisable: true },skipLocationChange: true, replaceUrl: true });
    }
    ...
    fieldInspect(item) {
      this.navCtrl.navigate(['/FieldInspectionPage'], { queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
    }
    vehicleValuation() {
      this.navCtrl.navigate(['/VehicleValuationPage'],{skipLocationChange: true, replaceUrl: true});
    }
  
    PDDSubmit(item) {
      if (item.postSancDocUpload == 'Y') {
        this.navCtrl.navigate(['/PddSubmissionPage'], { queryParams : { userType: this.userType, GrefId: this.refId, fieldDisable: true, submitData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
      } else {
        this.globFunc.showAlert("Alert", "Please complete Post sanction");
      }
  
    }
  
    viewDocuments(item) {
      this.navCtrl.navigate(['/DocumentUploadPage'], { queryParams :{ viewData: item, submitStatus: true },skipLocationChange: true, replaceUrl: true });
    }
  
    postSanction(item) {
      this.navCtrl.navigate(['/PostSanctionPage'], { queryParams : { viewData: JSON.stringify(item) },skipLocationChange: true, replaceUrl: true});
    }
    ...
    checkManualApprovalStatus(item) {
      ...
      this.master.restApiCallAngular('ApprovalStatus', body).then(res => {
        // console.log((<any>res));
        if ((<any>res).statusCode == '000') {
          this.globalData.globalLodingDismiss();
          if ((<any>res).statusDescription.toLowerCase() == "Approved".toLowerCase()) {
            this.sqlSupport.updateForPostSanction(item.applicationNumber, 1, 'N').then(data => {
              this.globalData.showAlert("Alert!", "Approved... Proceed to post sanction");
              this.loadAllApplicantDetails();
            })
          } else {
            this.globalData.globalLodingDismiss();
            this.globalData.showAlert("Alert!", (<any>res).statusDescription);
          }
        } else {
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert!", (<any>res).statusDescription);
