...
export class PddSubmissionPage implements OnInit {
  ...
  constructor(public router: Router, public modalCtrl: ModalController,
    ...
    this.PddDetails = this.formBuilder.group({
      ...
      remarks: ["", Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z ]*')])]
    })
    ...
    this.userInfo = JSON.parse(this.route.snapshot.queryParamMap.get('submitData'));
    ...
    this.sqlite.getPddDetailsFromDd(this.refId, this.id).then(data => {
      if (data.length > 0) {
        ...
        this.submittedData.forEach(element => {
          this.pddDetailsArray.push({ pddDocId: element.pddDocId, refId: element.refId, id: element.id, docType: element.docType, docDescription: element.docDescription, remarks: element.remarks, uploadFlag: element.uploadFlag, AllImages: '' });
  ...
  PddDetailsSave(value) {
    if (value.docType) {
      ...
      let docmentExists = this.pddDetailsArray.filter(data => data.docType == value.docType);

      if (docmentExists.length != 0) {
        this.globalData.showAlert("Alert!", "This Documents Added");
      } else {
        this.docValue = value.docType
        let docDescp = this.Document_Type.filter(data => data.docId == this.docValue)
        ...
        //   this.globalData.showAlert("Alert!", "Pdd Documents Added Successfully");
        // }).catch(err => {
        //   console.log(err);
        // })
      ...
      this.globalData.showAlert("Alert!", "Please Select Document Type");
    }
  }


  getPddDetailsArray(docValue) {
    this.sqlite.getPddDetailsSave(this.refId, this.id, docValue).then(data => {
      this.submittedData = data;
      this.submittedData.forEach(element => {
        this.pddDetailsArray.push({ pddDocId: element.pddDocId, refId: element.refId, id: element.id, docType: element.docType, docDescription: element.docDescription, remarks: element.remarks, uploadFlag: element.uploadFlag, AllImages: [] });
        if (element.uploadFlag == 'N') {
  ...
  async addPddImages(doc) {
    ...
    const modal = await this.modalCtrl.create({
      ...
      componentProps: { pddpics: newpddArray, submitstatus: this.submitDisable }
    });
  ...
  deletePddImages(doc) {
    this.sqlite.removePddDocImages(this.refId, this.id, doc).then(data => {
      console.log(data);
      if (data.length > 0) {
  ...
  uploadPddImages(doc) {
    if (this.pddDocImgs.length > 0) {
      for (let i = 0; i < this.pddDocImgs.length; i++) {
        this.sqlite.addPddDocImages(this.refId, this.id, doc, this.pddDocImgs[i].imgpath).then(data => {
        }).catch(Error => {
      ...
      this.pddDocImgs = [];
      this.pddDocImglen = 0;
      this.globalData.showAlert("Alert!", "Pdd Images Added Successfully");
    } else {
      this.globalData.showAlert("Alert!", "No Images Added");
    }
  }
  getPddImages(doc) {
    this.sqlite.getPddDocImages(this.refId, this.id, doc).then(docimg => {
      this.pddDocImgs = [];
      if (docimg.length > 0) {
        for (let i = 0; i < docimg.length; i++) {
          this.pddDocImgs.push({ "imgpath": docimg[i].pddDocImgs, "docType": docimg[i].docType });
        }
  ...
  async deleteDoc(docVal) {
    let alert = await this.alertCtrl.create({
      ...
      },
      {
        text: 'yes',
        handler: () => {
          this.sqlite.deletePddSavedDetailsFromDb(this.refId, this.id, docVal).then(data => {
            console.log(data);
            this.pddDocImgs = [];
            this.PddDetails.reset();
            this.getPddDetailsValue();
  ...
  PddDocUploadImgs() {
    this.sqlite.getPddUploadImgFromDb(this.refId, this.id).then(async data => {
      if (data.length > 0) {
    ...
    if (this.pddDocImgs.length > 0) {
      ...
      this.sqlite.getPddDetailsFromDd(this.refId, this.id).then(data => {
        if (data.length > 0) {
          ...
          this.master.restApiCallAngular('pdddocupload', body).then(res => {
            let pddDocRes = <any>res;
            if (pddDocRes.errorCode === '000') {
              this.globalFunction.globalLodingDismiss();
              this.globalData.showAlert('Alert', 'Pdd Documents Uploaded Successfully')
              this.pddDetailsArray.forEach(element => {
                this.sqlite.updatePddUpload(element.refId, element.id, 'Y').then(data => {
                  console.log(data);
                }).catch(err => {
                  console.log(err);
                });
              });
              this.sqliteSupport.updatePDDdocSuccessStatus(this.refId, this.id).then(data => {
                this.showFinalBtn = true;
              });
              // this.deleteAllData();
              this.uploadBtn = true;
              ...
              this.globalFunction.globalLodingDismiss();
              this.globalData.showAlert('Alert', 'Pdd Documents Image Should not be Empty');
              this.uploadBtn = false;
            } else {
              this.globalFunction.globalLodingDismiss();
              this.globalFunction.showAlert('Alert', pddDocRes.errorStatus);
              this.uploadBtn = false;
            }
          }).catch(err => {
            console.log(err);
      ...
      this.globalData.showAlert("Alert!", "Please Add Pdd Document Images");
    }
  }
  ...
  finalWorkFlow() {
    this.globalFunction.globalLodingPresent('Submitting for disbursed account Please wait...');
    let body = {
  ...
  capureImg(srcType) {
    ...
    Camera.getPhoto(options).then(async (img: any) => {
      ...
      this.globalData.showAlert("Document Image", "Document Not Added");
    })
  }

  async viewPddImages(doc) {
    console.log("doc", doc)
    let newpddArray = this.pddDetailsArray.filter(data => data.docType == doc);
    console.log("newpddArray", newpddArray)
  ...
  uploadService(data, pdffiles?) {
    ...
    let docImg = img.replace('data:image/*;charset=utf-8;base64,','data:image/jpeg;base64,/9j/');
    let fileType = data[0].fileType;
    let fileName: any;
    let type: any;
    let timestamp = new Date().getTime().toString();

    if (fileType == ".jpg" || fileType == ".jpeg" || fileType == ".png" || fileType == "image") {
  ...
  onFileSelect(event) {
    ...
    if (fileType == "application/pdf" || fileType == "application/msword" || fileType == "application/vnd.ms-excel" || fileType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" || fileType == "image/png" || fileType == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || fileType == "image/jpeg" || fileType == "text/plain") {
      ...
      this.globalData.showAlert('Alert', 'This Type Of File Is Not Supported')
      ...
      if (pddDocRes.errorCode === '000') {
        for (let i = 0; i < this.pddDetailsArray.length; i++) {
          if (this.pddDetailsArray[i].docType == file.DocId) {
            docDetails.imgpath = docDetails.imgpath.replace('data:image/jpeg;base64,/9j/','data:image/*;charset=utf-8;base64,');
            this.pddDetailsArray[i].AllImages.push(docDetails)
          }
        }
        console.log("pddDetailsArray", this.pddDetailsArray)
        ...
        this.globalFunction.globalLodingDismiss();
        this.globalData.showAlert('Alert', JSON.stringify(pddDocRes));
      } else {
        this.globalFunction.globalLodingDismiss();
        this.globalFunction.showAlert('Alert', pddDocRes.errorStatus);
      }
  ...
  onchangeDocType(value) {
    ...
    if (this.PddData.invalid) {
      ...
      this.globalFunction.showAlert('Alert', "Please Fill the Form");
    }
  }
  ...
  async fetchDetailsFromRC() {
    try {
      ...
      await this.orpApi.getRCDetails(this.PddData.get('rcNo').value).then((data: any) => {
        console.log('getRCDetails', data);
        this.globalData.globalLodingDismiss();
        if (data) {
          if (!data.blacklisted) {
            this.PddData.get('engineNo').setValue(data.engine_number ? data.engine_number : 'NA');
            this.PddData.get('chassisno').setValue(data.chassis_number ? data.chassis_number : 'NA');
            this.PddData.get('nameAsPerRC').setValue(data.vehicle_model ? data.vehicle_model : 'NA');
            this.PddData.get('hypothecationName').setValue(data.financing_authority ? data.financing_authority : 'NA');
            this.PddData.get('hypothecation').setValue(data.financing_authority ? 'YES' : 'NO');
            data.financing_authority ? this.hypothecation = true : this.hypothecation = false
            // this.RCFetchDetailsDisable = true
          } else {
            this.globalData.showAlert('Alert', 'Given RC Number is Black Listed!');
            this.reSetRCDetails();
            this.RCFetchDetailsDisable = false;
          }
        } else {
          this.globalData.showAlert('Alert', 'Details not found!');
          this.reSetRCDetails();
          this.RCFetchDetailsDisable = false;
        }
      })
    } catch (error) {
      this.sqliteSupport.addAuditTrail(new Date().getTime(), 'fetchDetailsFromRC', '', error);
      this.RCFetchDetailsDisable = false;
    }
  }
