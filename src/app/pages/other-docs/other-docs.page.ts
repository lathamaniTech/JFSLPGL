...
export class OtherDocsPage implements OnInit {
  ...
  docFinalReq = [];
  titles = [{ CODE: "01", NAME: "RC Book" }, { CODE: "02", NAME: "Insurances" },];
  attachPic: any;
  ...
  constructor(public navCtrl: NavController,
    ...
    this.OtherDocData = this.formBuilder.group({
      ...
      docDescription: ['', Validators.compose([Validators.maxLength(30), Validators.required])]
    })
  }
  ...
  otherDocSave(value) {
    if (this.attchImg) {
      let data = [this.refId, this.id, value.docType, value.docDescription, this.attachPic];
      ...
      this.globalData.showAlert("Alert!", "Please attach Image!");
    }
  }
    ...
    let alert = await this.alertCtrl.create({
      ...
      },
      {
        ...
        handler: () => {
          ...
          this.sqliteProvider.deleteOtherDocs(ids).then(data => {
            console.log(data);
            this.OtherDocData.reset();
            this.getNonSubmitDocDetails();
          }).catch(err => {
  ...
  showData(i) {
    ...
    this.OtherDocData = this.formBuilder.group({
      ...
      docDescription: [this.docsData[i].docDescription, Validators.compose([Validators.maxLength(30), Validators.required])]
    })
  ...
  submitOtherDocs() {
    if (this.docsData.length > 0) {
      if (this.network.type == 'none' || this.network.type == "unknown") {
        ...
        this.id = this.globalData.getId();
        let ids = { refId: this.refId, id: this.id };
        this.sqliteProvider.checkNonSubmitedOtherDocs(ids).then(totaldata => {
      ...
      this.globalData.showAlert("Alert!", "Please Add At least One Document to Submit!");
    }
  ...
  //             this.getNonSubmitDocDetails();
  //             this.globalData.globalLodingDismiss();
  //             this.globalData.showAlert("Alert!", "Given other documents submited successfully.");
  //           })
  //         } else {
  //           this.globalData.globalLodingDismiss();
  ...
  showSubmitData(i) {
    ...
    this.OtherDocData = this.formBuilder.group({
      ...
      docDescription: [this.submitedData[i].docDescription, Validators.compose([Validators.maxLength(20), Validators.required])]
    })
  ...
  //     })
  //   }, (err) => {
  //     this.globalData.showAlert("Document Image", "Document Not Uploaded");
  //   })
  // }
    ...
    if (this.submitType === 'submited') {
      ...
      Profilemodal.onDidDismiss().then((data: any) => {
        if (data == 'updateProfileIMAGE') {
          if (this.submitType === 'submited') {
            this.globalData.showAlert("Alert!", "Document already submitted!")
          } else {
            this.capureImg();
          }
        }
      ...
      if (this.OtherDocData.value['docType'] === '' || this.OtherDocData.value['docType'] === null || this.OtherDocData.value['docType'] === undefined) {
        this.globalData.showAlert("Alert!", "Must Select the Document Type value.");
      } else if (this.OtherDocData.value['docDescription'] === '' || this.OtherDocData.value['docDescription'] === null || this.OtherDocData.value['docDescription'] === undefined) {
        this.globalData.showAlert("Alert!", "Must Enter the Document Description value.");
