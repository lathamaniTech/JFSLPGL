...
export class ModalPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    public globalData: DataPassingProviderService, private modalCtrl: ModalController,
  ...
  async deleteData(proof) {
    if (this.fieldDisable) {
      this.globalData.showAlert("Alert!", "Unable to Delete Document.")
    }
    else {
        ...
        },
        {
          ...
          handler: () => {
            ...
            if (this.navParams.get("ID_proof") == "ProofPromoter") {
              this.sqliteProvider.getKarzaData(this.leadId, this.docNames[proof.proofName]).then(secKycCheck => {
                if (secKycCheck.length > 0) {
                  if (secKycCheck[0].karzaCheck == 'Y') {
                    this.sqliteProvider.removeProofData({ name: 'ProofPromoter', refId: proof.refId, id: proof.id, proofid: proof.pproofId }).then(_ => {
                      this.sqliteProvider.removeSecondKycData(this.leadId, this.docNames[proof.proofName]);
                      this.getIdProofDetails();
                      this.status = "proofDelete";
                    });
                  } else {
                    this.globalData.showAlert("Alert!", "We couldn't able to delete first kyc validation!");
                  }
                } else {
                  this.sqliteProvider.getPromoterProofDetails(proof.refId, proof.id).then((kycLength) => {
              ...
              this.sqliteProvider.removeProofData({ name: 'ProofEntity', refId: proof.refId, id: proof.id, proofid: proof.eproofId }).then(_ => {
