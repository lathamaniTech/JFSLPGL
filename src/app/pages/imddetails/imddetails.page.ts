...
export class ImddetailsPage implements OnInit {
  ...
  constructor(public router: Router,
    ...
    this.imdDetails = this.formBuilder.group({
      ...
      imdInstrument: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z0-9 ]*'), Validators.required])],
      imdACName: ['', Validators.compose([Validators.maxLength(30), Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      imdACNumber: ['', Validators.compose([Validators.maxLength(18), Validators.pattern('[0-9]*'), Validators.required])],
      imdAmount: ['', Validators.compose([Validators.maxLength(10), Validators.pattern('[0-9]*'), Validators.required])],
  ...
  imdDetailsSave(value) {
    if (this.imdImgs.length > 0) {
      this.sqlsupport.InsertIMDDetails(this.refId, this.id, this.userType, value, this.imdId).then(data => {
        if (this.imdId == "" || this.imdId == undefined || this.imdId == null) {
          this.imdId = data.insertId;
          this.uploadImdImg();
          this.globalData.showAlert("Alert!", "IMD Details Added Successfully");
        } else {
          this.updateImdImg(this.imdId);
          this.globalData.showAlert("Alert!", "IMD Details Updated Successfully");
        }
      })
    } else {
  ...
  getImdDetails() {
    this.sqlsupport.getImdDetails(this.refId, this.id).then(data => {
      if (data.length > 0) {
        ...
        this.imdDetails.controls.imdBName.setValue(JSON.parse(data[0].imdBName));
  ...
  async showpicmaodal(value) {
    if (value == "imdcheque") {
      ...
      const emodal = await this.modalCtrl.create({
        ...
        componentProps: { imdpics: this.imdImgs, submitstatus: this.submitDisable }
      });
  ...
  uploadImdImg() {
    for (let i = 0; i < this.imdImgs.length; i++) {
      this.sqlsupport.addImdImages(this.refId, this.id, this.imdImgs[i].imgpath, this.imdId).then(data => {
  ...
  updateImdImg(imdId) {
    //alert(imdId);
    this.sqlsupport.removeImdImages(imdId).then(data => {
      for (let i = 0; i < this.imdImgs.length; i++) {
        this.sqlsupport.addImdImages(this.refId, this.id, this.imdImgs[i].imgpath, imdId).then(data => {
          // console.log("promoter image insert" + data);
        }).catch(Error => {
          console.log("Failed!" + Error);
          this.globalData.showAlert("Alert!", "Failed!");
  ...
  getPaymentDetails() {
    this.sqliteProvider.getMasterDataUsingType('PaymentDetails').then(data => {
      this.payments = data;
    });
  }
