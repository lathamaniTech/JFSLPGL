...
export class FieldInspectionPage implements OnInit {
  ...
  constructor(public router: Router, public modalCtrl: ModalController,
    ...
    public route: ActivatedRoute, public master: RestService,
    ...
    // }
    this.fieldInfo = JSON.parse(this.route.snapshot.queryParamMap.get('submitData'));
  ...
  getVehicleWorkflow() {
    this.sqliteProvider.getVehicleApproval('Recommendation & Approval').then(positiveUser => {
      this.positiveUserList = positiveUser.filter(val => val.UserGroupName !== 'BH');
      // const filterBHValue = positiveUser;
      // this.positiveUserList = filterBHValue.filter(val => val.UserGroupName !== 'BH');
    }).then(data => {
      this.sqliteProvider.getVehicleWorkflowList('FI Reject Review').then(negativeUser => {
  ...
  formgroup() {
    this.fieldInspectionDetails = this.formBuilder.group({
      fieldLocation: ['', ([Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      ...
      customerRelationship: ['', ([Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      customerName: ['', ([Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      customerMobileNum: ['', ([Validators.pattern('[0-9]{10}$'), Validators.required])],
      customerAddress: ['', Validators.compose([Validators.pattern(/^[a-zA-Z0-9 ()_\-\[\]{};':"\\.`,<>\/]*$/), Validators.maxLength(40), Validators.minLength(3), Validators.required])],
      neighbourName1: ['', ([Validators.pattern('[a-zA-Z ]*'), Validators.required])],
      neighbour1Check: ['', Validators.required],
      neighbourName2: ['', ([Validators.pattern('[a-zA-Z ]*'), Validators.required])],
  ...
  fieldInspectionDetailsSave(formValue) {
    ...
    if (this.proofImgs.length === 0) {
      this.globalData.showAlert("Alert!", "Please Add Background Image");
      this.globalData.globalLodingDismiss();
    } else {
      this.sqliteProvider.insertFieldInspectionDetails(this.refId, this.id, formValue, this.fieldId, JSON.stringify(this.proofImgs), 'N').then(data => {
        console.log(data);
        if (this.fieldId === "" || this.fieldId === undefined || this.fieldId === null) {
          this.fieldId = data.insertId;
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert", "Field Investigation Values Added Successfully.");
          this.fieldSbt = false;
        } else {
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert", "Field Investigation Values Updated Successfully.");
          this.fieldSbt = false;
        }
      }).catch(err => {
        console.log(err)
    ...
    if (value == "idproof") {
      if (this.proofImglen < 3) {
        const modal = await this.modalCtrl.create({
          ...
          componentProps: { proofpics: this.proofImgs, submitstatus: this.submitDisable, fromFieldInves: true },
          cssClass: '',
          showBackdrop: true,
          animated: true
        })
        ...
        this.globalData.showAlert("Alert!", "Maximum Two Image only allowed.")
      }
    }
  }
  ...
  getAllFieldValues(data) {
    ...
    this.fieldInspectionDetails.get("customerName").setValue(data.firstname + " " + data.lastname);
    this.fieldInspectionDetails.get("customerMobileNum").setValue(this.globalData.basicDec(data.mobNum));
  ...
  getFieldInspectionFormValues() {
    if (!!this.refId) {
      try {
        ...
        this.sqliteProvider.getFieldInspectionDetails(this.refId, this.id).then(data => {
          console.log(data);
          if (data.length > 0) {
            ...
            for (let i = 0; i <= dataKeys.length - 1; i++) {
              if (exceptCol.indexOf(dataKeys[i]) == -1) {
                this.fieldInspectionDetails.controls[dataKeys[i]].setValue(data[0][dataKeys[i]]);
              }
            }
          }
        }, err => {
  ...
  submitFieldIns() {
    this.globalData.globalLodingPresent("Please wait...");
    this.sqliteProvider.getFieldInspectionDetails(this.refId, this.id).then(async imgdata => {
  ...
  uploadFieldInspection() {
    this.sqliteProvider.getFieldInspectionDetails(this.refId, this.id).then(data => {
      ...
      let fieldData = {
        ...
        "Date": data[0].inspectedDate.substring(8, 10) + "/" + data[0].inspectedDate.substring(5, 7) + "/" + data[0].inspectedDate.substring(0, 4),
      ...
      this.master.restApiCallAngular('FieldInvestigation', fieldData).then(res => {
        ...
        if (fieldRes.ErrorCode === '000') {
          ...
          this.fieldSbt = false;
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert('Alert', fieldRes.ErrorDesc ? fieldRes.ErrorDesc : "Field Investigation Submitted Failed!!!")
        } else {
          this.fieldSbt = false;
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert('Alert', fieldRes.ErrorDesc);
        }
      }).catch(err => {
        console.log(err)
      })
  ...
  uploadFieldImages(PropNo) {
    ...
    this.master.restApiCallAngular('UploadDocs', docs_upload).then(res => {
      ...
      if (sanctionRes.errorCode == "00") {
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert('Alert', 'Field Investigation Submitted Successfully')
        this.showField = false;
        this.fieldSbt = true;
        this.sqliteProvider.updateFieldInvestigationFlag(this.refId, this.id, 'Y', 'Y').then(data => {
          console.log(data);
        }).catch(err => {
          console.log(err);
        });
  ...
  onUserChange(ev) {
    if (this.feedback == '1') { // 1 = positive
      this.selectedUser = this.positiveUserList.find(data => data.UserGroup == ev.detail.value);
    } else {
      this.selectedUser = this.negativeUserList.find(data => data.UserGroup == ev.detail.value);
    }
  ...
  fiSubmit() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (fieldInvesRes.ErrorCode === '000') {
        ...
        this.sqliteSupport.updateManualApprovalForSubmit(this.applicantDetails[0].applicationNumber);
        if (this.feedback == '1') {
          this.sqliteSupport.updateManualApproval(this.applicantDetails[0].applicationNumber, 'Y').then(data => {
            this.router.navigate(['/ExistApplicationsPage'], { skipLocationChange: true, replaceUrl: true });
          })
        } else {
          this.globalData.showAlert('Alert', "Field Investigation rejected").then(data => {
