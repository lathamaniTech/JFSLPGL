...
export class NachPage implements OnInit {
  ...
  ionViewDidLoad() {
    ...
    this.router.navigate(['/NachPage'],{skipLocationChange: true, replaceUrl: true})
  }
  ionViewWillLeave() {
  ...
  proceedNextPage() {
    ...
    this.globalData.proccedOk('Alert', 'proceed to existing leads').then(data => {
      if(data == 'yes'){
        this.router.navigate(['/ExistingPage'], {queryParams: {username: this.username, _leadStatus: leadStatus},skipLocationChange: true, replaceUrl: true})
      }
    })
  }
