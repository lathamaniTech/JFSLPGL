...
export class SubmitPage {
  ...
  constructor(public navCtrl: Router, public navParams: NavParams,
    ...
    public http: HTTP, public globalData: DataPassingProviderService, public sqliteProvider: SqliteService,
    public globalFunc: GlobalService, public alertCtrl: AlertController,
    // public base64: Base64, 
    public master: RestService, public sqlSupport: SquliteSupportProviderService
    , public factoryRef: ComponentFactoryResolver, public network: Network, private errorLog: ErrorLogService, public activateRoute: ActivatedRoute) {
  ...
  private childConstructor(value) {
    ...
    this.coapplicants = this.naveParamsValue.coappDetails ? JSON.parse(this.naveParamsValue.coappDetails) : '';
    console.log(this.coapplicants, 'applicant in submit');
    this.guarantor = this.naveParamsValue.guaranDetails ? JSON.parse(this.naveParamsValue.guaranDetails) : '';
  ...
  lengthCheck() {
    this.sqliteProvider.getBasicDetails(this.applicant.refId, this.applicant.id).then(data => {
      this.basicLength = data.length;
    });
    this.sqliteProvider.getSourcingDetails(this.applicant.refId, this.applicant.id).then(data => {
      this.sourcingLength = data.length;
    });
    this.sqliteProvider.getPersonalDetails(this.applicant.refId, this.applicant.id).then(data => {
      this.promoterLength = data.length;
    });
    this.sqliteProvider.getPresentAddress(this.applicant.refId, this.applicant.id).then(data => {
      this.presentAdrsLength = data.length;
    });
    this.sqliteProvider.getPermanentAddress(this.applicant.refId, this.applicant.id).then(data => {
      this.permaAdrsLength = data.length;
    });
    this.sqliteProvider.getPromoterProofDetails(this.applicant.refId, this.applicant.id).then(data => {
      this.promoProofLength = data.length;
    });
    this.sqliteProvider.getVehicleDetails(this.applicant.refId, this.applicant.id).then(data => {
      this.vehicleDetailsLength = data.length;
    });

    if (this.applicant.guaFlag == 'Y') {
      this.sqliteProvider.getGuaranDetails(this.guarantor[0].refId).then(data => {
        this.guapromoterLength = data.length;
      });
      this.sqliteProvider.getPresentAddress(this.guarantor[0].refId, this.guarantor[0].id).then(data => {
        this.guapresentAdrsLength = data.length;
      });
      this.sqliteProvider.getPermanentAddress(this.guarantor[0].refId, this.guarantor[0].id).then(data => {
        this.guapermaAdrsLength = data.length;
      });
      this.sqliteProvider.getPromoterProofDetails(this.guarantor[0].refId, this.guarantor[0].id).then(data => {
        this.guapromoProofLength = data.length;
      });
    }

    if (this.applicant.coAppFlag == 'Y') {
      this.sqliteProvider.getCoappDetails(this.coapplicants[0].refId).then(data => {
        this.coappLength = data.length;
      });
      this.sqliteProvider.getPresentAddress(this.coapplicants[0].refId, this.coapplicants[0].id).then(data => {
        this.coapppresentAdrsLength = data.length;
      });
      this.sqliteProvider.getPermanentAddress(this.coapplicants[0].refId, this.coapplicants[0].id).then(data => {
        this.coapppermaAdrsLength = data.length;
      });
      this.sqliteProvider.getPromoterProofDetails(this.coapplicants[0].refId, this.coapplicants[0].id).then(data => {
  ...
  submitCall() {
    try {
      if ((this.network.type == 'none') || (this.network.type == 'unknown')) {
        this.globalData.showAlert("Alert!", "Please check your network connection!!!");
      } else {
        if (this.iagree) {
          // this.submitLeads();
          if (this.applicant.coAppFlag == 'Y') {
              ...
              if (this.appHimarkCheckStat == '1') {
                if (this.coappCibilCheckStat) {
                  if (this.coappHimarkCheckStat) {
                    ...
                    //     // this.lottieSubmit();
                    //   } else {
                    //     this.globalData.showAlert("Alert!", "Please complete all guarantor's hi-mark status!");
                    //   }
                    // } else {
                    //   this.globalData.showAlert("Alert!", "Please complete all guarantor's cibil status!");
                    // }
                  } else {
                    this.globalData.showAlert("Alert!", "Please complete all co-applicant's hi-mark status!");
                  }
                } else {
                  this.globalData.showAlert("Alert!", "Please complete all co-applicant's cibil status!");
                }
              } else {
                this.globalData.showAlert("Alert!", "Please complete applicant's hi-mark status!");
              }
            } else {
              this.globalData.showAlert("Alert!", "Please complete applicant's cibil status!");
            }
          ...
          this.globalData.showAlert("Alert!", "Please Agree Before Submit!");
        }
      }
    } catch (error) {
      console.log(error.message)
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "yyyy-MM-dd HH:mm:ss"), "submitCall", "submitCall", JSON.stringify(error));
    }
  }
  ...
  async lottieSubmit() {
    ...
    let modal = await this.modalCtrl.create({
      ...
      componentProps: { showBackdrop: true, enableBackdropDismiss: false, cssClass: "submit-modal" }
    });
    modal.onDidDismiss().then(data => {
      console.log(data, 'submitmodal ondismiss');
      this.navCtrl.navigate(['/ExistingPage'], { queryParams: { username: this.username, _leadStatus: 'online' } ,skipLocationChange: true, replaceUrl: true});
    })
    modal.present();
  }
  ...
  submitLeads() {
    try {
      ...
      if (this.submitStat === '1') {
        ...
        this.globalData.showAlert("Alert!", "This Lead already Submitted!");
      } else {
        ...
        this.sqliteProvider.getSubmitDetails(this.applicant.refId, this.applicant.id).then(submitlead => {
          if (submitlead[0].applicationNumber == "0" || submitlead[0].applicationNumber == null || submitlead[0].applicationNumber == "") {
            this.submitdata = {
              "LeadMain": {
                "Lead": {
                  ...
                  "LoanDetails": {
                    ...
                    "ProcessingFee": this.items[0].processingFee.toString().split('.')[0],    //new
            ...
            this.master.restApiCallAngular('finalSubmission', this.submitdata).then(async (data) => {
              ...
              if ((<any>data).LeadResMain.SuccessMsg === '000') {
                ...
                if (submitResponse.CustomerDetails.length < 1) {
                  ...
                  try {
                    customerDetailsApi = await this.master.restApiCallAngular('getVLcustdetails', { "propNo": this.applicationNumber });
                  } catch (error) {
                    console.log(error);
                    customerDetailsApi = { ErrorCode: '111' }
                  }
                  if (customerDetailsApi.ErrorCode == '000') {
                    this.sqliteProvider.updateNewSubmitDetails(this.applicationNumber, customerDetailsApi.CustomerDetails[0].SfosLeadId, customerDetailsApi.CustomerDetails[0].LpLeadId, customerDetailsApi.CustomerDetails[0].LpUrn, customerDetailsApi.CustomerDetails[0].LpCustid, this.applicant.refId, this.applicant.id, 'A')
                    if (customerDetailsApi.CustomerDetails.length > 1) {
                      for (let i = 1; i < customerDetailsApi.CustomerDetails.length; i++) {
                        this.sqliteProvider.updateNewSubmitDetails(this.applicationNumber, customerDetailsApi.CustomerDetails[i].SfosLeadId, customerDetailsApi.CustomerDetails[i].LpLeadId, customerDetailsApi.CustomerDetails[i].LpUrn, customerDetailsApi.CustomerDetails[i].LpCustid, this.applicant.refId, this.coapplicants[i - 1].id, 'C')
                      }
                    }
                  } else {
                    this.globalData.showAlert("Alert!", "Customer Id is not received");
                  }
                } else {
                  this.sqliteProvider.updateNewSubmitDetails(this.applicationNumber, submitResponse.CustomerDetails[0].SfosLeadId, submitResponse.CustomerDetails[0].LpLeadId, submitResponse.CustomerDetails[0].LpUrn, submitResponse.CustomerDetails[0].LpCustid, this.applicant.refId, this.applicant.id, 'A')
                  if (submitResponse.CustomerDetails.length > 1) {
                    for (let i = 1; i < submitResponse.CustomerDetails.length; i++) {
                      this.sqliteProvider.updateNewSubmitDetails(this.applicationNumber, submitResponse.CustomerDetails[i].SfosLeadId, submitResponse.CustomerDetails[i].LpLeadId, submitResponse.CustomerDetails[i].LpUrn, submitResponse.CustomerDetails[i].LpCustid, this.applicant.refId, this.coapplicants[i - 1].id, 'C')
                    }
                  }
                }
                // this.globalFunc.globalLodingDismiss();
                ...
                // this.submitStat = 1;
                this.applicationStatus = (<any>data).LeadResMain.AppStatus;
                ...
                this.globalData.showAlert("Alert!", "Application Submission Failed!");
              }
            }, (err) => {
              // loading.dismiss();
              // this.globalFunc.globalLodingDismiss();
          ...
          this.submitDisable = false;
          console.log("Err", err);
          this.globalData.showAlert("Alert!", "Submission is not happen!!!!");
        })
      }
    } catch (error) {
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "submitleads", "submitleads", JSON.stringify(error));
    }

  }

  async loadAllApplicantDetails() {
    this.sqliteProvider.getGivenKarzaDetailsByLeadid(this.applicant.leadId).then(async kar => {
      if (kar.length > 0) {
      ...
      await this.sqlSupport.getEKYCDetails(this.applicant.coAppGuaId).then(data => {
        if (data.length > 0) {
      ...
      this.sqliteProvider.getEntityDetails(this.applicant.refId, this.applicant.id).then(ent => {
        if (ent.length > 0) {
          this.sqliteProvider.getNonIndSubmitDataDetails(this.applicant.refId).then(data => {
            this.items = [];
            this.items = data;
            let custURN
            ...
            this.sqliteProvider.getPromoterProofDetails(this.items[0].refId, this.items[0].id).then(docs => {
              ...
              if (docs.length > 0) {
                ...
                let passExpiry;
                let passOther;
                if (this.getAppDocNumber("DRIVING LICENSE").promoexpiry == undefined) {
                  dlExpiry = "";
                  other = "";
                } else {
                  dlExpiry = this.getAppDocNumber("DRIVING LICENSE").promoexpiry.substring(8, 10) + "/" + this.getAppDocNumber("DRIVING LICENSE").promoexpiry.substring(5, 7) + "/" + this.getAppDocNumber("DRIVING LICENSE").promoexpiry.substring(0, 4);
                  other = "1";
                }
                if (this.getAppDocNumber("PASSPORT").promoexpiry == undefined) {
                  passExpiry = "";
                  passOther = "";
                } else {
                  passExpiry = this.getAppDocNumber("PASSPORT").promoexpiry.substring(8, 10) + "/" + this.getAppDocNumber("PASSPORT").promoexpiry.substring(5, 7) + "/" + this.getAppDocNumber("PASSPORT").promoexpiry.substring(0, 4);
                  passOther = "1";
                }
                custURN = this.items[0].applicType == 'New' ? null : this.items[0].URNnumber
                this.applicantDetails = [{
                  ...
                  "DOB": this.items[0].dob.substring(8, 10) + "/" + this.items[0].dob.substring(5, 7) + "/" + this.items[0].dob.substring(0, 4),
                  "Gender": this.items[0].gender,
                  "FatherName": this.items[0].fathername,
                  "MotherMaidenName": this.items[0].mothername,
                  "MobileNo": this.globalFunc.basicDec(this.items[0].mobNum),
                  //"PANAvailable": this.items[0].panAvailable,                       //chang
                  // "PANNo": this.getAppDocNumber("PAN CARD").promoIDRef || "",
                  "PANNo": this.globalFunc.basicDec(this.items[0].panNum),
                  ...
                  "SourcingID": this.items[0].sourIdName || this.items[0].sourIdName1 || "",
                  "JanaRefID": this.globalFunc.basicDec(this.getAppDocNumber("AADHAR CARD").promoIDRef) || "",       //Chang
		              "aespFlag": this.appAespFlag,
                  "leadId": this.items[0].coAppGuaId,
                  "VoterId": this.globalFunc.basicDec(this.getAppDocNumber("VOTER ID").promoIDRef) || "",              //Chang
                  "PassportNo": this.globalFunc.basicDec(this.getAppDocNumber("PASSPORT").promoIDRef) || "",           //Chang
                  "PassportExpiry": passExpiry || "",
                  "DrivingLcno": this.globalFunc.basicDec(this.getAppDocNumber("DRIVING LICENSE").promoIDRef) || "",        //new  
                  "dlExpiry": dlExpiry || "",                                                           //newz
                  "NegraIdvalue": this.globalFunc.basicDec(this.getAppDocNumber("NREGA").promoIDRef) || "",                 //new
                  "OtherIdType": this.leadIdType || "",                                            //new
                  "OtherIdvalue": this.globalFunc.basicDec(this.leadIdValue) || "",                                          //new
                  "OtherIdExpireDate": this.leadIdExpiry || "",                              //new
                  "emailId": this.globalFunc.basicDec(this.items[0].email),
                  "SpouseName": "",
                  "AlternateMobno": "",
                  "Vehicletype": "",
                  "ElectricVehicle": "",
                  ...
                  "LineManagerEmail": "",
                  "ResidentialStatus": "",
                  "form60": this.globalFunc.basicDec(this.items[0].form60),
                  "EkycFlag": this.items[0].janaRefId ? "Positive" : "Negative",
                  "CustomerNameasperEkyc": this.items[0].nameAsPerEkyc ? this.items[0].nameAsPerEkyc : "",
                  "aadharChangedFlag": this.items[0].ShowAadharDob,
                  "aadharDob": this.items[0].dobAadhar ? this.items[0].dobAadhar : "",
                  "aadharDobDocument": this.items[0].dobDocument ? this.items[0].dobDocument : "",
                  "NameAsPerPan": this.items[0].panName,
                  "DobAsPerPan": this.items[0].panValidation == 'Y' ? this.items[0].dob.substring(8, 10) + "/" + this.items[0].dob.substring(5, 7) + "/" + this.items[0].dob.substring(0, 4) : '',
                  ...
                  "upiValidate": this.items[0].nameupi && this.items[0].nameupi.length > 0 ? 'Y' : 'N',
                  "CustomerAddressDetails": [
                    {
                      "AddressType": "1",
                      "Address1": this.globalFunc.basicDec(this.items[0].perm_plots),
                      "Address2": this.globalFunc.basicDec(this.items[0].perm_locality),
                      "Address3": this.globalFunc.basicDec(this.items[0].perm_line3),
                      "city": this.globalFunc.basicDec(this.items[0].perm_cities),
                      "State": this.globalFunc.basicDec(this.items[0].perm_states),
                      "pincode": this.globalFunc.basicDec(this.items[0].perm_pincode),
                      "landmark": this.items[0].perm_landmark,
                      "YearIncurrentCity": this.items[0].perm_yrsCurCity,
                      "AddrAsPerKyc": this.items[0].perm_permAdrsKYC,
                      "OtherDoc": this.items[0].perm_manualEntry
                    },
                    {
                      "AddressType": "2",
                      "Address1": this.globalFunc.basicDec(this.items[0].pres_plots),
                      "Address2": this.globalFunc.basicDec(this.items[0].pres_locality),
                      "Address3": this.globalFunc.basicDec(this.items[0].pres_line3),
                      "city": this.globalFunc.basicDec(this.items[0].pres_cities),
                      "State": this.globalFunc.basicDec(this.items[0].pres_states),
                      "pincode": this.globalFunc.basicDec(this.items[0].pres_pincode),
                      "landmark": this.items[0].pres_landmark,
                      "YearIncurrentCity": this.items[0].pres_yrsCurCity,
                ...
                },
                {
                  ...
                  "DateofIncorporation": this.items[0].doi.substring(8, 10) + "/" + this.items[0].doi.substring(5, 7) + "/" + this.items[0].doi.substring(0, 4) || "",
          ...
          this.sqliteProvider.getSubmitDataDetails(this.applicant.refId).then(data => {
            this.items = [];
            this.items = data;
            ...
            this.sqliteProvider.getPromoterProofDetails(this.items[0].refId, this.items[0].id).then(docs => {
              if (docs.length > 0) {
                ...
                this.applicantDetails = [{
                  ...
                  "leadId": this.items[0].coAppGuaId,
                  "VoterId": this.globalFunc.basicDec(this.getAppDocNumber("VOTER ID").promoIDRef) || "",                   //Chang
                  "PassportNo": this.globalFunc.basicDec(this.getAppDocNumber("PASSPORT").promoIDRef) || "",                //new
                  "PassportExpiry": passExpiry || "",
                  "DrivingLcno": this.globalFunc.basicDec(this.getAppDocNumber("DRIVING LICENSE").promoIDRef) || "",        //new  
                  "dlExpiry": dlExpiry || "",                                                           //newz 
                  "NegraIdvalue": this.globalFunc.basicDec(this.getAppDocNumber("NREGA").promoIDRef) || "",                 //new
                  ...
                  "AlternateMobno": this.globalFunc.basicDec(this.items[0].altMobNum),
                  "Vehicletype": this.items[0].vehicleType,
                  ...
                  "JoiningDate": (this.items[0].joinDate) ? this.items[0].joinDate.split("-").reverse().join("/") : "", // change format
                  ...
                  "LineManagerEmail": this.globalFunc.basicDec(this.items[0].lmEmail),                  //changed
  ...
  slideslider(value) {
    this.navCtrl.navigate(['/NewapplicationPage'], { queryParams: { slideValue: value } ,skipLocationChange: true, replaceUrl: true});
  }

  getAppCibilCheckStatus() {
    this.sqliteProvider.getSubmitDetails(this.applicant.refId, this.applicant.id).then(data => {
      ...
      if (!(data[0].applicationNumber == "0" || data[0].applicationNumber == null || data[0].applicationNumber == "")) {
  ...
  getGuaCibilCheckStatus() {
    if (this.applicant.guaFlag == 'Y') {
      ...
      for (let i = 0; i < this.guarantor.length; i++) {
        this.sqliteProvider.getSubmitDetails(this.guarantor[i].refId, this.guarantor[i].id).then(data => {
  ...
  getCoappCibilCheckStatus() {
    if (this.applicant.coAppFlag == 'Y') {
      ...
      for (let i = 0; i < this.coapplicants.length; i++) {
        this.sqliteProvider.getSubmitDetails(this.coapplicants[i].refId, this.coapplicants[i].id).then(data => {
          cibilcheck = cibilcheck + data[0].cibilCheckStat;
          himarkcheck = himarkcheck + data[0].himarkCheckStat;
  ...
  getSecurityDetails() {
    this.sqliteProvider.getAdditionDtlData(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
  ...
  async getApiData(leadId, idType) {
    try {
      let karzaDetails = [];
      let applicantidType = [];
      let applicantType = await this.sqlSupport.getApplicantType(this.appLeadId)
      applicantidType = await this.sqlSupport.getApplicantidTypeByLeadid(leadId, 'licence')
      if (this.getDocumentName(idType) == 'AADHAR' || this.getDocumentName(idType) == 'AADHAR CARD') {
      ...
      else if (applicantidType.length > 0 && applicantidType[0].idType == 'licence') {
    ...
    try {
      this.sqliteProvider.getPersonalDetails(this.applicant.refId, this.applicant.id).then(data => {
        if (data.length > 0) {
          ...
          this.sqliteProvider.getPromoterProofDetails(this.applicant.refId, this.applicant.id).then(proof => {
            if (proof.length > 0) {
              for (let i = 0; i < proof.length; i++) {
                this.sqliteProvider.getpromoterproofImages(proof[i].pproofId).then(imgs => {
                  if (imgs.length > 0) {
                    for (let j = 0; j < imgs.length; j++) {
                      if (imgs[j].imgpath != undefined && imgs[j].imgpath != "") {
                        ...
                        let apidata = this.getApiData(this.appLeadId, proof[i].promoIDType);
                        let docReq = {
                          "DocId": proof[i].promoIDType,
                          "Document": docImg,
                          "leadID": this.appLeadId,
                          "Mandatory": this.getMandatoryCheck(proof[i].promoIDType),
                          "DocName": "Img_" + this.appLeadId + "_" + proof[i].promoIDType + "_" + j + ".jpg",
      ...
      this.sqlSupport.insertErrorLog(error.stack, "SubmitPage-getApplicantDocs")
    ...
    try {
      if (this.applicant.coAppFlag == 'Y') {
        for (let k = 0; k < this.coapplicants.length; k++) {
          this.sqliteProvider.getPersonalDetailsForPanValue(this.coapplicants[k].refId, this.coapplicants[k].userType).then(data => {
            this.CoappPANAvailable = data[0].panAvailable
            console.log(this.CoappPANAvailable);
          })
          this.sqliteProvider.getPersonalDetails(this.coapplicants[k].refId, this.coapplicants[k].id).then(data => {
            if (data.length > 0) {
              this.sqliteProvider.getPromoterProofDetails(this.coapplicants[k].refId, this.coapplicants[k].id).then(async docs => {
                if (docs.length > 0) {
                  ...
                  let passExpiry;
                  let passOther;
                  if (this.getCoAppDocNumber("DRIVING LICENSE").promoexpiry == undefined) {
                    dlExpiry = "";
                    other = "";
                  } else {
                    dlExpiry = this.getCoAppDocNumber("DRIVING LICENSE").promoexpiry.substring(8, 10) + "/" + this.getCoAppDocNumber("DRIVING LICENSE").promoexpiry.substring(5, 7) + "/" + this.getCoAppDocNumber("DRIVING LICENSE").promoexpiry.substring(0, 4);
                    other = "1";
                  }
                  if (this.getCoAppDocNumber("PASSPORT").promoexpiry == undefined) {
                    passExpiry = "";
                    passOther = "";
                  } else {
                    passExpiry = this.getCoAppDocNumber("PASSPORT").promoexpiry.substring(8, 10) + "/" + this.getCoAppDocNumber("PASSPORT").promoexpiry.substring(5, 7) + "/" + this.getCoAppDocNumber("PASSPORT").promoexpiry.substring(0, 4);
                    passOther = "1";
                  }
                  console.log("CoappAppExistingUniquenum ", this.coapplicants[k].URNnumber);
                    let coJanaRefID
                    await this.sqlSupport.getEKYCDetails(this.coapplicants[k].coAppGuaId).then(data => {
                      if (data.length > 0) {
                        if (data[0].aespFlag && data[0].aespFlag.length > 0) {
                  ...
                  let coapp = {
                    ...
                    "CoappAlternateMobno": this.globalFunc.basicDec(this.coapplicants[k].altMobNum), // changed
                    ...
                    "CoappMobileNo": this.globalFunc.basicDec(this.coapplicants[k].mobNum),
                    "CoappPANNo": this.globalFunc.basicDec(this.coapplicants[k].panNum),
                    "CoappAppImage": coappPic,
                    "CoappYearsattheResidence": this.coapplicants[k].perm_yrsCurCity,
                    ...
                    "CoappVoterId": this.globalFunc.basicDec(this.getCoAppDocNumber("VOTER ID").promoIDRef) || "",       //new
                    "CoappPassportNo": this.globalFunc.basicDec(this.getCoAppDocNumber("PASSPORT").promoIDRef) || "",    //new
                    "CoappPassportExpiry": passExpiry || "",                                         //new
                    "CoappNegraIdvalue": this.globalFunc.basicDec(this.getCoAppDocNumber("NREGA").promoIDRef) || "",     //new
                    "CoappDrivingLcno": this.globalFunc.basicDec(this.getCoAppDocNumber("DRIVING LICENSE").promoIDRef) || "", //new
                    "CoappdlExpiry": dlExpiry || "",
                    "CoappOtherIdType": other || "",
                    "CoappOtherIdvalue": this.globalFunc.basicDec(this.getCoAppDocNumber("DRIVING LICENSE").promoIDRef) || "",
                    "CoappOtherIdExpireDate": dlExpiry,
                    "CoappemailId": this.globalFunc.basicDec(this.coapplicants[k].email),
                    "CoappEmploymentStatus": this.coapplicants[k].employment,
                    "CoappEmployeeID": this.coapplicants[k].employeeId,               // changed
                    "CoappPresentEmployerName": this.coapplicants[k].employerName,    // changed
                    "CoappDesignation": this.coapplicants[k].designation,             // changed
                    "CoappJoiningDate": (this.coapplicants[k].joinDate) ? this.coapplicants[k].joinDate.split("-").reverse().join("/") : "", // changed
                    ...
                    "CoappVintageServiceBusiness": this.coapplicants[k].vinOfServ,     // changed
                    "CoappLineManagerName": this.coapplicants[k].lmName,               // changed
                    "CoappLineManagerEmail": this.globalFunc.basicDec(this.coapplicants[k].lmEmail),             // changed
                    "CoappResidentialStatus": this.coapplicants[k].resciStatus,        // changed
                    ...
                    "CoappaadharChangedFlag": this.coapplicants[k].ShowAadharDob,
                    "CoappaadharDob": this.coapplicants[k].dobAadhar ? this.coapplicants[k].dobAadhar : "",
                    "CoappaadharDobDocument": this.coapplicants[k].dobDocument ? this.coapplicants[k].dobDocument : "",                       //new
                    "CoappNameAsPerPan": this.coapplicants[k].panName,
                    "CoappDobAsPerPan": this.coapplicants[k].panValidation == 'Y' ? this.coapplicants[k].dob.substring(8, 10) + "/" + this.coapplicants[k].dob.substring(5, 7) + "/" + this.coapplicants[k].dob.substring(0, 4) : '',
                    "CoappPanValidation": this.coapplicants[k].panValidation,
                    "CoappNameValidation": this.coapplicants[k].nameValidation,
                    "CoappDOBValidation": this.coapplicants[k].DOBValidation,
                    "CoappSeedingStatus": this.coapplicants[k].seedingStatus,
                    "CustomerAddressDetails": [
                      {
                        "AddressType": "1",
                        "Address1": this.globalFunc.basicDec(this.coapplicants[k].perm_plots),
                        "Address2": this.globalFunc.basicDec(this.coapplicants[k].perm_locality),
                        "Address3": this.globalFunc.basicDec(this.coapplicants[k].perm_line3),
                        "city": this.globalFunc.basicDec(this.coapplicants[k].perm_cities),
                        "State": this.globalFunc.basicDec(this.coapplicants[k].perm_states),
                        "pincode": this.globalFunc.basicDec(this.coapplicants[k].perm_pincode),
                      ...
                      {
                        "AddressType": "2",
                        "Address1": this.globalFunc.basicDec(this.coapplicants[k].pres_plots),
                        "Address2": this.globalFunc.basicDec(this.coapplicants[k].pres_locality),
                        "Address3": this.globalFunc.basicDec(this.coapplicants[k].pres_line3),
                        "city": this.globalFunc.basicDec(this.coapplicants[k].pres_cities),
                        "State": this.globalFunc.basicDec(this.coapplicants[k].pres_states),
                        "pincode": this.globalFunc.basicDec(this.coapplicants[k].pres_pincode),
                        "landmark": this.coapplicants[k].pres_landmark,
                        "YearIncurrentCity": this.coapplicants[k].pres_yrsCurCity,
                        "AddrAsPerKyc": this.coapplicants[k].pres_presmAdrsKYC,
                      ...
                      },
                      {
                        ...
                        "YearIncurrentCity": this.coapplicants[k].busi_yrsCurCity
                      }
                    ]
                  }
                  ...
                  this.sqliteProvider.getPromoterProofDetails(this.coapplicants[k].refId, this.coapplicants[k].id).then(proof => {
                    if (proof.length > 0) {
                      for (let i = 0; i < proof.length; i++) {
                        this.sqliteProvider.getpromoterproofImages(proof[i].pproofId).then(imgs => {
                          if (imgs.length > 0) {
                            for (let j = 0; j < imgs.length; j++) {
                              if (imgs[j].imgpath != undefined && imgs[j].imgpath != "") {
                                // this.base64.encodeFile(imgs[j].imgpath).then(async (base64File: string) => {
                                let docImg = imgs[j].imgpath;
                                let coappappLeadId = this.coapplicants[k].coAppGuaId;
                                let coappapidata = this.getCoAppApiData(coappappLeadId, proof[i].promoIDType);
                                let docReq = {
                                  "DocId": proof[i].promoIDType,
                                  "Document": docImg,
                                  "leadID": this.coapplicants[k].coAppGuaId,
                                  "Mandatory": this.getMandatoryCheck(proof[i].promoIDType),
                                  "DocName": "Img_" + this.coapplicants[k].coAppGuaId + "_" + proof[i].promoIDType + "_" + j + ".jpg",
      ...
      this.sqlSupport.insertErrorLog(error.stack, "SubmitPage-getCoApplicantDetails")
    ...
    try {
      if (this.applicant.guaFlag == 'Y') {
        for (let k = 0; k < this.guarantor.length; k++) {
          this.sqliteProvider.getPersonalDetails(this.guarantor[k].refId, this.guarantor[k].id).then(data => {
            if (data.length > 0) {
              this.sqliteProvider.getPromoterProofDetails(this.guarantor[k].refId, this.guarantor[k].id).then(docs => {
                if (docs.length > 0) {
                  ...
                  console.log("GuaAppExistingUniquenum ", this.guarantor[k].URNnumber);
                  ...
                  let coapp = {
                    ...
                    "GuaDOB": this.guarantor[k].dob.substring(8, 10) + "/" + this.guarantor[k].dob.substring(5, 7) + "/" + this.guarantor[k].dob.substring(0, 4),
                    ...
                    "GuaMobileNo": this.globalFunc.basicDec(this.guarantor[k].mobNum),
                    "GuaPANAvailable": this.guarantor[k].panAvailable,
                    "GuaPANNo": this.globalFunc.basicDec(this.guarantor[k].panNum),
                    "GuaCustomerNameperKYC": this.globalFunc.basicDec(this.getKarzaData(this.guarantor[k].coAppGuaId)),
                    ...
                    "GuaAppImage": guaPic,
                    "GuaExistingCustomer": this.guarantor[k].custType == 'E' ? 'Y' : 'N',
                    "GuaJanaRefID": this.globalFunc.basicDec(this.getAppDocNumber("AADHAR CARD").promoIDRef) || "",
                    "GualeadId": this.guarantor[k].coAppGuaId,
                    "GuaVoterId": this.globalFunc.basicDec(this.getGuaDocNumber("VOTER ID").promoIDRef) || "",
                    "GuaPassportNo": this.globalFunc.basicDec(this.getGuaDocNumber("PASSPORT").promoIDRef) || "",
                    "GuaOtherIdType": "2",
                    "GuaOtherIdvalue": this.globalFunc.basicDec(this.getGuaDocNumber("DRIVING LICENSE").promoIDRef) || "",
                    "GuaOtherIdExpireDate": this.getGuaDocNumber("DRIVING LICENSE").promoexpiry.substring(8, 10) + "/" + this.getGuaDocNumber("DRIVING LICENSE").promoexpiry.substring(5, 7) + "/" + this.getGuaDocNumber("DRIVING LICENSE").promoexpiry.substring(0, 4),
                    "GuaemailId": this.globalFunc.basicDec(this.guarantor[k].email),
                    "CustomerAddressDetails": [
                      {
                        "AddressType": "1",
                        "Address1": this.globalFunc.basicDec(this.guarantor[k].perm_plots),
                        "Address2": this.globalFunc.basicDec(this.guarantor[k].perm_locality),
                        "Address3": "",
                        "city": this.globalFunc.basicDec(this.guarantor[k].perm_cities),
                        "State": this.globalFunc.basicDec(this.guarantor[k].perm_states),
                        "pincode": this.globalFunc.basicDec(this.guarantor[k].perm_pincode),
                        "landmark": this.guarantor[k].perm_landmark,
                        "YearIncurrentCity": this.guarantor[k].perm_yrsCurCity,
                        "AddrAsPerKyc": this.guarantor[k].perm_permAdrsKYC,
                        "OtherDoc": this.guarantor[k].perm_manualEntry
                      },
                      {
                        "AddressType": "2",
                        "Address1": this.globalFunc.basicDec(this.guarantor[k].pres_plots),
                        "Address2": this.globalFunc.basicDec(this.guarantor[k].pres_locality),
                        "Address3": "",
                        "city": this.globalFunc.basicDec(this.guarantor[k].pres_cities),
                        "State": this.globalFunc.basicDec(this.guarantor[k].pres_states),
                        "pincode": this.globalFunc.basicDec(this.guarantor[k].pres_pincode),
                        "landmark": this.guarantor[k].pres_landmark,
                        "YearIncurrentCity": this.guarantor[k].pres_yrsCurCity,
                  ...
                  this.sqliteProvider.getPromoterProofDetails(this.guarantor[k].refId, this.guarantor[k].id).then(proof => {
                    if (proof.length > 0) {
                      for (let i = 0; i < proof.length; i++) {
                        this.sqliteProvider.getpromoterproofImages(proof[i].pproofId).then(imgs => {
                          if (imgs.length > 0) {
                            for (let j = 0; j < imgs.length; j++) {
                              if (imgs[j].imgpath != undefined && imgs[j].imgpath != "") {
                                ...
                                let docReq = {
                                  ...
                                  "Mandatory": this.getMandatoryCheck(proof[i].promoIDType),
                                  "DocName": "Img_" + this.guarantor[k].coAppGuaId + "_" + proof[i].promoIDType + "_" + j + ".jpg"
                                }
                                this.docFinalReq.push(docReq);
                                // });
                              }
      ...
      this.sqlSupport.insertErrorLog(error.stack, "SubmitPage-getGuarantorDetails")
  ...
  IMDSubmit(appNo) {
    this.sqlSupport.getImdDetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
        this.IMD_data = data;
        this.getImdImgs(this.IMD_data[0].imdId, appNo);
      } else {
        this.globalData.showAlert("Alert!", "IMD data not found");
      }
    }).catch(e => {
      console.log("er" + e);
      this.items = [];
  ...
  initImdSubmit(appNo) {
    try {
      ...
      this.imdSubmitData =
      {
        ...
        "InstrumentDate": this.IMD_data[0].imdDate.substring(8, 10) + "/" + this.IMD_data[0].imdDate.substring(5, 7) + "/" + this.IMD_data[0].imdDate.substring(0, 4),
      ...
      this.master.restApiCallAngular('IMDCheck', this.imdSubmitData).then(data => {
        if ((<any>data).code == "000") {
          // this.globalFunc.globalLodingDismiss();
          // this.casaInit(appNo);
          this.sqlSupport.updateSubmitIMDDetails('1', this.appStatId).then(imd => {
            this.casaInit(appNo);
          });
          ...
          //  this.loading.dismiss();
          this.globalFunc.globalLodingDismiss();
          this.globalData.showAlert("Alert!", "Application IMD Details submission Failed!");
        } else {
          this.documentCheck = false;
          // this.loading.dismiss();
          this.globalFunc.globalLodingDismiss();
          this.globalData.showAlert("Alert!", "Something went wrong! IMD Details not submitted");
        }
      }, err => {
        this.documentCheck = false;
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "initImdSubmit", "initImdSubmit", JSON.stringify(error));
    }
    ...
    try {
      ...
      this.master.restApiCallAngular('CurrentAccDet', this.casaSubmitData).then(data => {
        if ((<any>data).ErrorCode == "000") {
          // this.loading.dismiss();
          this.sqlSupport.updateSubmitCASADetails('1', this.appStatId).then(data => {
            // this.globalFunc.globalLodingDismiss();
            this.getVehicleDetails(appNo);
          })
        } else {
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "casaInit", "casaInit", JSON.stringify(error));
    }
  ...
  getVehicleDetails(appNo) {
    try {
      ...
      const dealer = this.dummy_masterDealer.find(val => val.dealerCode == this.vehicle_data[0].dealerName);
      if (this.vehicleType == '1') {
        this.vehicleSubmitData = {
          ...
          "InsPolicyNo": this.vehicle_data[0].insPolicyNo ? this.vehicle_data[0].insPolicyNo : "",
          "InsCompany": this.vehicle_data[0].insCompany ? this.vehicle_data[0].insCompany : "",
          "InsDate": (this.items[0].insDate) ? this.items[0].insDate.split("-").reverse().join("/") : "",
          "InsExpireDate": (this.items[0].insExpDate) ? this.items[0].insExpDate.split("-").reverse().join("/") : "",
          "InsValue": this.vehicle_data[0].insValue ? this.vehicle_data[0].insValue : "",
        ...
        this.vehicleSubmitData = {
          ...
          "DealerAddress": this.vehicle_data[0].dealerAddress,
          "DealerAccountNumber": this.vehicle_data[0].dealerCurAcc,
          "UsedRegistrationdate": (this.items[0].registrationDate) ? this.items[0].registrationDate.split("-").reverse().join("/") : "",
      ...
      this.master.restApiCallAngular('vehicleDetails', this.vehicleSubmitData).then(data => {
        let vehicleSubmitData = <any>data;
        if (vehicleSubmitData.ErrorCode == "000") {
          // this.loading.dismiss();
          this.sqlSupport.updateSubmitVehicleSubmitDetails('1', this.appStatId).then(data => {
            // this.globalFunc.globalLodingDismiss();
            // this.NACHinit(appNo);
          ...
          if (vehicleSubmitData.ErrorDesc) {
            this.globalData.showAlert("Alert!", vehicleSubmitData.ErrorDesc);
          } else {
            this.globalData.showAlert("Alert!", "Vehicle details submission failed..!");
          }
        }
      }, err => {
        this.documentCheck = false;
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "getVehicleDetails", "getVehicleDetails", JSON.stringify(error));
    }
  ...
  getInsuranceSubmitDetails(appNo) {
    try {
      let nomDOB = this.vehicle_data[0].nomDOB ? this.vehicle_data[0].nomDOB.split("-").reverse().join("/") : ''
      ...
      this.master.restApiCallAngular('LifeInsuranceDetails', this.insuranceData).then(data => {
        ...
        if (insuranceData.ErrorCode == "000") {
          ...
          if (insuranceData.ErrorDesc) {
            this.globalData.showAlert("Alert!", insuranceData.ErrorDesc);
          } else {
            this.globalData.showAlert("Alert!", "Insurance details submission failed..!");
          }
        }
      }, err => {
        this.globalFunc.globalLodingDismiss();
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "getInsuranceSubmitDetails", "getInsuranceSubmitDetails", JSON.stringify(error));
    }
  }
  ...
  //         this.globalData.showAlert("Alert", (<any>data).ErrorDesc);
  //       } else {
  //         this.globalData.showAlert("Alert!", "NACH details submission failed");
  //       }
  //     }
  //   }, err => {
  //     // this.loading.dismiss();
  ...
  CASASubmitData() {
    this.sqlSupport.getCASADetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
        this.CASA_data = data;
        if (this.CASA_data[0].janaAcc == 'Y') {
          if (this.CASA_data[0].nomAvail == 'Y') {
        ...
        if (this.items[0].constitution == '001' || this.items[0].constitution == '004') {
          this.globalData.showAlert("Alert!", "CA/SA data not found");
        }
      }
    }).catch(e => {
      console.log("er" + e);
  ...
  getNomineeDetails() {
    this.sqlSupport.getNomineeDetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
  ...
  getAllVehcileDetails() {
    this.sqliteProvider.getVehicleDetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
        ...
        this.globalData.showAlert("Alert!", "vehicle details not found");
      }
    }).catch(e => {
      console.log("er" + e);
      this.items = [];
  ...
  getServiceDetails() {
    if (this.CASA_data[0].janaAcc == 'Y') {
      this.sqlSupport.getServDetails(this.applicant.refId, this.applicant.id).then(data => {
        if (data.length > 0) {
          this.service_data = data;
          // this.getServiceImgs(this.service_data[0].serId);
        } else {
          if (this.items[0].constitution == '001' || this.items[0].constitution == '004') {
            this.globalData.showAlert("Alert!", "Service data not found");
          }
        }
      }).catch(e => {
        console.log("er" + e);
  ...
  getNachImgs(nachId) {
    this.sqlSupport.getNachStateImages(nachId).then(nach => {
      if (nach.length > 0) {
        this.nach_imgs = [];
  ...
  getServiceImgs(serId) {
    this.sqlSupport.getSignImages(serId).then(sign => {
      if (sign.length > 0) {
        ...
        this.sqliteProvider.getSubmitDetails(this.applicant.refId, this.applicant.id).then(submitlead => {
          if (submitlead[0].IMD != "0" && submitlead[0].CASA != "0" && submitlead[0].NACH != "0") {
            this.uploadDocs();
          }
        });
        // }, (err) => {
  ...
  async uploadDocs() {
    try {
      ...
      this.master.restApiCallAngular('UploadDocs', docs_upload).then(async data => {
        if ((<any>data).errorCode == "00") {
            ...
            buttons: [
              {
                ...
                handler: () => {
                  ...
                  // this.loading.dismiss();
                  // this.globalFunc.globalLodingDismiss();
                  this.navCtrl.navigate(['/ExistApplicationsPage'],{skipLocationChange: true, replaceUrl: true});
                }
              }
            ]
          });
          ...
          this.submitDisable = false;
          this.docUpload = true;
          this.globalData.showAlert("Alert!", "Documents submission Failed!");
        }
      }, err => {
        this.globalFunc.globalLodingDismiss();
        this.documentCheck = false;
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(format(new Date(), "YYYY-MM-DD HH:mm:ss"), "uploadDocs", "uploadDocs", JSON.stringify(error));
    }
  ...
  getNACHDetails() {
    this.sqlSupport.getNachDetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
        this.NACH_data = data;
        console.log(this.NACH_data, "nach details");
      } else {
        this.globalData.showAlert("Alert!", "NACH details not found");
      }
    }).catch(e => {
      console.log("er" + e);
      this.NACH_data = [];
  ...
  getreferenceDetails() {
    this.sqlSupport.getreferenceDetails(this.applicant.refId, this.applicant.id).then(data => {
      if (data.length > 0) {
        ...
        this.globalData.showAlert("Alert!", "Reference details not found");
      }
    }).catch(e => {
      console.log("er" + e);
  ...
  getDocumentStatusCheck(): Promise<any> {
    try {
      ...
      this.sqlSupport.insertErrorLog(error.stack, "SubmitPage-getDocumentStatusCheck")
  ...
  //                   // this.globalData.showAlert("Alert!", `${this.docFinalReq.length} documents uploaded successfullly `)
  //                   this.documentCheck = false;
  ...
  //             this.globalData.showAlert(message, submessage);
  //             reject(false);
  ...
  documentUpload(document, flag): Promise<any> {
    try {
      ...
      console.log(error.message);
      this.globalData.showAlert("Alert!", error.message);
      this.sqliteProvider.addAuditTrail(moment(new Date()).format("YYYY-MM-DD HH:mm:ss"), "documentUpload", "documentUpload", JSON.stringify(error));
    }
  }
