...
export class CreditCheckPage implements OnInit {
  ...
  getCreditFlowGroup(){
    this.sqliteProvider.getVehicleWorkflowList('Credit Review').then(creditDesc => {
  ...
  checkEligibile() {
    ...
    this.globalFun.showAlert('Alert', 'Application is eligible.Proceed further');
  ...
  getGroupId() {
    this.sqlsupport.getGroupIdBasedOnUser(this.global.basicDec(localStorage.getItem('username'))).then(data => {
      if(data.length > 0) {
  ...
  getRequiredScore() {
    this.sqlsupport.getRequiredScoreBasedOnPrd(localStorage.getItem('product')).then(data => {
      this.requiredScore = data[0].prdCibilScore;
      if (this.userType == 'A') {
        if (this.requiredScore == '' || this.requiredScore == null || this.requiredScore == undefined) {
          this.global.showAlert('Alert', 'The Product Score is Empty');
        } else {
  ...
  forwardToWeb() {
    ...
    this.master.restApiCallAngular('mobileWorkflow', body).then(res => {
      ...
      if (creditCheck.ErrorCode === '000') {
        ...
        this.sqlsupport.updateWorkFlowStatus(this.applicantDetails[0].applicationNumber);
        ...
        this.getAppStatus();
        this.global.showAlert('Alert', "Successfully Forwarded");
        ...
        this.global.globalLodingDismiss();
        this.global.showAlert('Alert', creditCheck.ErrorDesc);
      }
  ...
  proceedNextPage() {
    this.globalFun.proccedOk("Alert", "Application forwarded to web for credit check").then(data => {
      if (data == "yes") {
