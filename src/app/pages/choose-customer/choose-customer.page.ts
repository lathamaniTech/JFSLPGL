...
export class ChooseCustomerPage implements OnInit {
  ...
  constructor(public globalFunc: GlobalService, public router: Router,
    public activeRoute: ActivatedRoute,
    public formBuilder: FormBuilder,
    public sqliteProvider: SqliteService, public globalData: DataPassingProviderService,
    public master: RestService, public network: Network,
      ...
      this.master.restApiCallAngular('ExCustDetails', cifvalues).then((data) => {
        ...
        if ((<any>data).ErrorCode == "000") {
          ...
          if ((<any>data).AppId == '0') {
            ...
            this.globalData.globalLodingDismiss();
            this.globalData.showAlert("Alert!", (<any>data).ErrorMsg);
          }
        }
  ...
  posidexCheck(value) {
    ...
    if (value.aadhar != "" && value.aadhar != null && value.aadhar != undefined) { idValue = value.aadhar }
    else if (value.pan != "" && value.pan != null && value.pan != undefined) { idValue = value.pan }
    else if (value.voter != "" && value.voter != null && value.voter != undefined) { idValue = value.voter }
    else if (value.license != "" && value.license != null && value.license != undefined) { idValue = value.license }
    else if (value.passport != "" && value.passport != null && value.passport != undefined) { idValue = value.passport }
    else {
      idValue = "";
    }
    this.sqliteProvider.getPromoterProofDetailsByProof(refId, idValue).then(data => {
      if (data.length == 0) {
        ...
        this.globalData.globalLodingDismiss();
        this.globalData.showAlert("Alert", "Same ID proof is captured in the same application!!!");
      }
    })
  }
    ...
    this.master.restApiCallAngular('AadharInsertVoulting', body).then((result) => {
      if (result != undefined && result != null && result != '') {
        if ((<any>result).status === '00') {
          this.sqliteSupport.duplicateAadharCheck(this.globalData.getrefId(), (<any>result).janaId).then(data => {
            if (data.length > 0) {
              this.globalData.globalLodingDismiss();
              this.globalData.showAlert("Alert!", "Given Aadhar number is already added in this application!");
            } else {
              this.globalData.globalLodingDismiss();
          ...
          this.globalData.globalLodingDismiss();
          this.globalData.showAlert("Alert!", (<any>result).error);
        }
      }
    }, err => {
      if (err.name == "TimeoutError") {
      ...
      if (data) {
        if (idType == 'pan') {
          ...
          this.router.navigate(['/NewapplicationPage'], {
            queryParams: {
              voter: data, leadStatus: this.leadStatus, leadId: leadId, userType: this.globalData.getborrowerType()
  ...
  aadharExistbtnCheck() {
    ...
    this.globalData.showAlert('Alert', 'Ekyc is not completed');
  }
  ...
  secKyc(idType, idNumber, leadId, body?) {
    this.router.navigate(['/secondKycPage'], { queryParams: { data: JSON.stringify(body), idType: idType, idNumber: idNumber, leadId: leadId, leadStatus: this.leadStatus, userType: this.globalData.getborrowerType() } , replaceUrl : true, skipLocationChange:true})
  }
