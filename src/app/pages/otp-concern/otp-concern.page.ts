...
export class OtpConcernPage {
  ...
  constructor(public navCtrl: NavController, 
    ...
    public globalData: DataPassingProviderService, public network: Network, 
    public http: HTTP, public loadCtrl: LoadingController, 
    public alertCtrl: AlertController, public toastCtrl: ToastController, 
    public master: RestService, public router: Router,
    // public base64: Base64, 
    public sqlSupport: SquliteSupportProviderService, private globFunc: GlobalService) {
  ...
  getGurantorDetails() {
    this.sqliteProvider.getGuaranDetails(this.refId).then(data => {
      this.guitems = [];
      this.guitems = data;
    }).catch(e => {
      console.log("er" + e);
  ...
  async cibilCheck() {
    if (this.network.type == 'none' || this.network.type == "unknown") {
      ...
      if (this.cibilCheckStat === '1') {
        ...
        this.globalData.showAlert("Alert!", "Already Cibil Checked!");
      } else {
        let body = {
          "CibilMobRequest": {
            "memberReferenceNum": this.items[0].coAppGuaId ? this.items[0].coAppGuaId : this.items[0].leadId,
            ...
            "addressSegment": [
              {
                ...
                "addressLine2": this.globFunc.basicDec(this.items[0].perm_locality),
        ...
        this.master.restApiCallAngular('Cibil', body).then((res) => {
          loading.dismiss();
          if (!this.urlCheck) {
            if ((<any>res).CibilMobResponseType.statusCode == '000') {
              ...
              if (this.cibilResult.Color === 'Green') {
                ...
                this.sqliteProvider.updateSubmitDetails(this.cibilCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, (<any>res).CibilMobResponseType.Color, (<any>res).CibilMobResponseType.Score, this.statId);
              ...
              this.globalData.showAlert("Alert!", (<any>res).CibilMobResponseType.statusCode + ": " + (<any>res).CibilMobResponseType.statusDescription);
            }
          } else {
            if (this.items[0].pres_plots === null) {
              this.globalData.showAlert("Alert!", "Please add Address Details for the Applicant!");
            } else if (this.cibilCheckStat === '0') {
              this.green = true;
              this.GcibilScore = '750';
              this.cibilCheckStat = 1;
              this.applicationStatus = "Not";
              this.sqliteProvider.updateSubmitDetails(this.cibilCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, "GREEN", "750", this.statId);
              //this.globalData.showAlert("Alert!", "Promoter Cibil Check Success1!");
  ...
  getCibilCheckStatus() {
    this.sqliteProvider.getSubmitDetails(this.refId, this.id).then(data => {
      ...
      if (this.cibilCheckStat === '1') {
        this.green = true;
        this.GcibilScore = '750';
        // this.globalData.showAlert("Alert!", "Already Promoter Cibil Checked!");
      }
    });
  }
      ...
      if (this.cibilGuaCheckStat === '1') {
        ...
        let body = {
          "CibilMobRequest": {
            ...
            "DOB": guarantor.dob.substring(8, 10) + guarantor.dob.substring(5, 7) + guarantor.dob.substring(0, 4),
          ...
          if (!this.urlCheck) {
            ...
            if ((<any>res).CibilMobResponseType.statusCode === '000') {
              ...
              if ((<any>res).CibilMobResponseType.Color === 'Green') {
                ...
                this.applicationStatus = "Not";
                this.sqliteProvider.updateSubmitDetails(this.cibilCoappCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, (<any>res).CibilMobResponseType.Color, (<any>res).CibilMobResponseType.Score, this.CstatId);
              ...
              //this.globalData.showAlert("Alert!", "Guarantor Cibil Check Success!");
            ...
            if (guarantor.address1 === null) {
              this.globalData.showAlert("Alert!", "Please add Address Details for the Guarantor!");
            } else if (this.cibilCoappCheckStat === '0') {
              this.guagreen = true;
              // this.GcibilGuaScore = '750';
              this.cibilCoappCheckStat = 1;
              this.applicationStatus = "Not";
              this.sqliteProvider.updateSubmitDetails(this.cibilCoappCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, "GREEN", "750", this.CstatId);
              //this.globalData.showAlert("Alert!", "Guarantor Cibil Check Success!");
          ...
          if (!this.urlCheck) {
            if ((<any>res).CibilMobResponseType.statusCode === '000') {
              ...
              if ((<any>res).CibilMobResponseType.Color === 'Green') {
                ...
                this.applicationStatus = "Not";
                this.sqliteProvider.updateSubmitDetails(this.cibilGuaCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, (<any>res).CibilMobResponseType.Color, (<any>res).CibilMobResponseType.Score, this.GstatId);
              } else if ((<any>res).CibilMobResponseType.Color === 'Amber') {
                this.guaamber = true;
            ...
            if (guarantor.address1 === null) {
              ...
              this.cibilGuaCheckStat = 1;
              this.applicationStatus = "Not";
              this.sqliteProvider.updateSubmitDetails(this.cibilGuaCheckStat, this.submitStat, this.applicationNumber, this.applicationStatus, "GREEN", "750", this.GstatId);
              //this.globalData.showAlert("Alert!", "Guarantor Cibil Check Success!");
              this.message = "Guarantor Cibil Check Success!"
              this.presentToast();
              this.navCtrl.pop();
        ...
        this.master.restApiCallAngular('Smsotp', this.smsbody).then((data) => {
          ...
          if (this.urlType) {
            ...
            if ((<any>data).SMS.Response.statusCode === 'Success') {
              ...
              // this.globalData.showAlert("Alert!", "CIBIL check success!!");
              // this.navCtrl.pop();
            }
          }
        }, (err) => {
    ...
    this.sqliteProvider.GetCbildocs(this.id).then(data => {
      ...
      if (data.length > 0) {
        for (var i = 0; i < data.length; i++) {
          ...
          if (this.getDocumentName(data[i].promoIDType) != "AADHAR" && this.getDocumentName(data[i].promoIDType) != "AADHAR CARD") {
  ...
  rejectApplication() {
    this.submitData = {
      "LeadMain": {
        "Lead": {
          ...
          "AppCreationdt": this.items[0].appRevd.substring(0, 2) + "/" + this.items[0].appRevd.substring(3, 5) + "/" + this.items[0].appRevd.substring(6, 10),
          ...
          "ApplicantDetails": [
            {
              ...
              "DateofInc": this.items[0].doi.substring(8, 10) + "/" + this.items[0].doi.substring(5, 7) + "/" + this.items[0].doi.substring(0, 4),
              "NicCode": "", //this.items[0].nic
              "OfficePremises": this.items[0].premises,
              "YearsInCity": this.items[0].yrsCurBusiAddr + "~" + this.items[0].monCurBusiAddr,
          ...
          "CoApplicantDetails": [
            {
              ...
              "DobCoapp": this.items[0].dob.substring(8, 10) + "/" + this.items[0].dob.substring(5, 7) + "/" + this.items[0].dob.substring(0, 4),
    ...
    this.master.restApiCallAngular('RejectedLeads', this.submitData).then((res) => {
